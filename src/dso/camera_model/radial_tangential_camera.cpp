...
bool RadtanCamera::Project(
  ...
  if (d_img_d_p3d) {
    number_t du_dx = fx *
                     (1 + k1 * r2 + k2 * r4 + 2 * k1 * x2 + 4 * k2 * x2 * r2 +
                      2 * p1 * y + 6 * p2 * x) *
                     (1.0f / p_3d[2]);
    number_t du_dy =
        fx * (2 * k1 * xy + 4 * k2 * r2 * xy + 2 * p1 * x + 2 * p2 * y) *
        (1.0f / p_3d[2]);
    number_t du_dz = -1.0f * (x * du_dx + y * du_dy);
    number_t dv_dx =
        fy * (2 * k1 * xy + 4 * k2 * r2 * xy + 2 * p1 * x + 2 * p2 * y) *
        (1.0f / p_3d[2]);
    number_t dv_dy = fy *
                     (1 + k1 * r2 + k2 * r4 + 2 * k1 * y2 + 4 * k2 * y2 * r2 +
                      6 * p1 * y + 2 * p2 * x) *
                     (1.0f / p_3d[2]);
  ...
  if (d_img_d_param) {
    ...
    (*d_img_d_param) << x, 0.0f, 1.0f, 0.0f, fx * x * r2, fx * x * r4,
        2 * fx * xy, fx * (r2 + 2 * x2), 0, y, 0, 1, fy * y * r2, fy * y * r4,
        fy * (r2 + 2 * y2), 2 * fy * xy;
  }

  number_t rad_dist_u = k1 * r2 + k2 * r2 * r2;
  x +=
      x * rad_dist_u + number_t(2.0) * p1 * xy + p2 * (r2 + number_t(2.0) * x2);
  y +=
      y * rad_dist_u + number_t(2.0) * p2 * xy + p1 * (r2 + number_t(2.0) * y2);
...
bool RadtanCamera::UnProject(
  ...
  for (int i = 0; i < n; ++i) {
    ...
    F(0, 0) = 1 + rad_dist_u + k1 * 2.0 * mx2_u + k2 * rho2_u * 4 * mx2_u +
              2.0 * p1 * y_tmp(1) + 6 * p2 * y_tmp(0);
    F(1, 0) = k1 * 2.0 * y_tmp(0) * y_tmp(1) +
              k2 * 4 * rho2_u * y_tmp(0) * y_tmp(1) + p1 * 2.0 * y_tmp(0) +
              2.0 * p2 * y_tmp(1);
    F(0, 1) = F(1, 0);
    F(1, 1) = 1 + rad_dist_u + k1 * 2.0 * my2_u + k2 * rho2_u * 4 * my2_u +
              6 * p1 * y_tmp(1) + 2.0 * p2 * y_tmp(0);

    y_tmp(0) +=
        y_tmp(0) * rad_dist_u + 2.0 * p1 * mxy_u + p2 * (rho2_u + 2.0 * mx2_u);
    y_tmp(1) +=
        y_tmp(1) * rad_dist_u + 2.0 * p2 * mxy_u + p1 * (rho2_u + 2.0 * my2_u);
