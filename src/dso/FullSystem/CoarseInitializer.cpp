...
CoarseInitializer::CoarseInitializer(int ww, int hh, MultiCamera* p_cam) : thisToNext_aff(0, 0), thisToNext(SE3()) {
  for (int lvl = 0; lvl < pyrLevelsUsed; lvl++) {
    points[lvl] = 0;
    // numPoints[lvl] = 0;
  }
  Rwb = Mat33::Identity();
  JbBuffer = new Vec10f[ww * hh * kCameraNumUsed /* * kCameraNumUsed * kCameraNumUsed*/
  ];                                             // todo roger, align with host uv, rounding pixel
  JbBuffer_new = new Vec10f[ww * hh * kCameraNumUsed /* * kCameraNumUsed * kCameraNumUsed*/];
...
#endif
                false) {
              ptsl[i].iR = ptsl[i].iR_triangle;  // TODO 每个点的深度初值都赋1，hessian赋0
...
/// start from lowest resolution
  ...
  for (int lvl = pyrLevelsUsed - 1; lvl >= 0; lvl--) {
    ...
    Mat88f H, Hsc;
    Vec8f b, bsc;
    for (int lvl_target_ = inner_loop_start_lvl /*pyrLevelsUsed - 1*/; lvl_target_ >= 0; lvl_target_--) {
      if (use_inner_loop) {
        lvl_target = lvl_target_;
        if (lvl_target_ < lvl) {
          // continue;
        }
        max_iter = maxIterations[lvl] > maxIterations[lvl_target] ? maxIterations[lvl] : maxIterations[lvl_target];
      } else {
        lvl_target = -1;  // lvl;
      ...
      //      ++target_cid)
      //      {
      resOld = calcResAndGS(-1, max_iter /*maxIterations[lvl]*/, lvl, H, b, Hsc, bsc, refToNew_current,
                            refToNew_aff_current, false, N, lvl <= 0, lvl_target);
      //      }
      //    }
      ...
      if (printDebug) {
        printf(
            ...
            "(|inc| = %f)! \t",
            lvl, 0, lambda, "INITIA", sqrtf((float)(resOld[0] / resOld[2])), sqrtf((float)(resOld[1] / resOld[2])),
      ...
      while (true) {
        ...
        //        for (int target_cid = 0; target_cid < kCameraNumUsed;
        //        ++target_cid) {
        resNew = calcResAndGS(iteration, max_iter /*maxIterations[lvl]*/, lvl, H_new, b_new, Hsc_new, bsc_new,
                              refToNew_new, refToNew_aff_new, false, N2, lvl <= 0, lvl_target);
        //        }
        //      }
        ...
        printf(
            "is_accept: %d, level: %d, hw_init: %f, [eTotalOld / "
            "eTotalNew]: [%0.1f / "
            "%0.1f], diff: "
            "%0.1f, diff_ratio: %0.4f, iter: %d, level: %d, incNorm: %f, "
            "lambda: "
            "%0.4f. [lvl_h / lvl_t]: [%d %d]\n",
            accept, lvl, resNew[3], eTotalOld, eTotalNew, eTotalNew - eTotalOld, (eTotalNew - eTotalOld) / eTotalOld,
            iteration, lvl, incNorm, lambda, lvl, lvl_target);
        if (printDebug) {
          printf(
              ...
              sqrtf((float)(resNew[1] / resNew[2])), eTotalOld / resNew[2], eTotalNew / resNew[2], incNorm);
        ...
        if (!(incNorm > eps) || iteration >= max_iter /*maxIterations[lvl]*/ || fails >= 3 /*200*/ ||
...
void CoarseInitializer::debugPlot(int lvl, std::vector<IOWrap::Output3DWrapper*>& wraps, SE3 T_th, bool show_details) {
  bool needCall = false;
  if (!show_details) {
  ...
  if (!show_details) {
    ...
    for (int cid = 0; cid < kCameraNumUsed; ++cid) {
      ...
      for (int i = 0; i < wl * hl; i++) iRImg.at(i, cid) = Vec3b(colorRef[i][0], colorRef[i][0], colorRef[i][0]);
    }
    for (int cid = 0; cid < kCameraNumUsed; ++cid) {
      int npts = level_cid_to_numPoints[lvl][cid];
      ...
      for (int i = 0; i < npts; i++) {
        ...
        if (!point->isGood) {
          iRImg.setPixel9(point->u + 0.5f, point->v + 0.5f, Vec3b(0, 0, 0), point->host_cid);
        } else {
          // printf("good point\n");
  ...
  if (true) {
    ...
    for (int host_cid = 0; host_cid < kCameraNumUsed; ++host_cid) {
      ...
      for (int i = 0; i < level_cid_to_numPoints[lvl][host_cid]; i++) {
        Pnt* point = points[lvl] + i + level_cid_to_npts_success_offset[lvl][host_cid];
        if (point->isGood) {
          nid += 1;
#ifndef USE_MULTI_CAM
          sid += point->iR;
      ...
      for (int target_cid = 0; target_cid < kCameraNumUsed; ++target_cid) {
        ...
        SE3 refToNew = newFrame->p_multi_camera->cid_to_T01_SE3[target_cid].inverse() * Tth *
                       newFrame->p_multi_camera->cid_to_T01_SE3[host_cid];
        Mat33f RKi = (refToNew.rotationMatrix() * intr_inv).cast<float>();
        for (int i = 0; i < level_cid_to_numPoints[lvl][host_cid]; i++) {
          Pnt* point_temp = points[lvl] + i + level_cid_to_npts_success_offset[lvl][host_cid];
          ...
          Vec3f pt = RKi * Vec3f(point_temp->u, point_temp->v, 1) + t * point_temp->idepth;
          ...
          //                          << ", idepth: " << new_idepth <<
          //                          std::endl;
          if (!(Ku > 15 && Kv > 15 && Ku < wG[lvl] - 15 && Kv < hG[lvl] - 15 && new_idepth > 0)) {
            //                isGood = false;
            //                break;
            continue;
          }
        ...
        target_cid_to_fac[target_cid] = target_cid_to_nid[target_cid] / target_cid_to_sid[target_cid];
      }
      for (int i = 0; i < level_cid_to_numPoints[lvl][host_cid]; i++) {
        Pnt* point = points[lvl] + i + level_cid_to_npts_success_offset[lvl][host_cid];
        // TODO roger, like SetFromImage in orca, 判断这个坐标纹理是否充分
        ...
        if (point->isGood) {
          ...
          img_host->setPixel9(point->u + 0.5, point->v + 0.5, makeRainbow3B(point->iR /*point->idepth*/ * fac),
                              host_cid);
          img_host->setPixelCirc(point->u + 0.5, point->v + 0.5, makeRainbow3B(point->iR /*point->idepth*/ * fac),
                                 host_cid);
        } else {
          img_host->setPixel9(point->u + 0.5, point->v + 0.5, makeRainbow3B(10), host_cid);
          img_host->setPixelCirc(point->u + 0.5, point->v + 0.5, makeRainbow3B(10), host_cid);
        }
        ...
        for (int target_cid = 0; target_cid < kCameraNumUsed; ++target_cid) {
          ...
          if (point->isGood && point->is_valid_project[target_cid]) {
            img_target->setPixel9(Ku + 0.5, Kv + 0.5, makeRainbow3B(new_idepth * target_cid_to_fac[target_cid]),
                                  target_cid);
            img_target->setPixelCirc(Ku + 0.5, Kv + 0.5, makeRainbow3B(new_idepth * target_cid_to_fac[target_cid]),
                                     target_cid);
          } else {
            if (false) {
              img_target->setPixel9(Ku + 0.5, Kv + 0.5, makeRainbow3B(10), target_cid);
            }
...
Vec4f CoarseInitializer::calcResAndGS(int iter, int max_iter, int lvl, MatStatef& H_out, VecStatef& b_out,
                                      MatStatef& H_out_sc, VecStatef& b_out_sc, const SE3& refToNew_,
                                      AffLight refToNew_aff, bool plot, int& N, bool show_image, int lvl_target_) {
  int lvl_target = 0;
  ...
  for (int host_cid = 0; host_cid < kCameraNumUsed; ++host_cid) {
    ...
    for (int i = 0; i < npts; i++) {
      ...
      Eigen::MatrixXf host_info, target_info, host_info_temp, target_info_temp, host_info_big, target_info_big;
      ...
      std::array<Eigen::MatrixXf, kCameraNumUsed> a_host_info, a_target_info, a_grad_new_host, a_grad_new_target;
      ...
      if (show_image && i % show_step == 0) {
        ...
        for (int cam = 0; cam < kCameraNumUsed; ++cam) {
          Eigen::Vector3f* colorCur = newFrame->dIp[lvl_target] + cam * wl_target * hl_target;
      ...
      MatXXf a_target_sigma_temp, a_err_mean, a_err_max, a_err_min, a_grad_mean, a_grad_max, a_grad_min, a_zncc_mean;
      a_target_sigma_temp.resize(kCameraNumUsed, 1);
      a_target_sigma_temp.setOnes();
      a_target_sigma_temp *= -20;
      a_grad_mean = a_grad_max = a_grad_min = a_err_mean = a_err_max = a_err_min = a_zncc_mean = a_target_sigma_temp;
      for (int target_cam_id = 0; target_cam_id < kCameraNumUsed; ++target_cam_id) {
        // Eigen::Vector3f *colorRef = firstFrame->dIp[lvl] + host_cid * wl *
        // hl;
        if (!use_cross_camera && host_cid != target_cam_id) {
          continue;
        }
        Eigen::Vector3f* colorNew = newFrame->dIp[lvl_target] + target_cam_id * wl_target * hl_target;
        //! 旋转矩阵R * 内参矩阵K_inv
        SE3 refToNew = newFrame->p_multi_camera->cid_to_T01_SE3[target_cam_id].inverse() * refToNew_ *
                       newFrame->p_multi_camera->cid_to_T01_SE3[host_cid];
        Mat33f RKi = (refToNew.rotationMatrix() * Ki[lvl]).cast<float>();
        Vec3f t = refToNew.translation().cast<float>();
        ...
        for (int idx = 0; idx < patternNumSeed; idx++) {
          ...
          Vec3f n_host = Vec3f((point->u + dx - cxl) * fxli, (point->v + dy - cyl) * fyli, 1);
          Rot = refToNew.rotationMatrix().cast<float>();
          trans = refToNew.translation().cast<float>();
          Vec3f X_target_scaled = refToNew.rotationMatrix().cast<float>() * n_host +
                                  refToNew.translation().cast<float>() * point->idepth_new;
          Mat33f X_target_scaled_skew;
          X_target_scaled_skew << (0), -X_target_scaled(2), X_target_scaled(1), X_target_scaled(2), (0),
              -X_target_scaled(0), -X_target_scaled(1), X_target_scaled(0), (0);
          d_uv_d_pt3d << fxl_target / X_target_scaled(2), 0,
              -fxl_target * X_target_scaled(0) / X_target_scaled(2) / X_target_scaled(2), 0,
              fyl_target / X_target_scaled(2),
              -fyl_target * X_target_scaled(1) / X_target_scaled(2) / X_target_scaled(2);
          d_pt3d_d_pose.leftCols(3) = point->idepth_new * Mat33f::Identity();
          d_pt3d_d_pose.rightCols(3) = -X_target_scaled_skew;
          d_uv_d_pose = d_uv_d_pt3d * d_pt3d_d_pose;
          ...
          float new_idepth = point->idepth_new / pt[2];
          // 落在边缘附近，深度小于0, 则不好
          if (!(Ku > 1 && Kv > 1 && Ku < wl_target - 2 && Kv < hl_target - 2 && new_idepth > 0)) {
            //                isGood = false;
            //                break;
            continue;
          }
          // 插值得到新图像中的 patch 像素值，(输入3维，输出3维像素值 +
          // x方向梯度 + y方向梯度)
          Vec3f hitColor = getInterpolatedElement33(colorNew, Ku, Kv, wl_target);
          // printf("level: [%d %d], [w h]: [%d %d], idx: %d, offset: [%f %f],
          // uv: [%f %f], dxdy: [%f %f]\n", lvl,lvl_target, wl, hl,idx,
          // patternPSeed[idx][0], patternPSeed[idx][1], point->u, point->v,
          // dx,dy);
          ...
          assert(std::abs(r2new_aff[0] * rlR + r2new_aff[1] - host_value_corrected) < 0.00001);
          // 像素值有穷, good
          if (!std::isfinite(rlR) || !std::isfinite((float)hitColor[0])) {
            //                isGood = false;
            //                break;
          ...
          // Huber权重
          float hw_temp =
              fabs(residual_temp) < setting_outlierTH_init ? 1 : setting_outlierTH_init / fabs(residual_temp);
          // huberweight * (2-huberweight) = Objective Function
          // robust 权重和函数之间的关系
          float energy_temp = hw_temp * residual_temp * residual_temp * (2 - hw_temp);
          if (std::abs(residual_temp) /*energy_temp*/ > 30 /*static_cast<float>(20 * (lvl + 1))*/) {
            // continue;
          }
          ...
          index_to_host_value.emplace(std::make_pair(idx + MAX_RES_PER_POINT_SEED * target_cam_id, hostColor));
          index_to_target_value.emplace(std::make_pair(idx + MAX_RES_PER_POINT_SEED * target_cam_id, hitColor));
          index_to_count_big.emplace(std::make_pair(idx + MAX_RES_PER_POINT_SEED * target_cam_id, count));
        ...
        if (ratio > ratio_thr1) {
          if (count_each_cam >= MAX_RES_PER_POINT_SEED) {
            ...
            float host_val_mean_temp = host_info_temp.col(0).sum() / static_cast<float>(host_info_temp.rows());
            float target_val_mean_temp = target_info_temp.col(0).sum() / static_cast<float>(target_info_temp.rows());
            ones_temp.conservativeResize(host_info_temp.rows(), 1);
            ones_temp.setOnes();

            host_info_temp.col(0) = host_info_temp.col(0) - host_val_mean_temp * ones_temp;
            target_info_temp.col(0) = target_info_temp.col(0) - target_val_mean_temp * ones_temp;
          ...
          if (count_each_cam > 0 && count_each_cam == MAX_RES_PER_POINT_SEED) {
            ...
            if ((host_sigma_temp > 3.0f && target_sigma_temp > 2.0f) /*||
              ...
              //                 target_sigma_temp);
              if (!calc_stats_only && ((err_min > 30000.f ||
#ifndef USE_ZNCC
                                        err_sum > 20.f
#else
                                        err_sum > 140.f
        ...
        if (count_each_cam >= MAX_RES_PER_POINT_SEED) {
          ...
          if (show_image && i % show_step == 0) {
            for (const Vec2f& uv : uv_draw) {
              img_target->setPixel9(uv[0], uv[1], makeRainbow3B(1.f / static_cast<float>(target_cam_id + 1)),
                                    target_cam_id);
            }
          }
#endif
          ...
          for (int id = 0; id < host_info_temp.rows(); ++id) {
            ...
            index_to_count.emplace(std::make_pair(id + MAX_RES_PER_POINT_SEED * target_cam_id, host_target_info_size));
      ...
      if (patch_num != 0) {
        for (int id = 0; id < kCameraNumUsed; ++id) {
          if (cam_info[id] > 0) {
            // printf("id: %d\n", id);
            host_val_mean = a_host_info[id].col(0).sum() / static_cast<float>(a_target_info[id].rows());
            target_val_mean = a_target_info[id].col(0).sum() / static_cast<float>(a_target_info[id].rows());
            ...
            ones.setOnes();
#ifdef USE_ZNCC
            a_host_info[id].col(0) = a_host_info[id].col(0) - host_val_mean * ones;
            a_target_info[id].col(0) = a_target_info[id].col(0) - target_val_mean * ones;
            ...
            Mat_ZNSSD_I.conservativeResize(a_target_info[id].rows(), a_target_info[id].rows());
            Mat_ZNSSD_I.setIdentity();

            J_ZNSSD_mean = Mat_ZNSSD_I - (ones / static_cast<float>(a_target_info[id].rows())) * ones.transpose();

            J_ZNSSD_J_I_host =
                setting_variableScale * ((Mat_ZNSSD_I - (a_host_info[id].col(0) * a_host_info[id].col(0).transpose())) /
                                         host_sigma * J_ZNSSD_mean);
            J_ZNSSD_J_I_target = setting_variableScale *
                                 ((Mat_ZNSSD_I - (a_target_info[id].col(0) * a_target_info[id].col(0).transpose())) /
                                  target_sigma * J_ZNSSD_mean);

            a_grad_new_host[id] = J_ZNSSD_J_I_host * a_host_info[id].rightCols(2);        // "new" gradient: 8x2
            a_grad_new_target[id] = J_ZNSSD_J_I_target * a_target_info[id].rightCols(2);  // "new" gradient: 8x2
            a_host_info[id].col(0) *= setting_variableScale;
            a_target_info[id].col(0) *= setting_variableScale;
#else
            a_grad_new_host[id] = a_host_info[id].rightCols(2);      // "new" gradient: 8x2
            a_grad_new_target[id] = a_target_info[id].rightCols(2);  // "new" gradient: 8x2
#endif
          }
        }
        // printf("dddd\n");
        ...
        J_ZNSSD_J_I_host = setting_variableScale * ((Mat_ZNSSD_I - (host_info.col(0) * host_info.col(0).transpose())) /
                                                    host_sigma * J_ZNSSD_mean);
        J_ZNSSD_J_I_target =
            setting_variableScale *
      ...
      for (int target_cid = 0; target_cid < kCameraNumUsed; ++target_cid) {
        ...
        const Mat66& extra_pose_jac = newFrame->p_multi_camera->cid_to_T01_inv_Adj[target_cid];
        // Eigen::Vector3f *colorRef = firstFrame->dIp[lvl] + host_cid * wl *
        // hl;
        Eigen::Vector3f* colorNew = newFrame->dIp[lvl_target] + target_cid * wl_target * hl_target;
        //! 旋转矩阵R * 内参矩阵K_inv
        SE3 refToNew = newFrame->p_multi_camera->cid_to_T01_SE3[target_cid].inverse() * refToNew_ *
                       newFrame->p_multi_camera->cid_to_T01_SE3[host_cid];
        Mat33f RKi = (refToNew.rotationMatrix() * Ki[lvl]).cast<float>();
        Vec3f t = refToNew.translation().cast<float>();
        Eigen::Vector2f r2new_aff = Eigen::Vector2f(exp(refToNew_aff.a), refToNew_aff.b);
...
//        float energy = 0;
        ...
        for (int idx = 0; idx < patternNumSeed; idx++) {
          ...
          d_uv_d_pt3d << fxl_target / X_target_scaled(2), 0,
              ...
              fyl_target / X_target_scaled(2),
              -fyl_target * X_target_scaled(1) / X_target_scaled(2) / X_target_scaled(2);
          d_pt3d_d_pose.leftCols(3) = point->idepth_new * Mat33f::Identity();  /// [t R]
          d_pt3d_d_pose.rightCols(3) = -X_target_scaled_skew;
          d_uv_d_pose = d_uv_d_pt3d * d_pt3d_d_pose;
          d_uv_host_d_n_host << fxl, 0, -fxl * n_host(0), 0, fyl, -fyl * n_host(1);
          d_uv_d_pose_inverse_comp =
          ...
          d_uv_target_d_x_target_scaled = d_uv_d_pt3d;
          d_uv_d_pose_fwd_jac =
              (d_uv_target_d_x_target_scaled.cast<double>() * d_pt3d_d_pose.cast<double>()).cast<float>();

          Mat26f d_uv_d_pose_fwd_jac_use =
              (d_uv_target_d_x_target_scaled.cast<double>() * d_pt3d_d_pose.cast<double>() * extra_pose_jac)
                  .cast<float>();
          ...
          Mat26f d_uv_d_pose_inverse_comp_use =
              (d_uv_host_d_n_host.cast<double>() * refToNew.rotationMatrix().transpose() *
               d_pt3d_d_pose.cast<double>() * extra_pose_jac)
                  .cast<float>();
          // printf("xyz: %f\n",pt[2]);
          // 归一化坐标 Pj
          ...
          float residual = hitColor[0] - r2new_aff[0] * rlR - r2new_aff[1];
          // Huber权重
          float hw = fabs(residual) < (setting_outlierTH_init /*+ std::abs(r2new_aff[1])*/)
                         ? 1
                         : (setting_outlierTH_init /*+ std::abs(r2new_aff[1])*/) / fabs(residual);
          if (true) {
          ...
          float residual_bak = hitColor[0] - r2new_aff[0] * rlR - r2new_aff[1];
          // printf("gg\n");
          float residual = 1 * (a_target_info[target_cid](idx, 0) - a_host_info[target_cid](idx, 0));
          if (lvl <= 1 && false) {
            printf("residual: %f, idx: %d, host_val: %f, target_val: %f\n", residual, idx,
                   a_host_info[target_cid](idx, 0), a_target_info[target_cid](idx, 0));
          }
          ...
          float hw = fabs(residual) < (setting_huberTH_init /*+ std::abs(r2new_aff[1])*/)
                         ? 1
                         : (setting_huberTH_init /*+ std::abs(r2new_aff[1])*/) / fabs(residual);
#else
          float norm1 = a_host_info[target_cid].col(0).norm();
          float norm2 = a_target_info[target_cid].col(0).norm();

          float zncc = a_target_info[target_cid].col(0).normalized().dot(a_host_info[target_cid].col(0).normalized());
          float r2 = 2 - 2 * zncc;
...
#else
          float dxInterp = hw * a_grad_new_target[target_cid](idx, 0) * fxl_target;
          float dyInterp = hw * a_grad_new_target[target_cid](idx, 1) * fyl_target;
#endif
          // TODO* 残差对 j(新状态) 位姿求导, 6
          ...
          Vec6f d_res_d_pose_inverse_comp =
              hw * Vec2f(hostColor[1], hostColor[2]).transpose() * d_uv_d_pose_inverse_comp;
          // printf("idx: %d, target_cid: %d, MAX_RES_PER_POINT_SEED: %d,
          // index_to_count: %d, [row col]: [%d %d], grad_new_host [rows, cols]:
          // [%d %d]\n",idx, target_cid,MAX_RES_PER_POINT_SEED,
          // index_to_count.at(idx + MAX_RES_PER_POINT_SEED *
          //                                                  target_cid),
          //                                                  a_grad_new_host[target_cid].rows(),
          //                                                  a_grad_new_host[target_cid].cols(),
          //                                                  grad_new_host.rows(),
          //                                                  grad_new_host.cols());
          // printf("%f\n", grad_new_host(index_to_count.at(idx +
          // MAX_RES_PER_POINT_SEED *
          //                                                  target_cid), 0));
          // printf("%f\n", a_grad_new_host[target_cid](idx, 0));
          assert(std::abs(grad_new_host(index_to_count.at(idx + MAX_RES_PER_POINT_SEED * target_cid), 0) -
                          a_grad_new_host[target_cid](idx, 0)) < 0.0001f);
          assert(std::abs(grad_new_host(index_to_count.at(idx + MAX_RES_PER_POINT_SEED * target_cid), 1) -
                          a_grad_new_host[target_cid](idx, 1)) < 0.0001f);
          assert(std::abs(grad_new_target(index_to_count.at(idx + MAX_RES_PER_POINT_SEED * target_cid), 0) -
                          a_grad_new_target[target_cid](idx, 0)) < 0.0001f);
          assert(std::abs(grad_new_target(index_to_count.at(idx + MAX_RES_PER_POINT_SEED * target_cid), 1) -
                          a_grad_new_target[target_cid](idx, 1)) < 0.0001f);
          Vec6f d_res_d_pose_fwd_jac =
              hw * Vec2f(a_grad_new_target[target_cid](idx, 0), a_grad_new_target[target_cid](idx, 1)).transpose() *
              d_uv_d_pose_fwd_jac;
          show.row(2) = show.row(0) - d_res_d_pose_fwd_jac.transpose();
          Vec6f d_res_d_pose_fwd_jac_use = (static_cast<double>(hw) * Vec2(hitColor[1], hitColor[2]).transpose() *
                                            d_uv_d_pose_fwd_jac_use.cast<double>())
                                               .cast<float>();
          Vec6f d_res_d_pose_inverse_comp_use =
              (static_cast<double>(hw) *
               Vec2(a_grad_new_host[target_cid](idx, 0), a_grad_new_host[target_cid](idx, 1)).transpose() *
               d_uv_d_pose_inverse_comp_use.cast<double>())
                  .cast<float>();
          assert(std::abs(a_grad_new_host[target_cid](idx, 0) - hostColor[1]) < 0.0001f);
          assert(std::abs(a_grad_new_host[target_cid](idx, 1) - hostColor[2]) < 0.0001f);
#else
          show.row(1) =
              show.row(0) -
              hw * Vec2f(a_grad_new_target[target_cid](idx, 0), a_grad_new_target[target_cid](idx, 1)).transpose() *
                  d_uv_d_pose;
          Vec6f d_res_d_pose_inverse_comp =
              hw * Vec2f(a_grad_new_host[target_cid](idx, 0), a_grad_new_host[target_cid](idx, 1)).transpose() *
              d_uv_d_pose_inverse_comp;
          Vec6f d_res_d_pose_fwd_jac =
              hw * Vec2f(a_grad_new_target[target_cid](idx, 0), a_grad_new_target[target_cid](idx, 1)).transpose() *
              d_uv_d_pose_fwd_jac;
          Vec6f d_res_d_pose_fwd_jac_use =
              (static_cast<double>(hw) *
               Vec2(a_grad_new_target[target_cid](idx, 0), a_grad_new_target[target_cid](idx, 1)).transpose() *
               d_uv_d_pose_fwd_jac_use.cast<double>())
                  .cast<float>();
          Vec6f d_res_d_pose_inverse_comp_use =
              (static_cast<double>(hw) *
...
#ifndef USE_ZNCC
          d_uv_d_idp_show(1) =
              d_uv_d_idp_show(0) - hw * Vec2f(hitColor[1], hitColor[2]).transpose() * d_uv_d_pt3d * trans;
          // std::cout<<"d_uv_d_c_show:\n"<<d_uv_d_c_show<<std::endl;
          ...
          float d_res_d_idp_inverse_comp =
              hw * Vec2f(a_grad_new_host[target_cid](idx, 0), a_grad_new_host[target_cid](idx, 1)).transpose() *
              d_uv_d_d_inverse_comp;
          float d_res_d_idp_fwd_jac = hw *
                                      Vec2f(a_grad_new_target[target_cid](idx /*index_to_count.at(idx +
                                                                                 MAX_RES_PER_POINT_SEED * target_cid)*/
                                                                          ,
                                                                          0),
          ...
          dp0[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = new_idepth * dxInterp;  //! dpi/pz' * dxfx
          dp1[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = new_idepth * dyInterp;  //! dpi/pz' * dyfy
          ...
          dp3[idx + MAX_RES_PER_POINT_SEED * target_cid_use] =
              -u * v * dxInterp - (1 + v * v) * dyInterp;  //! - px'py'/pz'^2*dxfy - (1+py'^2/pz'^2)*dyfy
          dp4[idx + MAX_RES_PER_POINT_SEED * target_cid_use] =
              (1 + u * u) * dxInterp + u * v * dyInterp;  //! (1+px'^2/pz'^2)*dxfx + px'py'/pz'^2*dxfy
          dp5[idx + MAX_RES_PER_POINT_SEED * target_cid_use] =
              -v * dxInterp + u * dyInterp;  //! -py'/pz'*dxfx + px'/pz'*dyfy

          dp0[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = d_res_d_pose_fwd_jac_use(0);
          dp1[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = d_res_d_pose_fwd_jac_use(1);
          dp2[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = d_res_d_pose_fwd_jac_use(2);
          dp3[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = d_res_d_pose_fwd_jac_use(3);
          dp4[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = d_res_d_pose_fwd_jac_use(4);
          dp5[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = d_res_d_pose_fwd_jac_use(5);
#else
          dp0[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = d_res_d_pose_fwd_jac_use(0);
          dp1[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = d_res_d_pose_fwd_jac_use(1);
          dp2[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = d_res_d_pose_fwd_jac_use(2);
...
#endif
#else
          dp0[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = d_res_d_pose_inverse_comp_use(0);
          dp1[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = d_res_d_pose_inverse_comp_use(1);
          dp2[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = d_res_d_pose_inverse_comp_use(2);
          dp3[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = d_res_d_pose_inverse_comp_use(3);
          dp4[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = d_res_d_pose_inverse_comp_use(4);
          dp5[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = d_res_d_pose_inverse_comp_use(5);
#endif
          // TODO* 残差对光度参数求导, 2
          dp6[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = -hw * r2new_aff[0] * rlR;  //! exp(aj-ai)*I(pi)
          dp7[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = -hw * 1;                   //! 对 b 导
                                                                         // TODO* 残差对 i(旧状态) 逆深度求导, 1
#ifndef USE_INVERSE_COMPOSITIONAL
#ifndef USE_ZNCC
          dd[idx + MAX_RES_PER_POINT_SEED * target_cid_use] =
              dxInterp * dxdd + dyInterp * dydd;  //! dxfx * 1/Pz * (tx - u*tz) +　dyfy *
                                                  //! 1/Pz * (tx - u*tz)
                                                  // printf("i: %d, host_cid: %d, target_cid: %d, idx: %d,
                                                  // residual: %f, J_idp: [%f %f %f], JtJ: %f, hw: %f,
                                                  // val[9]_accum: %f\n", i, host_cid, target_cid, idx,
                                                  // residual, dd[idx + MAX_RES_PER_POINT_SEED *
                                                  // target_cid_use], (float)(hw * Vec2f(hitColor[1],
                                                  // hitColor[2]).transpose() *  d_uv_d_pt3d * trans)[0],
                                                  // d_res_d_idp_fwd_jac, d_res_d_idp_fwd_jac *
                                                  // d_res_d_idp_fwd_jac, hw, JbBuffer_new[i + h[0] * w[0]
                                                  // * host_cid][9]);
#else
          dd[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = d_res_d_idp_fwd_jac;
#endif
#else
          dd[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = d_res_d_idp_inverse_comp;
#endif
          // TODO* 残差 res, 1
          r[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = hw * residual;  //! 残差 res
...
#ifndef USE_INVERSE_COMPOSITIONAL
#ifndef USE_ZNCC
          float maxstep = 1.0f / Vec2f(dxdd * fxl_target, dydd * fyl_target).norm();  //? 为什么这么设置
#else
          float maxstep = 1.0f / d_uv_d_d_fwd_jac.norm();
#endif
#else
          ...
          dp7[idx + MAX_RES_PER_POINT_SEED * target_cid_use] = 0;  //-hw * 1; //! 对 b 导
#endif
          JbBuffer_new[i + h[0] * w[0] * host_cid][0] +=
              dp0[idx + MAX_RES_PER_POINT_SEED * target_cid_use] * dd[idx + MAX_RES_PER_POINT_SEED * target_cid_use];
          JbBuffer_new[i + h[0] * w[0] * host_cid][1] +=
              dp1[idx + MAX_RES_PER_POINT_SEED * target_cid_use] * dd[idx + MAX_RES_PER_POINT_SEED * target_cid_use];
          JbBuffer_new[i + h[0] * w[0] * host_cid][2] +=
              dp2[idx + MAX_RES_PER_POINT_SEED * target_cid_use] * dd[idx + MAX_RES_PER_POINT_SEED * target_cid_use];
          JbBuffer_new[i + h[0] * w[0] * host_cid][3] +=
              dp3[idx + MAX_RES_PER_POINT_SEED * target_cid_use] * dd[idx + MAX_RES_PER_POINT_SEED * target_cid_use];
          JbBuffer_new[i + h[0] * w[0] * host_cid][4] +=
              dp4[idx + MAX_RES_PER_POINT_SEED * target_cid_use] * dd[idx + MAX_RES_PER_POINT_SEED * target_cid_use];
          JbBuffer_new[i + h[0] * w[0] * host_cid][5] +=
              dp5[idx + MAX_RES_PER_POINT_SEED * target_cid_use] * dd[idx + MAX_RES_PER_POINT_SEED * target_cid_use];
          JbBuffer_new[i + h[0] * w[0] * host_cid][6] +=
              dp6[idx + MAX_RES_PER_POINT_SEED * target_cid_use] * dd[idx + MAX_RES_PER_POINT_SEED * target_cid_use];
          JbBuffer_new[i + h[0] * w[0] * host_cid][7] +=
              dp7[idx + MAX_RES_PER_POINT_SEED * target_cid_use] * dd[idx + MAX_RES_PER_POINT_SEED * target_cid_use];
          JbBuffer_new[i + h[0] * w[0] * host_cid][8] +=
              r[idx + MAX_RES_PER_POINT_SEED * target_cid_use] * dd[idx + MAX_RES_PER_POINT_SEED * target_cid_use];
          // TODO 10 = 6 + 2 + 1 + 1 = pose + affine + idepth + res
          // TODO H22 hessian约等于JTJ，当变量为1维时，hessian = J^2
          JbBuffer_new[i + h[0] * w[0] * host_cid][9] +=
              dd[idx + MAX_RES_PER_POINT_SEED * target_cid_use] *
              dd[idx + MAX_RES_PER_POINT_SEED * target_cid_use];  /// 1/(1+sum(dd*dd))=inverse depth
          /// hessian entry, while now is just
          /// sum(dd*dd), H_{\beta \beta}

          assert(index_to_count.at(idx + MAX_RES_PER_POINT_SEED * target_cid) == cnt);
          if (false && (lvl == 2 && point->host_cid == 1 && i == 178 ||
                        JbBuffer_new[i + h[0] * w[0] * host_cid].cwiseAbs().minCoeff() <
                            1e-8 /*&& JbBuffer_new[i + h[0] * w[0] * host_cid].cwiseAbs().minCoeff() > 0*/)) {
            //              std::cout << "refToNew:\n" << refToNew.matrix3x4()
            //              << std::endl;
            std::cout << "hitColor: " << hitColor.transpose() << ", hostColor: " << rlR
            ...
            printf(
                "idx: %d, residual: %f, i: %d, lvl: %d, cid: %d, uv: [%f "
                "%f], iR: %f, isG0od: %d, isGoodNew: %d, idepth: %f, "
                "isdepthNew: %f\n",
                idx, residual, i, lvl, point->host_cid, point->u, point->v, point->isGood, point->isGood_new,
          ...
          for (int i = 0; i + 3 < patternNumSeed /** kCameraNumUsed*/;
               i += 4)  // this for loop has 2 steps each step step 4 stride.
            // (align with SSE)
            acc9.updateSSE(_mm_load_ps(((float*)(&dp0)) + i),  // _mm_load_ps load 4 float values
                                                               // from pointer address at a time
                           _mm_load_ps(((float*)(&dp1)) + i), _mm_load_ps(((float*)(&dp2)) + i),
                           _mm_load_ps(((float*)(&dp3)) + i), _mm_load_ps(((float*)(&dp4)) + i),
                           _mm_load_ps(((float*)(&dp5)) + i), _mm_load_ps(((float*)(&dp6)) + i),
          ...
          // H = Jt * J,  b = -Jt * b
          // 老老实实对H和b进行累加
          for (int i = (((patternNumSeed /* * kCameraNumUsed*/) >> 2) << 2); i < patternNumSeed /* * kCameraNumUsed*/;
               i++) {
      ...
      float target_grad_mean = target_grad_sum / static_cast<float>(cnt /*/ MAX_RES_PER_POINT_SEED*/);
      ...
      if (ratio > ratio_thr1 && calc_stats_only) {
        if (false) {
          ...
          std::cout << "a_target_sigma_temp: " << a_target_sigma_temp.transpose() << std::endl;
        ...
        for (int cam = 0; cam < kCameraNumUsed; ++cam) {
          if (a_err_mean(cam) < -10) {
            ...
            if (false) {
              std::cout << "11 a_err_mean: " << a_err_mean.transpose() << std::endl;
              std::cout << "11 a_zncc_mean: " << a_zncc_mean.transpose() << std::endl;
      ...
      float energy_each_cam = energy / static_cast<float>(cnt / MAX_RES_PER_POINT_SEED);
      if (cnt > 0) {
        if (false) {
...
#endif
      if (cnt == 0 || (false && ratio > ratio_thr1 && cnt > 0 && target_grad_mean < 3.f) ||
          ratio > ratio_thr2 /*iter >= 35*/ && ((cnt > 0 &&
#if 0
                energy_each_cam >
                    1.5 * point->median_energy /* point->min_energy,
                                                  point->energy[0]*/
#else
                                                 ((1.5 * zncc_avg < point->median_energy) || (point->max_zncc < 0.2))
#endif
                                                 &&point->energy[0] > 0) ||
                                                (/* !isGood || energy / static_cast<float>(cnt /
                                                 ...
                                                 ||
                                                 /*is_bad_res_count > threshold*/ good_cam_num == 0) /*|| cnt == 0*/
        ...
        if (bad_pid_count < 5 && cnt > 0) {
          printf(
              "bbbbb, cnt: %d, energy: %f, good_cam_num: %d, host_cid: %d, "
              "bad_pid_count: %d, target_grad_mean: %f, target_sigma_temp: "
              "%f, ratio: %f, err_avg: %f, zncc_avg: %f, iter: "
              "%d\nenergy_each_cam: %f, %f * energyTH: %f\n",
              cnt, energy, good_cam_num, host_cid, bad_pid_count, target_grad_mean, target_sigma_temp, ratio, err_avg,
              zncc_avg, iter, energy_each_cam, outlierTH_ratio, outlierTH_ratio * point->energyTH);
        } else {
          if (false) {
            printf(
                "ccccc, cnt: %d, bad_pid_count: %d, [lvl_h lvl_t]: [%d %d], "
                "iter: %d\n",
        ...
        if (bad_pid_count < 5 && cnt > 0) {
          ...
          std::cout << "i: " << i << ", host_cid: " << point->host_cid
                    << ", uv: " << Vec2f(point->u, point->v).transpose() << ", v_energy: " << energy_vec.transpose()
                    << std::endl;
          std::cout << "a_target_sigma_temp: " << a_target_sigma_temp.transpose() << std::endl;
          std::cout << "a_err_mean: " << a_err_mean.transpose() << std::endl;
          std::cout << "a_err_max: " << a_err_max.transpose() << std::endl;
          ...
          if (point->energy[0] > 0 || true) {
            printf(
                "cur err: %f, last err: %f, min_err: %f, med_err: %f, "
                "zncc_cur: %f\nrate: %f, rate_min: %f, rate_med: %f, "
                "rate_zncc_med: %f, max_zncc: %f\n\n\n",
                energy_each_cam, point->energy[0], point->min_energy, point->median_energy, zncc_avg,
                energy_each_cam / point->energy[0], energy_each_cam / point->min_energy,
                energy_each_cam / point->median_energy, point->median_energy / zncc_avg, point->max_zncc);
          }
        }
      ...
      if (cnt > 0) {
        E.updateSingle(energy / static_cast<float>(cnt / MAX_RES_PER_POINT_SEED));
        point->isGood_new = true;
        point->energy_new[0] = energy / static_cast<float>(cnt / MAX_RES_PER_POINT_SEED);
    ...
    printf(
        "######## [bad / all] npts: [%d %d] , iter: %d, [h_lvl t_lvl]: [%d "
        "%d] ###########\n",
      ...
      for (int i = 0; i < npts; i++) {
        ...
        if (!point->isGood_new)  // 点不好用之前的
        {
          ...
          if (kCameraNumUsed == 1) {
            ...
            point->energy_new[1] =
                (point->idepth_new - point->iR_triangle) * (point->idepth_new - point->iR_triangle);  //? 什么原理?
          }
#else
          point->energy_new[1] = 0;
#endif
  ...
  if (kCameraNumUsed == 1 /*|| true*/) {
    alphaEnergy =
        alphaW * (EAlpha.A + refToNew_.translation().squaredNorm() * (point_count));  // 平移越大, 越容易初始化成功?
  } else {
  ...
  for (int host_cid = 0; host_cid < kCameraNumUsed; ++host_cid) {
    for (int target_cid = 0; target_cid < 1 /*kCameraNumUsed*/; ++target_cid) {
      ...
      for (int i = 0; i < npts; i++) {
        ...
        point->lastHessian_new = JbBuffer_new[i + h[0] * w[0] * host_cid][9] / static_cast<float>(point->valid_cid_num);
...
#if !defined(USE_MULTI_CAM) || defined(ALWAYS_USE_IDP_PRIOR)
        if (kCameraNumUsed == 1) {
          JbBuffer_new[i + h[0] * w[0] * host_cid][8] += alphaOpt * (point->idepth_new - 1);
        } else {
          JbBuffer_new[i + h[0] * w[0] * host_cid][8] +=
              alphaOpt * (point->idepth_new -
                          /*point->iR */ point->iR_triangle);  // TODO Jt * w * res = 1 * w
                                                               // * res
        }
#endif
#if !defined(USE_MULTI_CAM) || defined(ALWAYS_USE_IDP_PRIOR)
        JbBuffer_new[i + h[0] * w[0] * host_cid][9] += alphaOpt;  // TODO Jt * w * J = 1 * w *1
        ...
        if (alphaOpt == 0 || kCameraNumUsed > 1) {
          if (kCameraNumUsed == 1 || true) {
            JbBuffer_new[i + h[0] * w[0] * host_cid][8] += couplingWeight * (point->idepth_new - point->iR);
          } else {
            JbBuffer_new[i + h[0] * w[0] * host_cid][8] += couplingWeight * 0;
          }
          JbBuffer_new[i + h[0] * w[0] * host_cid][9] += couplingWeight;
        ...
        /// H^{-1}_{\beta \beta}
        if (kCameraNumUsed == 1) {
          JbBuffer_new[i + h[0] * w[0] * host_cid][9] = 1 / (1 + JbBuffer_new[i + h[0] * w[0] * host_cid][9]);
        } else {
          ...
          JbBuffer_new[i + h[0] * w[0] * host_cid][9] = 1.f / (
        ...
        // JbBuffer_new[i + h[0] * w[0] * host_cid].transpose() << std::endl;
        acc9SC.updateSingleWeighted(
            (float)JbBuffer_new[i + h[0] * w[0] * host_cid][0], (float)JbBuffer_new[i + h[0] * w[0] * host_cid][1],
            (float)JbBuffer_new[i + h[0] * w[0] * host_cid][2], (float)JbBuffer_new[i + h[0] * w[0] * host_cid][3],
            (float)JbBuffer_new[i + h[0] * w[0] * host_cid][4], (float)JbBuffer_new[i + h[0] * w[0] * host_cid][5],
            (float)JbBuffer_new[i + h[0] * w[0] * host_cid][6], (float)JbBuffer_new[i + h[0] * w[0] * host_cid][7],
            (float)JbBuffer_new[i + h[0] * w[0] * host_cid][8], (float)JbBuffer_new[i + h[0] * w[0] * host_cid][9]);
      }
    }
  }
  acc9SC.finish();
    ...
    for (int i = 0; i < npts; i++) {
      ...
      for (int j = 0; j < 10; j++) {
        if (point->neighbours[j] == -1) {
          printf(
              "11 npts is less than 10, check detecion, traaceOn or depth "
              "filter_DSM\n");
          std::exit(1);
          continue;
        }
...
void CoarseInitializer::propagateUp(int srcLvl) {
  ...
  for (int cid = 0; cid < kCameraNumUsed; ++cid) {
    ...
    Pnt* ptst = points[srcLvl + 1] + level_cid_to_npts_success_offset[srcLvl + 1][cid];
...
void CoarseInitializer::propagateDown(int srcLvl) {
  ...
  for (int cid = 0; cid < kCameraNumUsed; ++cid) {
    ...
    /// source
    Pnt* ptss = points[srcLvl] + level_cid_to_npts_success_offset[srcLvl][cid];  // 当前层+1, 上一层的点集
    /// target
    Pnt* ptst = points[srcLvl - 1] + level_cid_to_npts_success_offset[srcLvl - 1][cid];  // 当前层点集
...
void CoarseInitializer::setFirst(CalibHessian* HCalib, FrameHessian* newFrameHessian) {
  ...
  for (int cid = 0; cid < kCameraNumUsed; ++cid) {
    for (int lvl = 0; lvl < pyrLevelsUsed; lvl++) {  //[ ***step 2*** ] 针对不同层数选择大梯度像素,
                                                     //第0层比较复杂1d,
      ...
      if (lvl == 0) {  // 第0层提取特征像素
        ...
        level_cid_to_npts[lvl][cid] =
            sel.makeMaps(firstFrame, statusMap + w[0] * h[0] * cid, densities[lvl] * w[0] * h[0], 1, false, 2, cid);
        ...
        //          for (int cid = 0; cid < kCameraNumUsed; ++cid) {
        level_cid_to_npts[lvl][cid] =
            makePixelStatus(firstFrame->dIp[lvl] + w[lvl] * h[lvl] * cid, statusMapB + w[0] * h[0] * cid, w[lvl],
                            h[lvl], densities[lvl] * w[0] * h[0], cid);
        npts += level_cid_to_npts[lvl][cid];
        //          }
      }
      // printf("lvl: %d, npts: %d\n", lvl, npts);
      ...
      for (int y = patternPaddingSeed + 1; y < hl - patternPaddingSeed - 2; y++) {
        for (int x = patternPaddingSeed + 1; x < wl - patternPaddingSeed - 2; x++) {
          // if(x==2) printf("y=%d!\n",y);
          // 如果是被选中的像素
          if ((lvl != 0 && statusMapB[x + y * wl + w[0] * h[0] * cid]) ||
            ...
            float my_type = (lvl != 0) ? 1 : statusMap[x + y * wl + w[0] * h[0] * cid];
            ImmaturePoint* pt = new ImmaturePoint(x, y, firstFrame, my_type, HCalib, cid, lvl);
            bool found = false;
            Vec2i* edge_pixel_start = firstFrame->edge_pixels[lvl] + wl * hl * cid;
            ...
            pl[nl].my_type = (lvl != 0) ? 1 : statusMap[x + y * wl + w[0] * h[0] * cid];
            pl[nl].host_cid = cid;
            Eigen::Vector3f* cpt = firstFrame->dIp[lvl] + x + y * w[lvl] + wl * hl * cid;  // 该像素梯度
            float sumGrad2 = 0;
            // 计算pattern内像素梯度和
            for (int idx = 0; idx < patternNumSeed; idx++) {
            ...
            //? 这个阈值怎么确定的...
            pl[nl].energyTH = patternNumSeed *
                              /*kCameraNumUsed * */ setting_outlierTH_init * setting_outlierTH_init;
      ...
      printf(
          "lvl: %d, cid: %d, valid seeds: %d, pt_num_not_found_in_edge: %d, "
          "edge_num: %d\n",
          lvl, cid, nl, pt_num_not_found_in_edge, firstFrame->edge_pixel_num[lvl][cid]);
      ...
      level_cid_to_npts_success_offset[lvl][cid] = level_cid_to_npts_offset[lvl][cid];
      //      }
    }
  }
  delete[] statusMap;
...
double CoarseInitializer::MultiViewTriangulation(const double& focal, const std::vector<Mat4>& poses,
                                                 const std::vector<Vec3>& points,
                                                 std::vector<std::pair<double, int>>& err_vec, VecX& errs,
                                                 Vec3& point_3d) {
...
void CoarseInitializer::convert_to_ImageData(cv::Mat& data, ImageDataAM& image_data, uint8_t camera_id) {
...
void CoarseInitializer::setFirstStereo(CalibHessian* HCalib, FrameHessian* newFrameHessian) {
  ...
  float* statusMap = new float[w[0] * h[0] * kCameraNumUsed];
  bool* statusMapB = new bool[w[0] * h[0] * kCameraNumUsed];
  Vec2f aff = AffLight::fromToVecExposure(firstFrame->ab_exposure, firstFrame->ab_exposure, firstFrame->aff_g2l(),
                                          firstFrame->aff_g2l())
                  .cast<float>();
  float densities[] = {0.03, 0.05, 0.15, 0.5, 1};  // 不同层取得点密度
  Vec3 point_3d_wcs;
  Vec3 point_3d_ccs;
  std::vector<std::pair<double, int>> err_vec = {std::pair<double, int>(0, 1), std::pair<double, int>(0, 2),
    ...
    for (int lvl = 0; lvl < pyrLevelsUsed; lvl++) {  //[ ***step 2*** ] 针对不同层数选择大梯度像素,
      ...
      for (int cam = 0; cam < kCameraNumUsed; ++cam) {
        ...
        for (size_t i = 0; i < kCameraNumUsed; ++i) {
          p_depth_filter_DSM_->px_err_angle_vec_[i] = std::atan(p_depth_filter_DSM_->px_noise_ / fx[lvl]);
        }

        assert(estimator_config_.search_level == 0);
        number_t search_level_focal_length = fx[lvl] * std::pow(2.0f, -estimator_config_.search_level);
        ...
        cid_to_img[cam]->DangerouslyCopyFrom(cid_to_image_data[cam].width, cid_to_image_data[cam].height,
      ...
      for (int y = patternPaddingSeed + 1; y < hl - patternPaddingSeed - 2; y++) {
        for (int x = patternPaddingSeed + 1; x < wl - patternPaddingSeed - 2; x++) {
          ...
          if ((lvl != 0 && statusMapB[x + y * wl + w[0] * h[0] * cid]) ||
            ...
            std::vector<Mat4> Tcws = {firstFrame->p_multi_camera->cid_to_T01[cid].inverse()};
            std::vector<Vec3> bearings = {(Ki[lvl] * Vec3(x, y, 1.0)).normalized()};
            if (kCameraNumUsed > 1) {
#if 0
...
#else
              Vec2i px = Vec2i(x, y);
              point.n = (firstFrame->p_multi_camera->level_cid_to_Kinv_temp.at(lvl).at(cid) *
                         Vec3(static_cast<number_t>(x), static_cast<number_t>(y), 1.))
                            .normalized();
              bool is_success = point.pyramid_patch.SetFromImg(cid_to_img[cid], px.cast<number_t>(), cid, is_corner,
              ...
              if (is_success) {
                ...
                MultiCameraEpipolarSearch::State state =
                    p_depth_filter_DSM_->p_multi_cam_epipolar_search_->FindEpipolarMatch(
                        point, cid, cid_to_img, 1, seed.rho, seed.sigma2, 1, cid_to_output, res_idp, -1, true, lvl);
...
#ifdef SHOW_DETECTION_RES
            img_host[lvl]->setPixel9(x + 0.5, y + 0.5, makeRainbow3B(pl[nl].idepth), cid);
#endif
  ...
  for (int lvl = 0; lvl < pyrLevelsUsed; ++lvl) {
    ...
    for (int cid = 0; cid < kCameraNumUsed; ++cid) {
      show_mat_vec[cid] = cv::Mat(img_host[lvl]->h, img_host[lvl]->w, CV_8UC3,
      ...
      if (lvl == pyrLevelsUsed - 1 && !pts[i].isGood) {
        ...
        for (int n = 0; n < 10; n++) {
          if (pts[i].neighbours[n] == -1) {
            printf(
                "22 npts is less than 10, check detecion, traaceOn or depth "
                "filter_DSM\n");
            std::exit(2);
            continue;
          }
...
void CoarseInitializer::doStep(int lvl, float lambda, Vec8f inc) {
  ...
  for (int target_cid = 0; target_cid < kCameraNumUsed; ++target_cid) {
    ...
    for (int i = 0; i < npts; i++) {
      ...
      float step = -b * JbBuffer[i + w[0] * h[0] * pts[i].host_cid /* * kCameraNumUsed*/][9] / (1 + lambda);
      // TODO
      ...
      if (!std::isfinite(newIdepth)) {
        printf(
            "newIdepth: %f, oldIdepth: %f,step: %f, i: %d, lvl: %d, cid: "
            "%d, uv: [%f %f]\n",
            newIdepth, pts[i].idepth, step, i, lvl, pts[i].host_cid, pts[i].u, pts[i].v);
        //                  std::cout <<
        ...
        std::cout << "JbBuffer[i + w[0] * h[0] * pts[i].host_cid]: "
                  << JbBuffer[i + w[0] * h[0] * pts[i].host_cid].transpose() << std::endl;
        std::cout << "JbBuffer_new[i + w[0] * h[0] * pts[i].host_cid]: "
                  << JbBuffer_new[i + w[0] * h[0] * pts[i].host_cid].transpose() << std::endl;
        std::exit(1);
      }
...
#else
        pts[i].idepth = pts[i].idepth_new = pts[i].idepth_new_triangle = pts[i].iR_triangle;
#endif
        continue;
      }
...
void CoarseInitializer::makeNN() {
  ...
  typedef nanoflann::KDTreeSingleIndexAdaptor<nanoflann::L2_Simple_Adaptor<float, FLANNPointcloud>, FLANNPointcloud, 2>
      KDTree;
#ifdef SHOW_NN
  ...
  for (int cid = 0; cid < kCameraNumUsed; ++cid) {
    ...
    for (int i = 0; i < pyrLevelsUsed; i++) {
      pcs[i] = FLANNPointcloud(level_cid_to_numPoints[i][cid],
    ...
    for (int lvl = 0; lvl < pyrLevelsUsed; lvl++) {
      ...
      Vec3f* colorSmall = firstFrame->dIp[lvl_small] + wG[lvl_small] * hG[lvl_small] * cid;
      ...
      for (int i = 0; i < npts_big; i++) {
        img_big->setPixel1(pts_big[i].u + 0.5, pts_big[i].v + 0.5, makeRainbow3B(0.1), cid);
      }
      Pnt* pts_small = points[lvl_small] + level_cid_to_npts_success_offset[lvl_small][cid];
      int npts_small = level_cid_to_numPoints[lvl_small][cid];
      for (int i = 0; i < npts_small; i++) {
        img_small->setPixel1(pts_small[i].u + 0.5, pts_small[i].v + 0.5, makeRainbow3B(0.1), cid);
      }
#endif
      Pnt* pts = points[lvl] + level_cid_to_npts_success_offset[lvl][cid];
      int npts = level_cid_to_numPoints[lvl][cid];
      ...
      for (int i = 0; i < npts; i++) {
#ifdef SHOW_NN
        img_big->setPixel4(pts[i].u + 0.5, pts[i].v + 0.5, makeRainbow3B(1), cid);
#endif
        ...
        if (lvl < pyrLevelsUsed - 1) {
          ...
          // 1][cid]);
          assert(ret_index[0] >= 0 && ret_index[0] < level_cid_to_numPoints[lvl + 1][cid]);
          ...
          for (int id = 0; id < npts_small; ++id) {
            float diff = (pt - Vec2f(pts_small[id].u, pts_small[id].v)).squaredNorm();
            diff_vec_small(id, 0) = diff;
            v_diff_small.emplace_back(diff);
          }
          std::sort(v_diff_small.begin(), v_diff_small.end(), [](const float& p1, const float& p2) { return p1 < p2; });
          int nearest_id = 0;
          printf("--------------------------------------------------------\n");
          for (int id = 0; id < npts_small; ++id) {
            float diff = (pt - Vec2f(pts_small[id].u, pts_small[id].v)).squaredNorm();
            if (std::abs(diff - diff_vec_small.minCoeff()) < 0.00001) {
              printf("nearest_id: %d, dist: %f, uv: [%f %f]\n", id, diff, pts_small[id].u, pts_small[id].v);
            }
          }
          for (int index = 0; index < nn; ++index) {
            float diff = (pt - Vec2f(pts_small[ret_index[index]].u, pts_small[ret_index[index]].v)).squaredNorm();
            if (index == 0) {
              assert(std::abs(diff - ret_dist[index]) < 0.0001);
              assert(std::abs(v_diff_small[index] - ret_dist[index]) < 0.0001);
            }
            printf(
                "small level neighbour, index: %d, diff: %f, sorted: %f, "
                "dist: %f\n",
                ret_index[index], diff, v_diff_small[index], ret_dist[index]);
          }
          MatXXf diff_vec_big;
          ...
          for (int id = 0; id < npts_big; ++id) {
            float diff = (pt_big - Vec2f(pts_big[id].u, pts_big[id].v)).squaredNorm();
            diff_vec_big(id, 0) = diff;
            v_diff_big.emplace_back(diff);
          }
          std::sort(v_diff_big.begin(), v_diff_big.end(), [](const float& p1, const float& p2) { return p1 < p2; });
          for (int id = 0; id < npts_big; ++id) {
            float diff = (pt_big - Vec2f(pts_big[id].u, pts_big[id].v)).squaredNorm();
            if (std::abs(diff - diff_vec_big.minCoeff()) < 0.00001) {
              printf("big level, nearest_id: %d, dist: %f, uv: [%f %f]\n", id, diff, pts_big[id].u, pts_big[id].v);
            }
          }
          for (int index = 0; index < nn; ++index) {
            float diff =
                (pt_big - Vec2f(pts_big[ret_index_big[index]].u, pts_big[ret_index_big[index]].v)).squaredNorm();
            assert(std::abs(diff - ret_dist_big[index]) < 0.0001);
            assert(std::abs(v_diff_big[index] - ret_dist_big[index]) < 0.0001);
            printf(
                "big level neighbour, index: %d, diff: %f, sorted: %f, "
                "dist: %f\n",
                ret_index_big[index], diff, v_diff_big[index], ret_dist_big[index]);
          }
