...
class BAExtension {
 public:
  ...
  virtual void updateBAOrdering(std::vector<dso::EFFrame*>& frames, gtsam::Ordering* ordering, KeyDimMap& baDimMap) = 0;
  ...
  virtual bool postSolve(gtsam::Values::shared_ptr values, gtsam::Values::shared_ptr newValues,
                         const gtsam::Vector& inc, const gtsam::Ordering& ordering, const KeyDimMap& baDimMap) = 0;
...
class BAGraphs {
 public:
  ...
  // values.
  virtual std::pair<gtsam::Matrix, gtsam::Vector> getHAndB(const gtsam::Values& values, const gtsam::Ordering& ordering,
  ...
  virtual void marginalizeFrame(const gtsam::FastVector<gtsam::Key>& keysToMarginalize,
...
class BAGTSAMIntegration {
 public:
  ...
  // weight part.
  using DynamicWeightCallback =
      std::function<double(double lastDSOEnergy, double lastRMSE, bool coarseTrackingWasGood)>;
 ...
 private:
  ...
  gtsam::LinearContainerFactor::shared_ptr convertedDSOHAndBToFactor(const gtsam::Matrix& H, const gtsam::Vector& b,
