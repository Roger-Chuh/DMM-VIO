...
public:
   ...
   * alpha;
   * */
  explicit UCMRTPCamera(CamId camera_id, int width, int height, bool use_exp)
  ...
  // s3, s4
  UCMRTPCamera(CamId camera_id, int width, int height,
...
public:
  void Distortion(const number_t &p_x, const number_t &p_y, number_t &d_u,
                  number_t &d_v, Mat2 *p_d_uv_d_xy = nullptr,
                  Eigen::Matrix<number_t, 2, Eigen::Dynamic> *d_uv_d_params =
                      nullptr) const;

  void UnDistortion(const number_t &d_u, const number_t &d_v, number_t &p_x,
                    number_t &p_y, Mat2 *d_xy_d_uv = nullptr,
                    Eigen::Matrix<number_t, 2, Eigen::Dynamic> *d_xy_d_params =
                        nullptr) const;
};
} // namespace dso