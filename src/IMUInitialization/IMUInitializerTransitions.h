...
public:
  ...
  // nullptr the caller state is responsible for creating a proper new state
...
// (The legacy options were removed but are still in the enum so that the
// indices are the same as earlier version).
...
enum class InitTransitionMode {
  LEGACY_UNUSED,
  COMBINED_MODEL,
  COMBINED_REPLACEMENT_MODEL,
  LEGACY_UNUSED2,
  COMBINED_NO_INITIAL_REPLACEMENT,
