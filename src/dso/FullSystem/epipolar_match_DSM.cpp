...
EpipolarMatchDSM::EpipolarMatchDSM(MultiCamera *p_level_to_multi_camera,
...
bool EpipolarMatchDSM::InFrame(const Vec2 &uv,
                               const std::shared_ptr<AlgsImage> &p_img,
                               int border) {
  if (uv[0] >= border && uv[0] < p_img->width - border && uv[1] >= border &&
      uv[1] < p_img->height - border) {
    return true;
  }
  return false;
}

EpipolarMatchDSM::TriResult
EpipolarMatchDSM::TriangulateWithCheckTheta(number_t &idp, const Mat4 &T01,
...
std::vector<size_t> EpipolarMatchDSM::FindLocalMaxima(
    const std::vector<SearchResult> &search_result_vec) {
