...
using symbol_shorthand::P, symbol_shorthand::S, symbol_shorthand::V,
    symbol_shorthand::B, symbol_shorthand::A;

PoseGraphBundleAdjustment::PoseGraphBundleAdjustment(
    DelayedMarginalizationGraphs *delayedMarginalizationPassed,
  ...
  inputDelayedGraph = delayedMarginalization->addDelayedGraph(
      settings.delay,
...
void PoseGraphBundleAdjustment::prepareOptimization() {
  ...
  disconnectedGraph = delayedMarginalization->addDisconnectedGraph(
      delayedGraph->getMaxGroupInGraph());
}

gtsam::Values dmvio::PoseGraphBundleAdjustment::optimize(
    gtsam::NonlinearFactor::shared_ptr activeDSOFactor,
    const gtsam::Values &baValues, const gtsam::Values &imuInitValues,
    bool noOptimization) {
  ...
  if (!imuInitValues.empty()) {
    ...
    std::cout << "Updating transform with index "
              << transformDSOToIMU->getSymbolInd()
              << " to val: " << transformDSOToIMU->getScale() << std::endl;
  }
  ...
  gtsam::Values values =
      buildGraph(*graph, baValues, inputValues, imuInitValues, noOptimization);
...
gtsam::Values dmvio::PoseGraphBundleAdjustment::extendGraph(
    gtsam::NonlinearFactor::shared_ptr activeDSOFactor,
  ...
  // Note: We need to add the cachedData before calling
  // insertIMUFactorsAndValues, because it accesses prevKFIds, which is updated
  // here.
  ...
  addedKeys.erase(std::unique(addedKeys.begin(), addedKeys.end()),
                  addedKeys.end());
  ...
  // We don't do negative energy compensation here again, although maybe it
  // might be useful.
  ...
  // We want to use disconnectedGraph->delayedCurrValues, and eraseAndInsert all
  // baValues.
  numIMUFactors =
      insertIMUFactorsAndValues(graph, cachedData, imuInputValues, values,
                                minConnectedPoseInd, imuBias, velocity);
...
gtsam::Values dmvio::PoseGraphBundleAdjustment::buildGraph(
    gtsam::NonlinearFactorGraph &graph, const gtsam::Values &poseInputValues,
  ...
  long long minConnectedPoseInd = insertValuesAndGetMinConnectedPoseInd(
      poseInputValues, poseInputValues2, values, keyVector);

  allPosesUsed = minConnectedPoseInd == 0;

  // First remove all preintegrated measurements before minConnectedPoseInd
  while (preintegratedForKF.size() > 0 &&
         preintegratedForKF.front().second <= minConnectedPoseInd) {
    preintegratedForKF.pop_front();
  }
  minConnectedPoseInd = std::max((int)minConnectedPoseInd,
                                 prevKFIds[preintegratedForKF[0].second]);
  assert(prevKFIds[preintegratedForKF[0].second] == minConnectedPoseInd);

  firstId = minConnectedPoseInd;

  if (!noOptimization) {
    // Workaround: Unfortunately the DSO marginalization factors can get a
    // (large) negative energy which is problematic for the GTSAM optimizer. So
    // we do one GN-iteration, and manually add this energy to the graph.
  ...
  // Insert values for IMU data: If initial velocity or bias does not exist for
  // a KF we use the values of the previous KF.
  numIMUFactors =
      insertIMUFactorsAndValues(graph, preintegratedForKF, imuInputValues,
                                values, minConnectedPoseInd, imuBias, velocity);

  latestInd = preintegratedForKF.back().second;

  std::cout << "PGBA: Built graph with " << numIMUFactors << " IMU factors "
            << "and " << numOptimizedPoses
            << " "
               "poses."
            << std::endl;
  ...
  auto factors = getPriorsAndAddValuesForTransform(
      *transformDSOToIMU, settings.transformPriors, values);
...
int dmvio::PoseGraphBundleAdjustment::insertIMUFactorsAndValues(
    NonlinearFactorGraph &graph,
    const std::deque<std::pair<gtsam::PreintegratedImuMeasurements, int>>
        &preintegratedMeasurements,
    const Values &imuInputValues, Values &values,
    long long int minConnectedPoseInd, imuBias::ConstantBias &imuBias,
    Vector3 &velocity) {
  int numFactors = 0;
  for (auto &&pair : preintegratedMeasurements) {
    ...
    NonlinearFactor::shared_ptr imuFactor(new ImuFactor(
        P(prevId), V(prevId), P(id), V(id), prevBiasKey, imuData));
    ...
    auto transformedFactor = boost::make_shared<PoseTransformationFactor>(
        imuFactor, *transformDSOToIMU, settings.conversionType);
...
long long int
dmvio::PoseGraphBundleAdjustment::insertValuesAndGetMinConnectedPoseInd(
    const Values &poseInputValues, const Values &poseInputValues2,
    Values &values, const KeyVector &keyVector) {
  ...
  for (auto &&key : keyVector) {
    ...
    // The reason is that the newest pose is not yet in the values stored inside
    ...
    if (sym.chr() == 'p') {
      ...
      if (lastPoseInd == -1) {
        ...
        bool skipDueToMinIMUFactor =
            removeIMUFactorsUntil >= 0 &&
            minConnectedPoseInd < removeIMUFactorsUntil;
...
gtsam::NonlinearFactor::shared_ptr dmvio::compensateNegativeEnergy(
    NonlinearFactorGraph &graph, const Values &values,
  ...
  if (optimError < 0) {
    ...
    // GTSAM doesn't like factors with just a constant so we add the scale as
    // well.
    Key scaleKey = S(transformForFakeFactor.getSymbolInd());
    JacobianFactor::shared_ptr constantFactor(
        new JacobianFactor(scaleKey, gtsam::Matrix11::Zero(), bConst));
    Values linPoint;
    linPoint.insert(scaleKey, ScaleGTSAM(transformForFakeFactor.getScale()));
    auto lcf =
        boost::make_shared<LinearContainerFactor>(constantFactor, linPoint);
    return lcf;
  }
  return nullptr;
}

void PoseGraphBundleAdjustment::addKeyframe(
    int id, gtsam::PreintegratedImuMeasurements imuMeasurements) {
...
void PoseGraphBundleAdjustment::updateGraphMarginalizationOrder() {
  ...
  std::deque<gtsam::FastVector<gtsam::Key>> &marginalizationOrder =
      this->delayedGraph->marginalizationOrder;
  ...
  for (auto &keyVector : marginalizationOrder) {
    ...
    // Make sure that we don't marginalize symbols optimized by the current
    // transform!
    keyVector.erase(std::remove_if(keyVector.begin(), keyVector.end(),
                                   [&dontMargKeys](const gtsam::Key &key) {
                                     return dontMargKeys.exists(key);
                                   }),
                    keyVector.end());
...
// Prepare for prepareGraphForMainOptimization (usually called in a separate
// thread)
void PoseGraphBundleAdjustment::preparePreparation(
    const gtsam::Values &optimizedValues) {
...
void PoseGraphBundleAdjustment::mainPreparation(
    const gtsam::Values &optimizedValues) {
  ...
  for (auto &&pair : optimizedValues) {
    ...
    if (chr != 'a' &&
        chr != 'p') // We don't overwrite DSO variables, as the evaluation point
                    // values shall be used for them.
    {
...
PoseGraphBundleAdjustment::prepareGraphForMainOptimization(
    const gtsam::Values &optimizedValues) {
  dmvio::TimeMeasurement fullMeas("PrepareGraphForMainOptimization");
  // Remove active DSO factor again!
  removeDSOFactorIfNeeded();

  // These should just be the factors arising from one call to
  // addMarginalizedPointsBA
  ...
  // delayedGraph->delayedCurrValues will be replaced with optimizedValues later
  // on (in mainPreparation). Non-DSO variables in delayedGraph->delayedValues
  // will be replaced with optimizedValues there as well. This means that we
  // need to add the new DSO delayedValues here.
  if (settings.prepareGraphAddDelValues) {
    for (auto &&val : disconnectedGraph->delayedValues) {
      if (!delayedGraph->delayedValues.exists(val.key)) {
        delayedGraph->delayedValues.insert(val.key, val.value);
  ...
  delayedGraph->marginalizationOrder.insert(
      delayedGraph->marginalizationOrder.end(),
      disconnectedGraph->marginalizationOrder.begin(),
      disconnectedGraph->marginalizationOrder.end());
  // This method must be called from within the BA thread, otherwise a mutex
...
int PoseGraphBundleAdjustment::getNumKFs() { return preintegratedForKF.size(); }

gtsam::Key PoseGraphBundleAdjustment::getBiasKey() { return B(latestInd); }
...
int PoseGraphBundleAdjustment::getLatestInd() const { return latestInd; }

std::shared_ptr<DelayedGraph>
PoseGraphBundleAdjustment::getInputDelayedGraph() const {
...
bool PoseGraphBundleAdjustment::isAllPosesUsed() const { return allPosesUsed; }

int PoseGraphBundleAdjustment::getFirstIdWithIMUData() const { return firstId; }
