...
#ifndef log_gamma
#define log_gamma(x)                                                           \
  ((x) > 15.f ? log_gamma_windschitl(x) : log_gamma_lanczos(x))
#endif
...
struct SingleLineInfo {
  /*endPoints, the coordinate origin is the top-left corner of the original
   *image. startPointX = sPointInScaledX * (factor)^scaledCount;	*/
  float startPointX;
  float startPointY;
  float endPointX;
  float endPointY;
  // endPoints, the coordinate origin is the top-left corner of the scaled
  // image.
  float sPointInScaledX;
  float sPointInScaledY;
  float ePointInScaledX;
  float ePointInScaledY;
  // direction of a line, the angle between positive line direction (dark side
  // is in the left) and positive X axis.
  float direction;
...
typedef std::vector<LineSet>
    ScaleLineSet; // each element in ScaleLineSet is a vector of lines which
                  // corresponds the
// same line detected in different scaled images.
...
struct EdgeChains {
  ...
  std::vector<unsigned int>
      sId; // the start index of each edge in the coordinate arrays
  unsigned int numOfEdges; // the number of edges whose length are larger than
                           // minLineLen; numOfEdges < sId.size;
};
struct LineChains {
  ...
  std::vector<unsigned int>
      sId; // the start index of each line in the coordinate arrays
  unsigned int numOfLines = 0; // the number of lines whose length are larger
                               // than minLineLen; numOfLines < sId.size;
};
...
public:
   ...
   *smoothed: In, flag to mark whether the image has already been smoothed by
   *Gaussian filter. return 1: error happen
   */
...
public:
  ...
  // store the line Equation coefficients, vec3=[w1,w2,w3] for line w1*x + w2*y
  // + w3=0;
  ...
  // store the line salience, which is the summation of gradients of pixels on
  // line
...
private:
  void InitEDLine_();
  /*For an input edge chain, find the best fit line, the default chain length is
   *minLineLen_ xCors:  In, pointer to the X coordinates of pixel chain; yCors:
   *In, pointer to the Y coordinates of pixel chain; offsetS:In, start index of
   *this chain in array; lineEquation: Out, [a,b] which are the coefficient of
   *lines y=ax+b(horizontal) or x=ay+b(vertical); return:  line fit error;
   *1:error happens;
   */
  float LeastSquaresLineFit_(unsigned int *xCors, unsigned int *yCors,
                             unsigned int offsetS,
                             std::array<float, 2> &lineEquation);
  /*For an input pixel chain, find the best fit line. Only do the update based
   *on new points. For A*x=v,  Least square estimation of x = Inv(A^T * A) *
   *(A^T * v); If some new observations are added, i.e, [A; A'] * x = [v; v'],
   ...
   *lineEquation: Out, [a,b] which are the coefficient of lines
   *y=ax+b(horizontal) or x=ay+b(vertical); return:  line fit error; 1:error
   *happens;
   */
  float LeastSquaresLineFit_(unsigned int *xCors, unsigned int *yCors,
   ...
   * for a structure to be perceptually meaningful, the expectation of this
   * structure by chance must be very low.
   */
...
private:
   ...
   *Only those pixel whose gradient magnitude are larger than this threshold
   *will be taken as possible edge points. Default value is 36*/
  ...
  /*This type of storage order is because of the order of edge detection
   *process.
   *For each edge, start from one anchor point, first go right, then go left or
   *first go down, then go up*/
  unsigned int *pFirstPartEdgeX_;  // store the X coordinates of the first part
                                   // of the pixels for chains
  unsigned int *pFirstPartEdgeY_;  // store the Y coordinates of the first part
                                   // of the pixels for chains
  unsigned int *pFirstPartEdgeS_;  // store the start index of every edge chain
                                   // in the first part arrays
  unsigned int *pSecondPartEdgeX_; // store the X coordinates of the second part
                                   // of the pixels for chains
  unsigned int *pSecondPartEdgeY_; // store the Y coordinates of the second part
                                   // of the pixels for chains
  unsigned int *pSecondPartEdgeS_; // store the start index of every edge chain
                                   // in the second part arrays
  ...
  static int float_equal(float a, float b) {
    ...
    if (abs_max < 1.175494351e-38F)
      abs_max = 1.175494351e-38F;
    /* equal if relative error <= factor x eps */
    return (abs_diff / abs_max) <=
           (RELATIVE_ERROR_FACTOR * 2.2204460492503131e-016);
  }
  ...
  static float log_gamma_lanczos(float x) {
    static float q[7] = {75122.6331530f, 80916.6278952f, 36308.2951477f,
                         ...
                         2.50662827511f};
    float a = (x + 0.5f) * std::log(x + 5.5f) - (x + 5.5f);
    float b = 0.f;
    int n;
    for (n = 0; n < 7; n++) {
      a -= std::log(x + (float)n);
      b += q[n] * std::pow(x, (float)n);
    }
    return a + std::log(b);
  }
  /** Computes the natural logarithm of the absolute value of
  ...
  static float nfa(int n, int k, float p, float logNT) {
    ...
    log1term = log_gamma((float)n + 1.f) - log_gamma((float)k + 1.f) -
               log_gamma((float)(n - k) + 1.f) + (float)k * log(p) +
               (float)(n - k) * log(1.f - p);
    term = exp(log1term);
    ...
    for (i = k + 1; i <= n; i++) {
      ...
      if (bin_term < 1.f) {
        err = term *
              ((1.f - pow(mult_term, (float)(n - i + 1))) / (1.f - mult_term) -
               1.f);
...
public:
  /* Interface.*/
  int Run(float scaleX, float scaleY, boundingbox_t bbox, image_int8u_p image,
          ScaleLineSet &keyLines);
...
private:
  ...
  void InverseGaussianSamplerLines(pixel_float_t gs_scale,
                                   ScaleLineSet &keyLines);

private:
  /*For each scaled of image, we define an EDLineDetector, because we can get
   *gradient images (dxImg, dyImg, gImg) from the EDLineDetector class without
   *extra computation cost. Another reason is that, if we use a single
   *EDLineDetector to detect lines in different scaled of images, then we need
   *to allocate and release memory for gradient images (dxImg, dyImg, gImg)
   *repeatedly for their varying size*/
  std::vector<EDLineDetector *> edLineVec_;

  // int ksize_; //the size of Gaussian kernel: ksize X ksize, default value
  // is 5.

  unsigned int numOfBand_; // the number of band used to compute line descriptor
  unsigned int widthOfBand_;      // the width of band;
  std::vector<float> gaussCoefL_; // the local gaussian coefficient apply to the
                                  // orthogonal line direction within each band;
  std::vector<float> gaussCoefG_; // the global gaussian coefficient apply to
                                  // each Row within line support region
};

EDLineDetector::EDLineDetector() {
  // set parameters for line segment detection

  gradienThreshold_ = 15; // ***** ORIGINAL WAS 25
...
static void sobel_edge(ORIENT_CODE oriention, image_int8u_p src,
                       image_int16s_p dst) {
...
static void mcv_sobel(ORIENT_CODE oriention, image_int8u_p src,
                      image_int16s_p dst) {
...
static int array_add(image_float_p src1, image_float_p src2,
                     image_float_p dst) {
  int _code = 0;
  int k, nsize;
...
static void array_add(image_int16s_p src1, image_int16s_p src2,
                      image_int16s_p dst) {
...
static void mcv_add(image_int16s_p src1, image_int16s_p src2,
                    image_int16s_p dst) {
...
static void array_threshold(short thresh, image_int16s_p src,
                            image_int16s_p dst) {
  int nsize;
  int k;
  short *psrc = NULL, *pdst = NULL;
...
static void mcv_threshold(short thresh, image_int16s_p src,
                          image_int16s_p dst) {
  array_threshold(thresh, src, dst);
}

static void array_compare_lt(image_int16s_p src1, image_int16s_p src2,
                             image_int8u_p dst) {
...
static void mcv_compare_CMP_LT(image_int16s_p src1, image_int16s_p src2,
                               image_int8u_p dst) {
...
// A*AT
static int array_multiply_transpose_float(image_float_p src,
                                          image_float_p dst) {
  int i, j;
  int m, n, l;
  float _sum;
...
// A*BT
static int array_multiply2_transpose_float(image_float_p src1,
                                           image_float_p src2,
                                           image_float_p dst) {
  int i, j, k;
  float _sum;

  if (src1 == NULL || src2 == NULL || dst == NULL || src1->data == NULL ||
      src2->data == NULL || dst->data == NULL)
    return 1;
  ...
  for (i = 0; i < dst->ysize; i++) {
    for (j = 0; j < dst->xsize; j++) {
      ...
      for (k = 0; k < src1->xsize; k++) {
        _sum +=
            (src1->data[i * src1->xsize + k] * src2->data[j * src2->xsize + k]);
      }
...
static int array_multiply(image_float_p src1, image_float_p src2,
                          image_float_p dst) {
  int i, j, l, u;
  int m, n, k;
...
static void mcv_multiply_float(image_float_p src1, image_float_p src2,
                               image_float_p dst) {
...
static void mcv_multiply2_transpose_float(image_float_p src1,
                                          image_float_p src2,
                                          image_float_p dst) {
  array_multiply2_transpose_float(src1, src2, dst);
}

int EDLineDetector::EdgeDrawing(image_int8u_p image, EdgeChains &edgeChains,
                                bool smoothed) {
  ...
  if ((gImg_ == NULL) || ((gImg_->xsize != (int)imageWidth) ||
                          (gImg_->ysize != (int)imageHeight))) {
    if (pFirstPartEdgeX_ != NULL) {
      delete[] pFirstPartEdgeX_;
      pFirstPartEdgeX_ = NULL;
      delete[] pFirstPartEdgeY_;
  ...
  memset(pAnchorX_, 0,
         edgePixelArraySize * sizeof(unsigned int)); // initialization
  ...
  for (unsigned int h = 1; h < imageHeight - 1; h = h + scanIntervals_) {
    ...
    for (unsigned int w = 1; w < imageWidth - 1; w = w + scanIntervals_) {
      ...
      if (pdirImg[indexInArray] ==
          Horizontal) { // if the direction of pixel is horizontal, then compare
                        // with up and down
        if (pgImg[indexInArray] >=
                pgImg[indexInArray - imageWidth] + anchorThreshold_ &&
            pgImg[indexInArray] >=
                pgImg[indexInArray + imageWidth] +
                    anchorThreshold_) { // (w,h) is accepted as an anchor
  ...
  if (anchorsSize > edgePixelArraySize) {
    printf("anchor size is larger than its maximal size. anchorsSize = %d, "
           "maximal size = %d\n",
  ...
  memset(edgeImage_->data, ZERO,
         edgeImage_->xsize * edgeImage_->ysize * sizeof(unsigned char));
  unsigned char *pEdgeImg = edgeImage_->data;
  memset(pFirstPartEdgeX_, 0,
         edgePixelArraySize * sizeof(unsigned int)); // initialization
  ...
  for (unsigned int i = 0; i < anchorsSize; i++) {
    ...
    if (pdirImg[indexInArray] ==
        Horizontal) { // if the direction of this pixel is horizontal, then go
                      // left and right.
      ...
      while (pgImg[indexInArray] > 0 && !pEdgeImg[indexInArray]) {
        ...
        if (pdirImg[indexInArray] == Horizontal) { // should go left or right
          if (lastDirection == UpDir ||
              lastDirection == DownDir) { // change the pixel direction now
          ...
          lastX = x;
          lastY = y;
          if (lastDirection == RightDir ||
              shouldGoDirection == RightDir) { // go right
            if (x == imageWidth - 1 || y == 0 ||
                y == imageHeight - 1) { // reach the image border
              break;
            }
            // Look at 3 neighbors to the right and pick the one with the max.
            // gradient value
            ...
            lastDirection = RightDir;
          } else if (lastDirection == LeftDir ||
                     shouldGoDirection == LeftDir) { // go left
            if (x == 0 || y == 0 ||
                y == imageHeight - 1) { // reach the image border
              break;
            }
            // Look at 3 neighbors to the left and pick the one with the max.
            // gradient value
            gValue1 = (unsigned char)pgImg[indexInArray - imageWidth - 1];
            gValue2 = (unsigned char)pgImg[indexInArray - 1];
            gValue3 = (unsigned char)pgImg[indexInArray + imageWidth - 1];
          ...
          if (lastDirection == RightDir ||
              lastDirection == LeftDir) { // change the pixel direction now
          ...
          lastX = x;
          lastY = y;
          if (lastDirection == DownDir ||
              shouldGoDirection == DownDir) { // go down
            if (x == 0 || x == imageWidth - 1 ||
                y == imageHeight - 1) { // reach the image border
              break;
            }
            // Look at 3 neighbors to the down and pick the one with the max.
            // gradient value
            gValue1 = (unsigned char)pgImg[indexInArray + imageWidth + 1];
          ...
          } else if (lastDirection == UpDir ||
                     shouldGoDirection == UpDir) { // go up
            if (x == 0 || x == imageWidth - 1 ||
                y == 0) { // reach the image border
              break;
            }
            // Look at 3 neighbors to the up and pick the one with the max.
            // gradient value
            gValue1 = (unsigned char)pgImg[indexInArray - imageWidth + 1];
            gValue2 = (unsigned char)pgImg[indexInArray - imageWidth];
            gValue3 = (unsigned char)pgImg[indexInArray - imageWidth - 1];
      ...
      pEdgeImg[indexInArray] =
          0; // mark the anchor point be a non-edge pixel and
      lastDirection = LeftDir;
  ...
  if (offsetPS > maxNumOfEdge) {
    printf("Edge drawing Error: The total number of edges is larger than "
           "MaxNumOfEdge, "
  ...
  if (offsetPFirst > edgePixelArraySize || offsetPSecond > edgePixelArraySize) {
    printf(
        "Edge drawing Error: The total number of edge pixels is larger than "
        "MaxNumOfEdgePixels, "
  ...
  for (unsigned int edgeId = 0; edgeId < offsetPS; edgeId++) {
    ...
    for (tempID = offsetPFirst - 1; tempID >= indexInArray;
    ...
    for (tempID = indexInArray + 1; tempID < (int)offsetPSecond;
...
int EDLineDetector::EDline(image_int8u_p image, LineChains &lines,
                           bool smoothed) {
  ...
  printf("edges.numOfEdges; %zu, linePixelID: %zu\n", edges.numOfEdges,
         linePixelID);
  ...
  unsigned int offsetInEdgeArrayS, offsetInEdgeArrayE,
      newOffsetS; // start index and end index
  ...
  for (unsigned int edgeID = 0; edgeID < edges.numOfEdges; edgeID++) {
    ...
    while (offsetInEdgeArrayE >
           offsetInEdgeArrayS +
               minLineLen_) { // extract line segments from an edge, may find
      ...
      while (offsetInEdgeArrayE > offsetInEdgeArrayS + minLineLen_) {
        ...
        offsetInEdgeArrayS +=
            SkipEdgePoint; // skip the first two pixel in the chain and try with
      ...
      float pointToLineDis; // for a line ax+by+c=0 and a point(xi, yi),
      ...
      if (pdirImg[pEdgeYCors[offsetInEdgeArrayS] * imageWidth +
                  pEdgeXCors[offsetInEdgeArrayS]] ==
          Horizontal) { // y=ax+b, i.e. ax-y+b=0
        while (bExtended) {
          tryTimes++;
          if (bFirstTry) {
            bFirstTry = false;
            for (int i = 0; i < minLineLen_;
                 i++) { // First add the initial line segment to the line array
              pLineXCors[offsetInLineArray] = pEdgeXCors[offsetInEdgeArrayS];
              pLineYCors[offsetInLineArray++] =
                  pEdgeYCors[offsetInEdgeArrayS++];
            }
          } else { // after each try, line is extended, line equation should be
                   // re-estimated
            // adjust the line equation
            lineFitErr = LeastSquaresLineFit_(pLineXCors, pLineYCors,
                                              pLineSID[numOfLines], newOffsetS,
          ...
          while (offsetInEdgeArrayE > offsetInEdgeArrayS) {
            pointToLineDis =
                fabs(lineEquation[0] * pEdgeXCors[offsetInEdgeArrayS] -
          ...
          // pop back the last few outliers from lines and return them to edge
          // chain
          offsetInLineArray -= numOfOutlier;
          offsetInEdgeArrayS -= numOfOutlier;
          if (offsetInLineArray - newOffsetS > 0 &&
              tryTimes < TryTime) { // some new pixels are added to the line
          } else {
            bExtended = false; // no new pixels are added.
          }
        }
        // the line equation coefficients,for line w1x+w2y+w3 =0, we normalize
        // it to make w1^2+w2^2 = 1.
        std::array<float, 3> lineEqu = {lineEquation[0] * coef1, -1 * coef1,
                                        lineEquation[1] * coef1};
        if (LineValidation_(pLineXCors, pLineYCors, pLineSID[numOfLines],
                            offsetInLineArray, lineEqu,
          ...
          offsetInLineArray = pLineSID[numOfLines]; // line was not accepted,
                                                    // the offset is set back
        }
        ...
        while (bExtended) {
          ...
          while (offsetInEdgeArrayE > offsetInEdgeArrayS) {
            pointToLineDis =
                fabs(pEdgeXCors[offsetInEdgeArrayS] -
                     lineEquation[0] * pEdgeYCors[offsetInEdgeArrayS] -
                     lineEquation[1]) *
                coef1;
            pLineXCors[offsetInLineArray] = pEdgeXCors[offsetInEdgeArrayS];
            pLineYCors[offsetInLineArray++] = pEdgeYCors[offsetInEdgeArrayS++];
        ...
        // the line equation coefficients,for line w1x+w2y+w3 =0, we normalize
        // it to make w1^2+w2^2 = 1.
        std::array<float, 3> lineEqu = {1 * coef1, -lineEquation[0] * coef1,
                                        -lineEquation[1] * coef1};
        if (LineValidation_(pLineXCors, pLineYCors, pLineSID[numOfLines],
                            offsetInLineArray, lineEqu,
                            direction)) { // check the line
      ...
      // Extract line segments from the remaining pixel; Current chain has been
      // shortened already.
    }
...
float EDLineDetector::LeastSquaresLineFit_(unsigned int *xCors,
                                           unsigned int *yCors,
                                           unsigned int offsetS,
                                           std::array<float, 2> &lineEquation) {
  ...
  if (pdirImg[yCors[offsetS] * imageWidth + xCors[offsetS]] == Horizontal) {
    /*Build the system,and solve it using least square regression: mat * [a,b]^T
     * = vec */
    ...
    pATA = ATA->data;
    coef = 1.f /
           (float(pATA[0]) * float(pATA[3]) - float(pATA[1]) * float(pATA[2]));
    //		lineEquation = svd.Invert(ATA) * matT * vec;

    lineEquation[0] =
        coef * (float(pATA[3]) * float(ATV->data[0 * ATV->xsize + 0]) -
                float(pATA[1]) * float(ATV->data[0 * ATV->xsize + 1]));
    lineEquation[1] =
        coef * (float(pATA[0]) * float(ATV->data[0 * ATV->xsize + 1]) -
                float(pATA[2]) * float(ATV->data[0 * ATV->xsize + 0]));

    /*compute line fit error */
    for (int i = 0; i < minLineLen_; i++) {
      coef = float(yCors[offset]) - float(xCors[offset++]) * lineEquation[0] -
             lineEquation[1];
      fitError += coef * coef;
    }
  ...
  if (pdirImg[yCors[offsetS] * imageWidth + xCors[offsetS]] == Vertical) {
    ...
    /*compute line fit error */
    for (int i = 0; i < minLineLen_; i++) {
      coef = float(xCors[offset]) - float(yCors[offset++]) * lineEquation[0] -
             lineEquation[1];
      fitError += coef * coef;
    }
    return sqrt(fitError);
...
float EDLineDetector::LeastSquaresLineFit_(unsigned int *xCors,
  ...
  if (length <= 0 || newLength <= 0) {
    printf("EDLineDetector::LeastSquaresLineFit_ Error:"
           " the expected line index is wrong...offsetE = %d, offsetS = %d, "
           "newOffsetS = %d\n",
   ...
   *then we try to find a horizontal line, y=ax+b;*/
  if (pdirImg[yCors[offsetS] * imageWidth + xCors[offsetS]] == Horizontal) {
    /*Build the new system,and solve it using least square regression: mat *
     * [a,b]^T = vec */
...
bool EDLineDetector::LineValidation_(unsigned int *xCors, unsigned int *yCors,
                                     unsigned int offsetS, unsigned int offsetE,
                                     std::array<float, 3> &lineEquation,
                                     float &direction) {
  if (bValidate_) {
    int n = offsetE - offsetS;
    /*first compute the direction of line, make sure that the dark side always
     *be the left side of a line.*/
    ...
    //    dx = std::floor(dx * 100000.0) / 100000.0;
    //    dy = std::floor(dy * 100000.0) / 100000.0;
    if (meanGradientX == 0 &&
        meanGradientY ==
            0) { // not possible, if happens, it must be a wrong line,
      return false;
    }
    if (meanGradientX > 0 &&
        meanGradientY >=
            0) { // first quadrant, and positive direction of X axis.
      direction = atan2(-dy, dx); // line direction is in fourth quadrant
    }
    if (meanGradientX <= 0 &&
        meanGradientY >
            0) { // second quadrant, and positive direction of Y axis.
      direction = atan2(dy, dx); // line direction is in first quadrant
    }
    if (meanGradientX < 0 &&
        meanGradientY <=
            0) { // third quadrant, and negative direction of X axis.
      direction = atan2(dy, -dx); // line direction is in second quadrant
    }
    if (meanGradientX >= 0 &&
        meanGradientY <
            0) { // fourth quadrant, and negative direction of Y axis.
      direction = atan2(-dy, -dx); // line direction is in third quadrant
    }
    /*then check whether the line is on the border of the image. We don't keep
     * the border line.*/
    if (fabs(direction) < 0.15f ||
        PI - fabs(direction) < 0.15f) { // Horizontal line
      if (fabs(lineEquation[2]) < 10 ||
          fabs(imageHeight - fabs(lineEquation[2])) <
              10) { // upper border or lower border
        return false;
      }
    }
    if (fabs(fabs(direction) - PI * 0.5f) < 0.15f) { // Vertical line
      if (fabs(lineEquation[2]) < 10 ||
          fabs(imageWidth - fabs(lineEquation[2])) <
              10) { // left border or right border
        return false;
      }
    }
    // count the aligned points on the line which have the same direction as the
    // line.
    float disDirection;
    int k = 0;
    for (int i = 0; i < n; i++) {
      disDirection = fabs(direction - pointDirection[i]);
      if (fabs(2 * PI - disDirection) < 0.392699f ||
          disDirection <
              0.392699f) { // same direction, pi/8 = 0.392699081698724
        k++;
      }
    }
...
int EDLineDetector::EDline(image_int8u_p image, bool smoothed) {
  ...
  if (EDline(image, lines_, smoothed))
    return 1;
  // printf("d2, lines_.numOfLines: %zu\n", lines_.numOfLines);
  if (lines_.numOfLines == ZERO) {
    return 1;
  }
  lineSalience_.clear();
  lineSalience_.resize(lines_.numOfLines);
  // printf("d3, lines_.numOfLines: %d\n", lines_.numOfLines);
  ...
  printf("xCors: %d, yCors: %d, sId: %d, numOfLines: %d\n", lines_.xCors.size(),
         lines_.yCors.size(), lines_.sId.size(), lines_.numOfLines);
  if (lines_.xCors.size() == ZERO || lines_.yCors.size() == ZERO ||
      lines_.sId.size() == ZERO || lines_.numOfLines == ZERO) {
    return 1;
  }
  for (unsigned int i = 0; i < lineSalience_.size(); i++) {
    int salience = 0;
    // printf("i: %d\n", i);
    for (indexInLineArray = pSID[i]; indexInLineArray < pSID[i + 1];
         indexInLineArray++) {
      salience += (unsigned char)
          pgImg[pYCor[indexInLineArray] * imageWidth + pXCor[indexInLineArray]];
    }
    lineSalience_[i] = (float)salience;
  }
  // printf("d4, lines_.numOfLines: %d\n", lines_.numOfLines);
  return 0;
}
...
LineDescriptor::LineDescriptor(unsigned int numOfBand,
...
LineDescriptor::~LineDescriptor() {
  if ((edLineVec_[ZERO]->imageWidth != NULL) &&
      (edLineVec_[ZERO]->imageHeight != NULL))
...
/*Line detection method: element in keyLines[i] includes a set of lines which is
 * the same line detected in different scaled images.
 */
int LineDescriptor::ScaledKeyLines(image_int8u_p image,
                                   ScaleLineSet &keyLines) {
  ...
  /*lines which correspond to the same line in the scaled images will be stored
   * in the same element of ScaleLineSet.*/
  std::vector<ScaledLine> scaledLines(
      numOfFinalLine); // store the lines in ScaledLine structure
  numOfFinalLine =
      0; // store the number of finally accepted lines in ScaleLineSet
  unsigned int lineIDInScaleLineVec = 0;
  float dx, dy;
  for (unsigned int lineCurId = 0; lineCurId < edLineVec_[0]->lines_.numOfLines;
       lineCurId++) { // add all line detected in the original image
    ...
    dx = fabs(edLineVec_[0]->lineEndpoints_[lineCurId][0] -
              edLineVec_[0]->lineEndpoints_[lineCurId][2]); // x1-x2
    dy = fabs(edLineVec_[0]->lineEndpoints_[lineCurId][1] -
              edLineVec_[0]->lineEndpoints_[lineCurId][3]); // y1-y2
  ...
  for (unsigned int lineID = 0; lineID < numOfFinalLine; lineID++) {
    ...
    if ((direction >= 0.75f * PI && direction < PI) ||
        (direction >= -PI && direction < -0.75f * PI)) {
      if (dx > 0) {
        shouldChange = true;
      }
    }
...
static image_int8u_p gaussian_sampler_byte(image_int8u_p in,
                                           pixel_float_t scale,
                                           float sigma_scale) {
  image_float_p aux = NULL;
  image_int8u_p out = NULL;
...
/*This function is used to get numbers of pixels in a line from image.*/
int LineDescriptor::GetLinePixelsNums(float startX, float startY, float endX,
                                      float endY) {
  int num;
...
/*This function is used to get information of lines from downsampled image.*/
void LineDescriptor::InverseGaussianSamplerLines(pixel_float_t gs_scale,
                                                 ScaleLineSet &keyLines) {
  ...
  if ((gs_scale.u != 1.f) && (gs_scale.v != 1.f)) {
    for (k = 0; k < nsize; k++) {
      ...
      keyLines[k][0].numOfPixels = GetLinePixelsNums(
          keyLines[k][0].startPointX, keyLines[k][0].startPointY,
          keyLines[k][0].endPointX, keyLines[k][0].endPointY);
    }
...
int _edge_drawing_line_detector(unsigned char *src, int w, int h, float scaleX,
...
int EdgeDrawingLineDetector(unsigned char *src, int w, int h, float scaleX,
  ...
  return _edge_drawing_line_detector(src, w, h, scaleX, scaleY, bbox, lines);
}
} // namespace edlines
} // namespace dso