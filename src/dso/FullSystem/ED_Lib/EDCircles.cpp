...
EDCircles::EDCircles(Mat srcImage) : EDPF(srcImage) {
  ...
  // ---------------------------------
  int bufferSize = 0;
  for (int i = 0; i < segmentPoints.size(); i++) bufferSize += (int)segmentPoints[i].size();
...
cv::Mat EDCircles::drawResult(bool onImage, ImageStyle style) {
  ...
  if (style == ImageStyle::ELLIPSES || style == ImageStyle::BOTH)
    for (int i = 0; i < ellipses.size(); i++) {
      double degree = (ellipses[i].theta * 180) / PI;  // convert radian to degree (opencv's
    ...
    for (int curSegmentNo = 0; curSegmentNo < segmentNos; curSegmentNo++) {
      ...
      while (firstLine < stopLine - 1) {
        ...
        if (lastLine - firstLine == 1) {
          ...
          if (info[firstLine].angle >= PI / 12 && info[firstLine].angle <= PI / 4 && totalLineLength >= 40 &&
...
void EDCircles::ValidateCircles() {
  ...
  for (int i = 0; i < noCircles1;) {
    ...
    if (circle->isEllipse)
      circumference = computeEllipsePerimeter(&circle->eq);
    else
...
Circle* EDCircles::addCircle(Circle* circles, int& noCircles, double xc, double yc, double r, double circleFitError,
...
double EDCircles::ComputeEllipseCenterAndAxisLengths(EllipseEquation* eq, double* pxc, double* pyc,
...
//
void EDCircles::addArc(MyArc* arcs, int& noArcs, double xc, double yc, double r, double circleFitError, double sTheta,
                       double eTheta, int turn, int segmentNo, int sx, int sy, int ex, int ey, double* x, double* y,
...
//
void EDCircles::addArc(MyArc* arcs, int& noArcs, double xc, double yc, double r, double circleFitError, double sTheta,
                       double eTheta, int turn, int segmentNo, EllipseEquation* pEq, double ellipseFitError, int sx,
                       int sy, int ex, int ey, double* x, double* y, int noPixels, double overlapRatio) {
  arcs[noArcs].xc = xc;
  arcs[noArcs].yc = yc;
  arcs[noArcs].r = r;
  arcs[noArcs].circleFitError = circleFitError;
...
//
void EDCircles::ComputeStartAndEndAngles(double xc, double yc, double r, double* x, double* y, int len, double* psTheta,
                                         double* peTheta) {
  double sx = x[0];
  double sy = y[0];
  double ex = x[len - 1];
