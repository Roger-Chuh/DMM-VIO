 ...
 * main_dso_pangolin.cpp of the project DSO written by Jakob Engel, but have
 * been modified for the inclusion in DM-VIO. The original versions have the
 * copyright Copyright 2016 Technical University of Munich and Intel. Developed
 * by Jakob Engel <engelj at in dot tum dot de>,
 *
...
void MainSettings::parseArgument(char *arg, SettingsUtil &settingsUtil) {
  ...
  if (1 == sscanf(arg, "mode=%d", &option)) {
    ...
    if (option == 3) {
      // This mode is useful because mode 0 assumes that exposure is available
      // (as it adds a strong prior to the affine brightness change between
      // images), and mode 1 does not use vignette at all. This mode uses
...
void dmvio::MainSettings::settingsDefault(int preset) {
  ...
  if (preset == 0 || preset == 1) {
    ...
    preload = preset == 1;
    setting_desiredImmatureDensity = 5000; // 3000; // 1500;
    setting_desiredPointDensity = 2000;    // 2000;    // 1000;
    setting_minFrames = 5;                 // 15;//5;
    setting_maxFrames = 30;                // 12;//7;//30;//7;
