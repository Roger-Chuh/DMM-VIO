...
typedef std::vector<LineSet> ScaleLineSet;  // each element in ScaleLineSet is a vector of lines which
                                            // corresponds the
...
class EDLineDetector {
 ...
 private:
   ...
   *1:error happens;
   */
  float LeastSquaresLineFit_(unsigned int* xCors, unsigned int* yCors, unsigned int offsetS,
   ...
   *happens;
   */
  float LeastSquaresLineFit_(unsigned int* xCors, unsigned int* yCors, unsigned int offsetS, unsigned int newOffsetS,
   ...
   */
  bool LineValidation_(unsigned int* xCors, unsigned int* yCors, unsigned int offsetS, unsigned int offsetE,
 ...
 private:
  ...
  static float log_gamma_lanczos(float x) {
    static float q[7] = {75122.6331530f, 80916.6278952f, 36308.2951477f, 8687.24529705f,
                         1168.92649479f, 83.8676043424f, 2.50662827511f};
  ...
  static float log_gamma_windschitl(float x) {
    return 0.918938533204673f + (x - 0.5f) * log(x) - x + 0.5f * x * log(x * sinh(1 / x) + 1 / (810.f * pow(x, 6.f)));
  }
  /** Computes -log10(NFA).*/
  static float nfa(int n, int k, float p, float logNT) {
    ...
    log1term = log_gamma((float)n + 1.f) - log_gamma((float)k + 1.f) - log_gamma((float)(n - k) + 1.f) +
    ...
    for (i = k + 1; i <= n; i++) {
      ...
      bin_tail += term;
      if (bin_term < 1.f) {
        err = term * ((1.f - pow(mult_term, (float)(n - i + 1))) / (1.f - mult_term) - 1.f);
...
class LineDescriptor {
 ...
 public:
  /* Interface.*/
  int Run(float scaleX, float scaleY, boundingbox_t bbox, image_int8u_p image, ScaleLineSet& keyLines);
...
static void mcv_sobel(ORIENT_CODE oriention, image_int8u_p src, image_int16s_p dst) { sobel_edge(oriention, src, dst); }
...
static void mcv_abs(image_int16s_p src, image_int16s_p dst) { array_abs(src, dst); }
...
static int array_add(image_float_p src1, image_float_p src2, image_float_p dst) {
  ...
  if ((src1 == NULL) || (src2 == NULL) || (dst == NULL) || (src1->xsize != src2->xsize) ||
      (src1->ysize != src2->ysize) || (src1->xsize != dst->xsize) || (src1->ysize != dst->ysize))
    return 1;
...
static void mcv_add(image_int16s_p src1, image_int16s_p src2, image_int16s_p dst) { array_add(src1, src2, dst); }

static void mcv_add(image_float_p src1, image_float_p src2, image_float_p dst) { array_add(src1, src2, dst); }
...
static void mcv_threshold(short thresh, image_int16s_p src, image_int16s_p dst) { array_threshold(thresh, src, dst); }
...
static void mcv_mat_divide(int n, image_int16s_p src, image_int16s_p dst) { array_devide(n, src, dst); }
...
static int array_multiply_transpose_float(image_float_p src, image_float_p dst) {
  ...
  if (src == NULL || dst == NULL || src->data == NULL || dst->data == NULL) return 1;

  if (dst->xsize != dst->ysize) return 1;

  // A*AT
  if (src->ysize != dst->xsize) return 1;
  ...
  for (i = 0; i <= m - 1; i++)
    for (j = 0; j <= m - 1; j++) {
      _sum = 0.f;
      for (l = 0; l <= n - 1; l++) _sum += psrc[i * n + l] * psrc[j * n + l];
      pdst[i * m + j] = _sum;
    }
...
static int array_multiply2_transpose_float(image_float_p src1, image_float_p src2, image_float_p dst) {
  ...
  if (src1 == NULL || src2 == NULL || dst == NULL || src1->data == NULL || src2->data == NULL || dst->data == NULL)
    return 1;

  // A*BT
  if (src1->ysize != dst->ysize || src2->ysize != dst->xsize) return 1;

  if (src1->xsize != src2->xsize) return 1;
...
static int array_multiply(image_float_p src1, image_float_p src2, image_float_p dst) {
  ...
  if (src1->xsize != src2->ysize) return 1;

  if (dst->ysize != src1->ysize || dst->xsize != src2->xsize) return 1;
  ...
  for (i = 0; i <= m - 1; i++)
    for (j = 0; j <= k - 1; j++) {
      u = i * k + j;
      c[u] = 0.f;
      for (l = 0; l <= n - 1; l++) c[u] = c[u] + a[i * n + l] * b[l * k + j];
    }
...
int EDLineDetector::EdgeDrawing(image_int8u_p image, EdgeChains& edgeChains, bool smoothed) {
  ...
  for (unsigned int h = 1; h < imageHeight - 1; h = h + scanIntervals_) {
    ...
    for (unsigned int w = 1; w < imageWidth - 1; w = w + scanIntervals_) {
      indexInArray = offy + w;
      if (pdirImg[indexInArray] == Horizontal) {  // if the direction of pixel is horizontal, then compare
                                                  // with up and down
  ...
  if (anchorsSize > edgePixelArraySize) {
    printf(
        "anchor size is larger than its maximal size. anchorsSize = %d, "
        "maximal size = %d\n",
  ...
  for (unsigned int i = 0; i < anchorsSize; i++) {
    ...
    if (pdirImg[indexInArray] == Horizontal) {  // if the direction of this pixel is horizontal, then go
                                                // left and right.
  ...
  if (offsetPS > maxNumOfEdge) {
    printf(
        "Edge drawing Error: The total number of edges is larger than "
        "MaxNumOfEdge, "
...
int EDLineDetector::EDline(image_int8u_p image, LineChains& lines, bool smoothed) {
  ...
  for (unsigned int edgeID = 0; edgeID < edges.numOfEdges; edgeID++) {
    ...
    while (offsetInEdgeArrayE > offsetInEdgeArrayS + minLineLen_) {  // extract line segments from an edge, may find
      ...
      while (offsetInEdgeArrayE > offsetInEdgeArrayS + minLineLen_) {
        ...
        offsetInEdgeArrayS += SkipEdgePoint;            // skip the first two pixel in the chain and try with
...
float EDLineDetector::LeastSquaresLineFit_(unsigned int* xCors, unsigned int* yCors, unsigned int offsetS,
                                           std::array<float, 2>& lineEquation) {
...
float EDLineDetector::LeastSquaresLineFit_(unsigned int* xCors, unsigned int* yCors, unsigned int offsetS,
  ...
  if (length <= 0 || newLength <= 0) {
    printf(
        "EDLineDetector::LeastSquaresLineFit_ Error:"
...
bool EDLineDetector::LineValidation_(unsigned int* xCors, unsigned int* yCors, unsigned int offsetS,
                                     unsigned int offsetE, std::array<float, 3>& lineEquation, float& direction) {
  if (bValidate_) {
     ...
     * the border line.*/
    if (fabs(direction) < 0.15f || PI - fabs(direction) < 0.15f) {  // Horizontal line
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (fabs(2 * PI - disDirection) < 0.392699f ||
          disDirection < 0.392699f) {  // same direction, pi/8 = 0.392699081698724
        k++;
      }
    }
...
int EDLineDetector::EDline(image_int8u_p image, bool smoothed) {
  // printf("d1\n");
  if (EDline(image, lines_, smoothed)) return 1;
  ...
  printf("xCors: %d, yCors: %d, sId: %d, numOfLines: %d\n", lines_.xCors.size(), lines_.yCors.size(), lines_.sId.size(),
         lines_.numOfLines);
  if (lines_.xCors.size() == ZERO || lines_.yCors.size() == ZERO || lines_.sId.size() == ZERO ||
      lines_.numOfLines == ZERO) {
    return 1;
  }
...
int LineDescriptor::ScaledKeyLines(image_int8u_p image, ScaleLineSet& keyLines) {
  ...
  for (unsigned int lineID = 0; lineID < numOfFinalLine; lineID++) {
    ...
    singleLine.numOfPixels =
        edLineVec_[ZERO]->lines_.sId[lineIDInScaled + 1] - edLineVec_[ZERO]->lines_.sId[lineIDInScaled];
...
void LineDescriptor::InverseGaussianSamplerLines(pixel_float_t gs_scale, ScaleLineSet& keyLines) {
  ...
  if ((gs_scale.u != 1.f) && (gs_scale.v != 1.f)) {
    for (k = 0; k < nsize; k++) {
      ...
      keyLines[k][0].numOfPixels = GetLinePixelsNums(keyLines[k][0].startPointX, keyLines[k][0].startPointY,
...
int LineDescriptor::Run(float scaleX, float scaleY, boundingbox_t bbox, image_int8u_p image, ScaleLineSet& keyLines) {
...
int _edge_drawing_line_detector(unsigned char* src, int w, int h, float scaleX, float scaleY, boundingbox_t bbox,
  ...
  // printf("a1\n");
  if (lineDesc.Run(scaleX, scaleY, bbox, _src, lineVec)) return 1;
  // printf("a2\n");
  for (k = 0; k < lineVec.size(); k++) {
    lines.push_back(
        {lineVec[k][0].startPointX, lineVec[k][0].startPointY, lineVec[k][0].endPointX, lineVec[k][0].endPointY});
  }
...
int EdgeDrawingLineDetector(unsigned char* src, int w, int h, float scaleX, float scaleY, boundingbox_t bbox,
  ...
  if (scaleX * bbox.width < 10 || scaleY * bbox.height < 10) return 1;

  if (bbox.x < 0 || bbox.x > w - 1 || bbox.y < 0 || bbox.y > h - 1 || bbox.width <= 0 || bbox.height <= 0 ||
