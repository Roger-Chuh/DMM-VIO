...
int main(int argc, char **argv) {
  ...
  if (argc > 1)
    ...
    filename = "/home/roger/work/dm-vio/dm-vio/src/dso/FullSystem/ED_Lib/"
               "planet_glow.jpg";
  ...
  for (int i = 0; i < 3; i++) {
    ...
    tm.stop();
    std::cout << "testED.getEdgeImage()  (Original)  : " << tm.getTimeMilli()
              << " ms." << endl;
    ...
    tm.stop();
    std::cout << "detectEdges()            (OpenCV)  : " << tm.getTimeMilli()
              << " ms." << endl;

    Mat anchImg = testED.getAnchorImage();
    Mat gradImg = testED.getGradImage();
    imwrite("/home/roger/work/dm-vio/dm-vio/src/dso/FullSystem/ED_Lib/"
            "GradImage.png",
            gradImg);
    imwrite("/home/roger/work/dm-vio/dm-vio/src/dso/FullSystem/ED_Lib/"
            "AnchorImage.png",
    ...
    cout << "different pixel count              : " << countNonZero(diff)
         << endl;
    ...
    tm.stop();
    cout << "detectLines()            (OpenCV)  : " << tm.getTimeMilli()
         << " ms." << endl;
    ...
    tm.stop();
    cout << "-------------------------------------------------\n";
    cout << "EDCircles(testEDLines)             : " << tm.getTimeMilli()
         << " ms." << endl;
    ...
    tm.stop();
    cout << "detectEllipses()         (OpenCV)  : " << tm.getTimeMilli()
         << " ms." << endl;
    cout << "-------------------------------------------------\n";
    ...
    for (int i = 0; i < found_circles.size(); i++) {
      Point center((int)found_circles[i].center.x,
                   (int)found_circles[i].center.y);
    ...
    for (int i = 0; i < found_ellipses.size(); i++) {
      Point center((int)found_ellipses[i].center.x,
                   (int)found_ellipses[i].center.y);
      Size axes((int)found_ellipses[i].axes.width,
                (int)found_ellipses[i].axes.height);
    ...
    for (size_t i = 0; i < ellipses.size(); i++) {
      ...
      Size axes((int)ellipses[i][2] + (int)ellipses[i][3],
                (int)ellipses[i][2] + (int)ellipses[i][4]);
      double angle(ellipses[i][5]);
      Scalar color =
          ellipses[i][2] == 0 ? Scalar(255, 255, 0) : Scalar(0, 255, 0);
    ...
    tm.stop();
    cout << "testEDPF.getEdgeImage()            : " << tm.getTimeMilli()
         << " ms." << endl;
    ...
    tm.stop();
    cout << "-------------------------------------------------\n";
    cout << "testEDColor.getEdgeImage()         : " << tm.getTimeMilli()
         << " ms." << endl;
    ...
    tm.stop();
    cout << "detectEdges()            (OpenCV)  : " << tm.getTimeMilli()
         << " ms." << endl;
    ...
    tm.stop();
    cout << "-------------------------------------------------\n";
    cout << "get lines from color image         : " << tm.getTimeMilli()
         << " ms." << endl;
    ...
    tm.stop();
    cout << "get circles from color image       : " << tm.getTimeMilli()
         << " ms." << endl;
    tm.reset();
    tm.start();
    ed->detectEllipses(ellipses);
