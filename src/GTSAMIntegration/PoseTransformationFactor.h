...
// To pass a factor which works in coordinate system IMU, to a graph working in
// coordinate system DSO, we have to pass wrap the factor in TransformDSOToIMU.
// The class can also fix variables of the child factor.
// The factor will also apply First-Estimates Jacobians if setFEJMapForGraph has
...
public:
  ...
  enum ConversionType { JACOBIAN_FACTOR, JACOBIAN_BAKED_IN };

  // factor is the child factor which is transformed.
  // poseTransformation is the transformation which will be applied to values to
  // to convert from the graph to the child factor. conversionType defines how
