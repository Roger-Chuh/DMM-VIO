...
// Main class for the integration of IMU data into DSO. Mostly a facade as the
// implementation logic is forwarded to either CoarseIMULogic (for the coarse
// tracking), BAIMULogic (for the integration of IMU into the main bundle
// adjustment), and IMUInitializer (for the IMU initialization).
// This class is responsible for making sure that the preintegration and
...
public:
  ...
  // optimization iteration. returns new refToFrame, gets H and b as an input.
  // incA and incB are output parameters that contain the increment of the
  // affine lightning transforms after the method call, incNorm is the norm of
  ...
  Sophus::SE3 computeCoarseUpdate(const dso::Mat88 &H, const dso::Vec8 &b,
                                  float extrapFac, float lambda, double &incA,
                                  double &incB, double &incNorm);
  ...
  // tells the IMU-Integration that this frame has become a keyframe that will
  // shortly be bundle-adjusted.
  ...
  // These methods are both called right after the BA optimization, the former
  // outside and the latter inside the mutex for changing the coarse tracking
  // ref.
  void postOptimization(int keyframeId);

  // updateBAValues should be called before this. Returns true if this BA
  // optimization has used IMU data (which is also true if and only if the next
  // CoarseTracking will use IMU data.)
  bool finishKeyframeOptimization(int keyframeId);

  // Called when all keyframe operations (including marginalization) are
  // finished. Note that this can be after the tracking reference has already
  // changed e.g. the order can be: BA finished (finishKeyframeOptimization) ->
  // tracking reference changed -> in other thread marginalization and post BA
  // stuff -> finishKeyframeOperations()
...
private:
  ...
  // These variables are set when the BA optimization is finished, so the coarse
  // tracking can later initialize with these values.
