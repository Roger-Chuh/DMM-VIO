...
class ImageFolderReader {
 public:
  ImageFolderReader(std::string path, std::string calibFile, std::string gammaFile, std::string vignetteFile,
                    bool use16BitPassed, bool is_yvr_ = false, int width_in = 0, int height_in = 0,
                    std::array<std::pair<cv::Mat, cv::Mat>, kCameraNumUsed>* p_cid_to_undist_map_ = nullptr,
                    Eigen::Matrix<float, Eigen::Dynamic, Eigen::Dynamic>* p_vig_mat_ = nullptr) {
    this->path = path;
  ...
  void setGlobalCalibration2(const Eigen::Matrix3f K, const int& width, const int& height) {
  ...
  ImageAndExposure* getImage2(int id, bool forceLoadDirectly = false) { return getImage_internal2(id, 0); }
  ...
  dmvio::IMUData getIMUData(int i) {
    ...
    if (i <= 0 || i - 1 >= (int)imuDataAllFrames.size()) {
      std::cout << "i <= 0 || i - 1 >= (int)imuDataAllFrames.size()" << std::endl;
      std::exit(1);
  ...
  bool loadGTData(std::string gtFile) {
    ...
    while (!tr.eof() && tr.good()) {
      ...
      if (17 == sscanf(buf,
                       ...
                       "lf,%lf,%lf,%lf",
                       &id, &p1, &p2, &p3, &qw, &qx, &qy, &qz, &v1, &v2, &v3, &br1, &br2, &br3, &bp1, &bp2, &bp3)) {
  ...
  void loadIMUData2(const std::map<int64_t, ImuDataSingle>& imu_stack, dso::aligned_vector<CalibFrame>& frameInfo) {
  ...
  void loadIMUData(std::string imuFile = "") {
    ...
    if (imuStream.good()) {
      ...
      for (size_t j = 0; j < getNumImages(); ++j) {
        ...
        if (imuStamp > imageTimestamp) {
          std::cout << "IMU-data too old -> skipping frame, imu stamp: " << imuStamp
                    << ", img stamp: " << imageTimestamp << std::endl;
      ...
      for (size_t j = startFrame; j < getNumImages() - 1; j++) {
        ...
        assert(imuStamp == imageTimestamp);  // Otherwise we would need to interpolate IMU
                                             // data which is not implemented atm.
        dmvio::IMUData imuData;
 ...
 private:
  ...
  MinimalImageB* getImageRaw_internal2(int id, int unused) {
    ...
    if (!isZipped) {
      // CHANGE FOR ZIP FILE
      return IOWrap::readImageBW_8U2(id, p_input_data, p_cid_to_undist_map, p_vig_mat);
    } else {
#if HAS_ZIPLIB
      if (databuffer == 0) databuffer = new char[widthOrg * heightOrg * 6 + 10000];
      zip_file_t* fle = zip_fopen(ziparchive, files[id].c_str(), 0);
  ...
  ImageAndExposure* getImage_internal(int id, int unused) {
    if (use16Bit) {
      ...
      ImageAndExposure* ret2 =
          undistort->undistort<unsigned short>(minimg, (exposures.size() == 0 ? 1.0f : exposures[id]),
    ...
    if (use16Bit) {
      ...
      for (int cid = 0; cid < kCameraNumUsed; ++cid) {
        for (int i = 0; i < minimg->w * minimg->h; i++) {
          result->image[i + minimg->w * minimg->h * cid] = float(1.0) * minimg->data[i + minimg->w * minimg->h * cid];
        }
      }
#endif
      delete minimg;
  ...
  bool use16Bit;
  bool is_yvr = false;
  std::array<std::pair<cv::Mat, cv::Mat>, kCameraNumUsed>* p_cid_to_undist_map = nullptr;
