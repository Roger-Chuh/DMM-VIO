...
EDLineDetector::EDLineDetector() {
  ...
  ksize_ = 5;
  sigma_ = 1.0;
  gradienThreshold_ = 15; // 80;  // ***** ORIGINAL WAS 25
...
int EDLineDetector::EdgeDrawing(cv::Mat &image, EdgeChains &edgeChains,
  ...
  //		memcpy(cvImage->data.ptr, pImage, pixelNum*sizeof(unsigned
  // char));
  //	}
  ...
  for (unsigned int w = 1; w < imageWidth - 1; w = w + scanIntervals_) {
    for (unsigned int h = 1; h < imageHeight - 1; h = h + scanIntervals_) {
      ...
      } else { // if(pdirImg[indexInArray]==Vertical){//it is vertical edge,
               // should be compared with left and right
        // gValue2 = pgImg[indexInArray];
  ...
  if (anchorsSize > edgePixelArraySize) {
    cout << "anchor size is larger than its maximal size. anchorsSize="
         << anchorsSize << ", maximal size = " << edgePixelArraySize << endl;
    return -1;
  }
#ifdef DEBUGEdgeDrawing
  ...
  for (unsigned int i = 0; i < anchorsSize; i++) {
     ...
     *    to vertical or vice versa.?? (This is turned out not correct. From the
     * online edge draw demo we can figure out that authors don't implement this
     * rule either because their extracted edge chain could be a circle which
     * means pixel directions would definitely be different in somewhere on the
     * chain.)
  ...
  if (offsetPS > maxNumOfEdge) {
    cout << "Edge drawing Error: The total number of edges is larger than "
            "MaxNumOfEdge, "
            "numofedge = "
  ...
  if (offsetPFirst > edgePixelArraySize || offsetPSecond > edgePixelArraySize) {
    cout << "Edge drawing Error: The total number of edge pixels is larger "
            "than MaxNumOfEdgePixels, "
            "numofedgePixel1 = "
         << offsetPFirst << ",  numofedgePixel2 = " << offsetPSecond
         << ", MaxNumOfEdgePixel=" << edgePixelArraySize << endl;
    return -1;
  }

  /*now all the edge information are stored in pFirstPartEdgeX_,
   *pFirstPartEdgeY_, pFirstPartEdgeS_,  pSecondPartEdgeX_, pSecondPartEdgeY_,
   *pSecondPartEdgeS_; we should reorganize them into edgeChains for easily
   *using.	*/
  int tempID;
  ...
  for (unsigned int edgeId = 0; edgeId < offsetPS; edgeId++) {
    // step1, put the first and second parts edge coordinates together from edge
    // start to edge end
    ...
    for (tempID = indexInArray + 1; tempID < offsetPSecond;
  ...
  //	/*Show the extracted edge cvImage in color. Each chain is in different
  // color.*/ 	IplImage* cvColorImg =
  ...
  //		for(indexInCors = psId[i]; indexInCors<psId[i+1];
  // indexInCors++){
...
int EDLineDetector::EDline(cv::Mat &image, LineChains &lines, bool smoothed) {
  ...
  for (unsigned int edgeID = 0; edgeID < edges.numOfEdges; edgeID++) {
    ...
    while (offsetInEdgeArrayE >
      ...
      double pointToLineDis; // for a line ax+by+c=0 and a point(xi, yi),
      ...
      if (pdirImg[pEdgeYCors[offsetInEdgeArrayS] * imageWidth +
        ...
        std::array<double, 3> lineEqu = {lineEquation[0] * coef1, -1 * coef1,
        ...
        if (LineValidation_(pLineXCors, pLineYCors, pLineSID[numOfLines],
           ...
           *we project the first and last pixels in the pixelChain onto the best
           *fit line to get the line endpoints. xp=
           *(w2^2*x0-w1*w2*y0-w3*w1)/(w1^2+w2^2) yp=
           *(w1^2*y0-w1*w2*x0-w3*w2)/(w1^2+w2^2)  */
        ...
        // the line equation coefficients,for line w1x+w2y+w3 =0, we normalize
        // it to make w1^2+w2^2 = 1.
        std::array<double, 3> lineEqu = {1 * coef1, -lineEquation[0] * coef1,
                                         -lineEquation[1] * coef1};
        if (LineValidation_(pLineXCors, pLineYCors, pLineSID[numOfLines],
                            offsetInLineArray, lineEqu,
                            direction)) { // check the line
...
//		for(offsetInLineArray = pLineSID[i];
// offsetInLineArray<pLineSID[i+1]; offsetInLineArray++){
...
double
EDLineDetector::LeastSquaresLineFit_(unsigned int *xCors, unsigned int *yCors,
  ...
  if (pdirImg[yCors[offsetS] * imageWidth + xCors[offsetS]] == Horizontal) {
    ...
    pATA = ATA.ptr<float>();
    coef = 1.0 / (double(pATA[0]) * double(pATA[3]) -
                  double(pATA[1]) * double(pATA[2]));
    //		lineEquation = svd.Invert(ATA) * matT * vec;
    lineEquation[0] = coef * (double(pATA[3]) * double(ATV[0][0]) -
                              double(pATA[1]) * double(ATV[0][1]));
    lineEquation[1] = coef * (double(pATA[0]) * double(ATV[0][1]) -
                              double(pATA[2]) * double(ATV[0][0]));
    /*compute line fit error */
    for (int i = 0; i < minLineLen_; i++) {
      coef = double(yCors[offset]) - double(xCors[offset++]) * lineEquation[0] -
             lineEquation[1];
      fitError += coef * coef;
    }
  ...
  if (pdirImg[yCors[offsetS] * imageWidth + xCors[offsetS]] == Vertical) {
    ...
    /*compute line fit error */
    for (int i = 0; i < minLineLen_; i++) {
      coef = double(xCors[offset]) - double(yCors[offset++]) * lineEquation[0] -
             lineEquation[1];
      fitError += coef * coef;
    }
    return sqrt(fitError);
...
double EDLineDetector::LeastSquaresLineFit_(
    unsigned int *xCors, unsigned int *yCors, unsigned int offsetS,
  ...
  if (length <= 0 || newLength <= 0) {
    cout << "EDLineDetector::LeastSquaresLineFit_ Error:"
         ...
         << offsetE << ", offsetS=" << offsetS << ", newOffsetS=" << newOffsetS
         << endl;
    return -1;
  }
  if (lineEquation.size() != 2) {
  ...
  if (pdirImg[yCors[offsetS] * imageWidth + xCors[offsetS]] == Horizontal) {
    ...
    //			coef = double(yCors[offsetS]) - double(xCors[offsetS++])
    //* lineEquation[0] - lineEquation[1]; 			fitError +=
    // coef*coef;
    //		}
    return 0;
  }
  ...
  if (pdirImg[yCors[offsetS] * imageWidth + xCors[offsetS]] == Vertical) {
    ...
    /*compute line fit error */
    //		for(int i=0; i<length; i++){
    //			coef = double(xCors[offsetS]) - double(yCors[offsetS++])
    //* lineEquation[0] - lineEquation[1]; 			fitError +=
    // coef*coef;
    //		}
  }
...
bool EDLineDetector::LineValidation_(unsigned int *xCors, unsigned int *yCors,
  ...
  if (bValidate_) {
     ...
     * the border line.*/
    if (fabs(direction) < 0.15 ||
        M_PI - fabs(direction) < 0.15) { // Horizontal line
...
int EDLineDetector::EDline(cv::Mat &image, bool smoothed) {
  ...
  if (lines_.xCors.size() == 0 || lines_.yCors.size() == 0 ||
      lines_.sId.size() == 0 || lines_.numOfLines == 0) {
    return -1;
  }
