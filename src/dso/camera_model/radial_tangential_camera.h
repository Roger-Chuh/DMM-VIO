...
class RadtanCamera : public CameraBase {
 public:
  ...
  explicit RadtanCamera(CamId camera_id, int width, int height) : CameraBase(camera_id, width, height) {
    camera_model_ = CameraModel::kRadialTangential;
    kParamLength = 8;
  }

  // parameters: fx, fy, cx, cy, k1, k2, p1, p2
  RadtanCamera(CamId camera_id, int width, int height, const number_t* parameters)
