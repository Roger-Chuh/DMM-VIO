...
// - The called method can return the new state (or nullptr if the same state
// should be kept).
// - By manually calling the methods of IMUInitStateChanger (stored in
// IMUInitializerLogic). This is for changing the
//   state in a separate thread and mainly used by the realtime states.
//   IMPORTANT: This should not be called inside the methods defined by
//   IMUInitializerState, as they already have a lock on the state (and it would
//   result in a deadlock). They should instead just return the new state.
