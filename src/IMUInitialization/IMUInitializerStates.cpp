...
std::unique_ptr<IMUInitializerState> dmvio::DefaultActiveIMUInitializerState::addPose(const dso::FrameShell& shell,
...
std::unique_ptr<IMUInitializerState> dmvio::DefaultActiveIMUInitializerState::postBAInit(
    int keyframeId, gtsam::NonlinearFactor::shared_ptr activeHBFactor, const gtsam::Values& baValues, double timestamp,
...
std::unique_ptr<IMUInitializerState> dmvio::CoarseIMUInitState::addPose(const dso::FrameShell& shell,
                                                                        bool willBecomeKeyframe,
                                                                        const IMUData* imuData) {
...
void dmvio::CoarseIMUInitState::print(std::ostream& str) const { str << "CoarseIMUInit"; }
...
std::unique_ptr<IMUInitializerState> dmvio::RealtimeCoarseIMUInitState::addPose(const dso::FrameShell& shell,
                                                                                bool willBecomeKeyframe,
                                                                                const IMUData* imuData) {
...
void dmvio::RealtimeCoarseIMUInitState::print(std::ostream& str) const { str << "RealtimeCoarseIMUInitState"; }

std::unique_ptr<dmvio::IMUInitializerState> dmvio::createCoarseIMUInitState(
    dmvio::IMUInitializerLogic& imuInitLogic, dmvio::StateTransitionModel& transitionModel) {
...
std::unique_ptr<IMUInitializerState> dmvio::createPGBAState(IMUInitializerLogic& imuInitLogic,
                                                            StateTransitionModel& transitionModel,
...
gtsam::Values* prepareInitValuesForPGBA(std::unique_ptr<gtsam::Values>& initValuesNew,
                                        const std::unique_ptr<gtsam::Values>& initValues, const gtsam::Values& baValues,
...
std::unique_ptr<IMUInitializerState> dmvio::PGBAState::postBAInit(
  ...
  try {
    ...
    auto optimizedValues =
        std::make_unique<gtsam::Values>(logic.pgba->optimize(activeHBFactor, baValues, *initValuesUsed, false));
    ...
    if (pair.first) {
      ...
      auto newState = std::make_unique<InitializedFromPGBAState>(logic, transitionModel, std::move(optimizedValues));
      return std::move(newState);
    } else {
...
void RealtimePGBAState::threadRun() {
  ...
  } else  // Takeover result.
  {
    ...
    auto newState = std::make_unique<InitializedFromRealtimePGBAState>(
        logic, transitionModel, std::move(optimizedValues), std::move(cachedData));

    logic.stateChanger.setState(std::move(newState));
  }
}

void dmvio::RealtimePGBAState::print(std::ostream& str) const { str << "RealtimePGBAState"; }
...
std::pair<std::unique_ptr<IMUInitializerState>, bool> dmvio::InitializedFromPGBAState::initializeIfReady() {
  ...
  delayedGraph->setMaxGroupInGraph(BAIMULogic::METRIC_GROUP);  // From now on all factors should be added to
                                                               // this.
...
void dmvio::InitializedFromPGBAState::print(std::ostream& str) const { str << "InitializedFromPGBAState"; }
...
std::unique_ptr<IMUInitializerState> InitializedFromRealtimePGBAState::postBAInit(
    int keyframeId, gtsam::NonlinearFactor::shared_ptr activeHBFactor, const gtsam::Values& baValues, double timestamp,
    const gtsam::PreintegratedImuMeasurements& imuMeasurements) {
...
std::unique_ptr<IMUInitializerState> dmvio::PotentialMarginalizationReplacementState::postBAInit(
    int keyframeId, gtsam::NonlinearFactor::shared_ptr activeHBFactor, const gtsam::Values& baValues, double timestamp,
    const gtsam::PreintegratedImuMeasurements& imuMeasurements) {
  DefaultActiveIMUInitializerState::postBAInit(keyframeId, activeHBFactor, baValues, timestamp, imuMeasurements);
  ...
  bool changed =
      thresholdVariableChanges(useSecondTH ? logic.settings.secondThresholdSettings : logic.settings.thresholdSettings,
  ...
  // actually optimized.
  auto values = std::make_unique<gtsam::Values>(logic.pgba->optimize(activeHBFactor, baValues, initValues, true));
  ...
  if (!useSecondTH && firstId >= startIdForSecondTH) {
    ...
    bool reallyChanged = thresholdVariableChanges(logic.settings.secondThresholdSettings, baValues, fejVals->fejValues);
    if (!reallyChanged) {
...
dmvio::MarginalizationReplacementReadyState::MarginalizationReplacementReadyState(
    dmvio::IMUInitializerLogic& imuInitLogic, dmvio::StateTransitionModel& transitionModel,
    std::unique_ptr<gtsam::Values>&& optimizedValues)
    : DefaultActiveIMUInitializerState(imuInitLogic, transitionModel), optimizedValues(std::move(optimizedValues)) {}
