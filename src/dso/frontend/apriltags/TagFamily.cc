...
void TagFamily::decode(TagDetection& det, unsigned long long rCode, double tagLen, double tagVar, int& hamDist) const {
  ...
  // printf("errorRecoveryBits: %d\n", errorRecoveryBits);
  // printf("blackBorder: %d\n", blackBorder);
  if (true) {
    //  if (tagLen < 7000000) {
    det.good = (det.hammingDistance <= errorRecoveryBits);
  } else {
    det.good = (det.hammingDistance <= 9);
  }
  if (blackBorder == 3) {
    if (tagLen >= 60 && tagVar / tagLen <= 0.20 && det.hammingDistance <= 9) {
      det.good = true;
    }
    if (tagLen >= 70 && tagVar / tagLen <= 0.25 && det.hammingDistance <= 9) {
      det.good = true;
    }
    if (tagLen >= 80 && tagVar / tagLen <= 0.30 && det.hammingDistance <= 9) {
      det.good = true;
    }
  }
  if (!det.good) {
    // printf("tagLen: %f, det.hammingDistance: %d, tagVar: %f, var_len_ratio:
    // %f\n", tagLen, det.hammingDistance, tagVar, tagVar/tagLen);
  }
  det.obsCode = rCode;
  det.code = bestCode;
}
