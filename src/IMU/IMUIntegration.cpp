...
Sophus::SE3 IMUIntegration::addIMUData(const IMUData &imuData, int frameId,
  ...
  if (preparedKeyframe != -1 && !onlyForHint) {
    // Currently a new bundle adjustment is in progress -> Also put the imu data
    // into the preintegration for the next coarse tracking.
    if (imuData.size() > 0) {
...
Sophus::SE3 IMUIntegration::computeCoarseUpdate(const dso::Mat88 &H_in,
                                                const dso::Vec8 &b_in,
  ...
  assert(isCoarseInitialized()); // Caller is responsible for not calling if not
                                 // initialized.
  Sophus::SE3d newReferenceToFrame = coarseLogic->computeCoarseUpdate(
      H_in, b_in, extrapFac, lambda, incA, incB, incNorm);
...
void IMUIntegration::setGTData(dmvio::GTData *gtData, int frameId) {
  ...
  // the correct gtData for the KF + it might be deleted...)
  baLogic->setCurrGtData(gtData, frameId);

  // Note: At the moment coarse biases are only printed for frames that will
  // become a KF (because otherwise this method is not called).
...
void IMUIntegration::prepareKeyframe(int frameId) {
  ...
  // old one hasn't finished BA yet. There are two ways this can happen:
  // 1. The old keyframe is currently in BA and the system already needs a new
  // KF.
  // 2. The system has just prepared the previous preparedKeyframe, but hasn't
  // started BA yet. In this case, normal DSO would optimize the new frame and
  // the old preparedKeyframe would not become a KF.

  if (previouslyPrepared) {
    // If the last KF was also prepared the two buffers were already swapped.
    // Then the latest IMU-data has to be added to the current buffer. In the
  ...
  // Take latest bias from BA. Maybe this should actually locked. Currently
  // however the BA must be finished already, as else the assertion would have
  // failed. Therefore the latestBias cannot be changed right now.
...
// This method gets the latest BA values for the coarse tracker (for the next
// graph).
...
void IMUIntegration::skipPreparedKeyframe() { preparedKeyframe = -1; }

void IMUIntegration::keyframeCreated(int frameId) {
  assert(frameId == preparedKeyframe);
  preparedKFCreated = true;
  // This is locked with mutex, meaning that there cannot be a BA, or a call to
  // prepareKeyframe at the same time.
