...
// This file contains methods to convert poses betwen coordinate systems, and
// also compute relative derivatives (Jacobians) to convert Hessians and GTSAM
// factors between the systems. Usage: Define a child class of
// PoseTransformation which can transform poses and also compute relative
// derivatives. Then either convert a Hessian (H) and gradient vector (b) using
// convertHAndBWithPoseTransformation, or simply use a PoseTransformationFactor.

// Defines on which side of of the pose the epsilon is for the input (first) and
// output (second) transformation. When computing derivatives w.r.t. poses using
// Lie Groups, the increment can either be placed on the left or the right side.
// DSO places the increment on the left side. i.e. newPose = exp(increment) *
// oldPose. GTSAM places the increment on the right side, i.e. newPose = oldPose
// * exp(increment). Both approaches are equally valid, but the Jacobians need
// to be computed consistently with the increment side. With the relative
// Jacobians computed by our PoseTransformations we can convert between the two.
// Note: that for computing relative Jacobians they work on the inverse
...
// system. Also contains methods to compute relative derivatives. These will
// transform Jacobians according to the transformation. An example
// implementation is TransformDSOToIMU (in PoseTransformationIMU.h).
// --------------------------------------------------
// By default only derivatives w.r.t. to the pose are computed (by the method
// getPoseDerivative). However some transformations have additional variables
// which are optimized (e.g. the scale, or the transform T_cam_imu), which can
// influence the transformed pose. These additional variables can be queried
// with getAllOptimizedSymbols, and derivatives w.r.t. these variables can be
// computed with getAllDerivatives. When optimizing these additional variables
// (e.g. in a factor graph), they can be updated with updateẂithValues.
// --------------------------------------------------
// This class also contains a default implementation for getPoseDerivative using
// numeric Jacobians, which should work out of the box, in case the child class
// does not override the method with an analytic Jacobian computation. Child
...
// to the Hessians: To convert a Hessian that is defined in the IMU space to the
...
// Compute the numeric Jacobian w.r.t the given variable, which can either be
// the argument (pose) itself or one of the members of transformation. Note that
// this method modifies the passed variableToChange (which can be an internal
...
gtsam::Matrix computeNumericJacobian(PoseTransformation &transformation,
  ...
  // Numeric Jacobians work by slightly changing the pose in each direction, and
  // then computing how much it effects the converted pose.
