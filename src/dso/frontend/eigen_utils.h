...
class Pose3 {
 public:
  ...
  Pose3(const Eigen::Quaternion<T>& q, const Eigen::Vector3<T>& t) : q_(q), t_(t) {}
  ...
  Pose3(const Eigen::Matrix<T, 3, 3>& R, const Eigen::Vector3<T>& t) {
    ...
    if (std::abs(q_.norm() - 1) > 1e-6) {
      ...
      // quaternion.");
      std::cerr << "The quaternion extracted is not a unit quaternion difference is " << std::abs(q_.norm() - 1)
                << std::endl;
      q_.normalize();
    }
    t_ = t;
  ...
  Eigen::Vector4<T> GetRotation() const { return Eigen::Vector4<T>(q_.w(), q_.x(), q_.y(), q_.z()); }
  ...
  Eigen::Vector3<T> operator*(const Eigen::Vector3<T>& p) const { return this->rotation() * p + this->translation(); }
  ...
  friend std::ostream& operator<<(std::ostream& os, const Pose3<T>& pose) {
    os << "q: " << pose.rotation().w() << " " << pose.rotation().x() << " " << pose.rotation().y() << " "
       << pose.rotation().z() << ", t: " << pose.translation().transpose();
    return os;
  }
