...
//* 零空间, 好奇怎么求???
    ...
    //TODO* 这三个是与线性化点的增量(相对于固定线性化点的增量), 而光度参数不是增量, state就是值 !!!
...
//内联提高效率, 返回上面的值
    ...
    EIGEN_STRONG_INLINE const Vec10 get_state_minus_stateZero() const {return get_state() - get_state_zero();} //TODO x小量可以直接减 lie algebra
...
//* 获得先验信息矩阵， 怎么感觉除了第一帧没什么用
	inline Vec10 getPrior()//TODO 得到帧位姿的先验  第0帧，pose和ab都有先验，非0帧只有ab有先验，pose没有
	{
		Vec10 p =  Vec10::Zero();
...
struct PointHessian
{
	...
	inline void setIdepthZero(float idepth) {//TODO 调完setIdepthZero会立马调setIdepthScaled，所以idp的线性化点是一直在变的，等于没固定线性化点, idp的状态是在一直更新的，不是固定的
		idepth_zero = idepth;
