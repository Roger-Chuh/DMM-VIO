...
class IMUInitSettings {
 public:
  ...
  // If positive add a scale prior with this stddev after the initialization
  // (meant to be used with transitionModels 4 and 5 for the ablations).
  double scalePriorAfterInit = 0.0;

  // If false, the visual-inertial system is initialized immediately (with scale
  // 1). Then we still run the initializer for re-initialization though. A value
  // of false will not work well if the objects in the scene are far away. This
  // is mostly a legacy setting which might not work very well at the moment,
  // but it could be useful for future extensions.
  bool disableVIOUntilFirstInit = true;

  // Setting for debugging. Do IMU initialization in separate thread, even if we
  // are in non-realtime mode.
