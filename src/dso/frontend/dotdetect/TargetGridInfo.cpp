...
bool maxRectangle(const Eigen::MatrixXi &matrix, int minSize, Rectangle &result,
...
bool TargetGridInfo::matchRectangle(const Eigen::MatrixXi &smallMatrix,
                                    int &sdr, int &sdc, int &idg, int &dr,
                                    int &dc, int &plateId) {
  double minDiffValue = std::numeric_limits<double>::max();
  for (const std::pair<const int, std::array<Eigen::MatrixXi, 4>> &onePlate :
       Plate_PG_) {
    for (int i = 0; i < onePlate.second.size(); ++i) {
      for (int idx = 0; idx + smallMatrix.rows() <= onePlate.second[i].rows();
           ++idx) {
        for (int idy = 0; idy + smallMatrix.cols() <= onePlate.second[i].cols();
             ++idy) {
          const Eigen::MatrixXi &diffMatrix =
              onePlate.second[i].block(idx, idy, smallMatrix.rows(),
                                       smallMatrix.cols()) -
              smallMatrix;
...
#ifdef DEBUGDOT
  std::cerr << "minDiffValue:" << minDiffValue << " plate id:" << plateId
            << " idg: " << idg << std::endl;
  std::cerr << "smallMatrix:\n" << smallMatrix << std::endl;
  std::cerr << "diff mat:\n"
            << Plate_PG_.at(plateId)[idg].block(
                   dr + sdr, dc + sdc, smallMatrix.rows(), smallMatrix.cols()) -
                   smallMatrix
            << std::endl;
#endif
  ...
  // find global min
  if (minDiffValue <
      0.05 * (float)smallMatrix.cols() *
          (float)smallMatrix.rows()) { // 0.05: inlier point ratio
    if (minDiffValue < 0.5)
      return true;
    Rectangle temp{};
    const Eigen::MatrixXi diffMatrix =
        Plate_PG_.at(plateId)[idg].block(dr + sdr, dc + sdc, smallMatrix.rows(),
                                         smallMatrix.cols()) -
        smallMatrix;
...
bool TargetGridInfo::UniqueMatch(const Eigen::MatrixXi &m, int &idg, int &dr,
                                 int &dc, int &plateId) {
  int cols = (int)m.cols();
  int rows = (int)m.rows();
  if (cols < uniqueSize || rows < uniqueSize)
    return false;
  Rectangle maxRec{};
  if (maxRectangle(m, uniqueSize, maxRec, compareMat01())) {
    const Eigen::MatrixXi &smallMatrix =
        m.block(maxRec.top, maxRec.left, maxRec.height, maxRec.width);
    return matchRectangle(smallMatrix, maxRec.top, maxRec.left, idg, dr, dc,
                          plateId);
  }
#ifdef DEBUGDOT
...
bool TargetGridInfo::Match(
    std::map<
        Eigen::Vector2i const, Vertex *, lessCompare,
        Eigen::aligned_allocator<std::pair<Eigen::Vector2i const, Vertex *>>>
        &obs,
    int &plateId, int &Plate_PG_idx) {
  Eigen::Vector2i omin(std::numeric_limits<int>::max(),
                       std::numeric_limits<int>::max());
  Eigen::Vector2i omax(std::numeric_limits<int>::min(),
...
#ifdef DEBUGDOT
    std::cerr << "target_m: cols:" << m.cols() << " rows:" << m.rows()
              << std::endl;
    std::cerr << "target_m: \n" << m << std::endl;
#endif
    // Match methods
    ...
    if (res) {
      ...
      for (auto &ob : obs) {
        ...
        if (ob.second->value >= 0 && originPattern.y() >= 0 &&
            originPattern.y() < Plate_PG_.at(plateId)[Plate_PG_idx].rows() &&
            originPattern.x() >= 0 &&
            originPattern.x() < Plate_PG_.at(plateId)[Plate_PG_idx].cols() &&
            ob.second->pg(1) >= 0 && ob.second->pg(0) >= 0 &&
            ob.second->pg(1) < m.rows() && ob.second->pg(0) < m.cols() &&
            ...
            m(ob.second->pg(1), ob.second->pg(0)) ==
                Plate_PG_.at(plateId)[Plate_PG_idx](originPattern.y(),
                                                    originPattern.x())) {
