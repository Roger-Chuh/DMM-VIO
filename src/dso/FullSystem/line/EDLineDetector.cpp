...
int EDLineDetector::EDline(cv::Mat& image, LineChains& lines, bool smoothed) {
  ...
  lines.numOfLines = 0;
  if (edges.numOfEdges == 0) return -1;
    //	bValidate_ =false;
#ifdef DEBUGEDLine
...
double EDLineDetector::LeastSquaresLineFit_(unsigned int* xCors, unsigned int* yCors, unsigned int offsetS,
                                            std::array<double, 2>& lineEquation) {
...
bool EDLineDetector::LineValidation_(unsigned int* xCors, unsigned int* yCors, unsigned int offsetS,
                                     unsigned int offsetE, std::array<double, 3>& lineEquation, float& direction) {
  if (bValidate_) {
...
int EDLineDetector::EDline(cv::Mat& image, bool smoothed) {
  ...
  if (lines_.xCors.size() == 0 || lines_.yCors.size() == 0 || lines_.sId.size() == 0 || lines_.numOfLines == 0) {
    return -1;
  }
