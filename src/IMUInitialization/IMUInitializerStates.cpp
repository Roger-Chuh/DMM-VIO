...
dmvio::DefaultActiveIMUInitializerState::DefaultActiveIMUInitializerState(
...
dmvio::DefaultActiveIMUInitializerState::addPose(const dso::FrameShell &shell,
...
std::unique_ptr<IMUInitializerState>
dmvio::DefaultActiveIMUInitializerState::postBAInit(
  ...
  // This is actually not called for the very first KF which is what we want in
  // this case.
...
dmvio::DefaultActiveIMUInitializerState::initializeIfReady() {
  return std::make_pair(nullptr, false);
}

dmvio::CoarseIMUInitState::CoarseIMUInitState(
...
std::unique_ptr<IMUInitializerState>
dmvio::CoarseIMUInitState::addPose(const dso::FrameShell &shell,
                                   bool willBecomeKeyframe,
                                   const IMUData *imuData) {
  ...
  if (logic.coarseIMUOptimizer->numFrames > 5 && willBecomeKeyframe) {
    ...
    auto newState = transitionModel.coarseIMUInitOptimized(
        logic.coarseIMUOptimizer->optimizedValues, variances);
    if (newState) {
      // This means that the initialization was successful and the
      // CoarseIMUOptimizer can take over the values to the new optimization.
      logic.coarseIMUOptimizer->takeOverOptimizedValues();
      logic.pgba->removeIMUFactorsUntil =
          logic.coarseIMUOptimizer->imuFactorsRemovedUntil;
...
dmvio::RealtimeCoarseIMUInitState::RealtimeCoarseIMUInitState(
    dmvio::IMUInitializerLogic &imuInitLogic,
    dmvio::StateTransitionModel &transitionModel)
    : DefaultActiveIMUInitializerState(imuInitLogic, transitionModel) {}

std::unique_ptr<IMUInitializerState>
dmvio::RealtimeCoarseIMUInitState::addPose(const dso::FrameShell &shell,
                                           bool willBecomeKeyframe,
                                           const IMUData *imuData) {
  ...
  case NOT_RUNNING:
    DefaultActiveIMUInitializerState::addPose(shell, willBecomeKeyframe,
                                              imuData);
    if (logic.coarseIMUOptimizer->numFrames > 5 && willBecomeKeyframe &&
        !dso::setting_fullResetRequested) {
...
void dmvio::RealtimeCoarseIMUInitState::threadRun() {
  dmvio::TimeMeasurement timeMeasurement(
      "RealtimeCoarseIMUInitState::threadRun");
  ...
  if (newState) {
    // This means that the initialization was successful and the
    // CoarseIMUInitOptimizer can take over the values to the new optimization.
    logic.coarseIMUOptimizer->takeOverOptimizedValues();
    logic.pgba->removeIMUFactorsUntil =
        logic.coarseIMUOptimizer->imuFactorsRemovedUntil;
  }

  // Acquire lock to change the state. This ensures that the addPose method will
  // not be called for this state in the meantime, which would lead to IMU data
  // being lost.
  auto lock = logic.stateChanger.acquireSetStateLock();
  // Add cached imu data and poses.
  for (auto &&data : cachedData) {
    DefaultActiveIMUInitializerState::addPose(
...
dmvio::createCoarseIMUInitState(dmvio::IMUInitializerLogic &imuInitLogic,
                                dmvio::StateTransitionModel &transitionModel) {
  if (imuInitLogic.realtimeCoarseIMUInit) {
    return std::make_unique<RealtimeCoarseIMUInitState>(imuInitLogic,
                                                        transitionModel);
  } else {
...
std::unique_ptr<IMUInitializerState>
dmvio::createPGBAState(IMUInitializerLogic &imuInitLogic,
                       StateTransitionModel &transitionModel,
                       std::unique_ptr<gtsam::Values> &&initValues) {
  if (imuInitLogic.realtimePGBA) {
    return std::make_unique<RealtimePGBAState>(imuInitLogic, transitionModel,
                                               std::move(initValues));
  } else {
    return std::make_unique<PGBAState>(imuInitLogic, transitionModel,
                                       std::move(initValues));
  }
}

dmvio::PGBAState::PGBAState(dmvio::IMUInitializerLogic &imuInitLogic,
                            StateTransitionModel &transitionModel,
                            std::unique_ptr<gtsam::Values> &&initValuesPassed)
    : DefaultActiveIMUInitializerState(imuInitLogic, transitionModel),
      initValues(std::move(initValuesPassed)) {}

gtsam::Values *
prepareInitValuesForPGBA(std::unique_ptr<gtsam::Values> &initValuesNew,
  ...
  if (!initValues) {
    ...
    if (baValues.exists(gtsam::Symbol('s', 0))) {
      // Note: These delayedCurrValues also contain IMU variables (even though
      // the graph doesn't) (only if IMU is initialized in the main graph).
      initValuesNew = std::make_unique<gtsam::Values>(
          logic.pgba->getInputDelayedGraph()->getDelayedCurrValues());
...
std::unique_ptr<IMUInitializerState> dmvio::PGBAState::postBAInit(
    int keyframeId, gtsam::NonlinearFactor::shared_ptr activeHBFactor,
    const gtsam::Values &baValues, double timestamp,
    const gtsam::PreintegratedImuMeasurements &imuMeasurements) {
  DefaultActiveIMUInitializerState::postBAInit(
      keyframeId, activeHBFactor, baValues, timestamp, imuMeasurements);

  if (logic.pgba->getNumKFs() < 3) {
    return nullptr;
  }

  // Don't overwrite member variable to not influence the potential next
  // optimization.
  std::unique_ptr<gtsam::Values> initValuesNew;
  gtsam::Values *initValuesUsed =
      prepareInitValuesForPGBA(initValuesNew, initValues, baValues, logic);
  ...
  try {
    ...
    IMUInitVariances variances(
        logic.pgba->getMarginals(*optimizedValues),
        gtsam::Symbol('s', logic.transformDSOToIMUAfterPGBA->getSymbolInd()),
        logic.pgba->getBiasKey());

    // Write out result!
    auto &&bias = optimizedValues->at<gtsam::imuBias::ConstantBias>(
        logic.pgba->getBiasKey());
    if (!variances.indetermined) {
      std::cout << "PGBA: " << logic.transformDSOToIMUAfterPGBA->getScale()
                << " var: " << variances.scaleVariance << " "
    ...
    if (pair.first) {
      ...
      // If values are taken, and pair.second is not nullptr, over we are
      // responsible for creating the new state.
      auto newState = std::make_unique<InitializedFromPGBAState>(
          logic, transitionModel, std::move(optimizedValues));
      return std::move(newState);
    } else {
...
void dmvio::PGBAState::print(std::ostream &str) const { str << "PGBAState"; }

dmvio::RealtimePGBAState::RealtimePGBAState(
    dmvio::IMUInitializerLogic &imuInitLogic,
    StateTransitionModel &transitionModel,
    std::unique_ptr<gtsam::Values> &&initValuesPassed)
    : DefaultActiveIMUInitializerState(imuInitLogic, transitionModel),
...
std::unique_ptr<IMUInitializerState> dmvio::RealtimePGBAState::postBAInit(
    int keyframeId, gtsam::NonlinearFactor::shared_ptr activeHBFactor,
    const gtsam::Values &baValues, double timestamp,
    const gtsam::PreintegratedImuMeasurements &imuMeasurements) {
...
void RealtimePGBAState::threadRun() {
  ...
  if (!newStatePair.first) // Don't take over result (but maybe switch to new
                           // state provided by transition)
  {
    logic.pgba->optimizationResultNotUsed();

    // Lock and add new things.
    // Acquire lock to change the state. This ensures that the postBAInit method
    // will not be called for this state in the meantime, which would lead to
    // data being lost.
    auto lock = logic.stateChanger.acquireSetStateLock();
    ...
    if (!newStatePair.second)
      running = false; // do one or more optimization.
  ...
  {
    assert(!newStatePair
                .second); // other state created not yet supported for RT mode.
    ...
    auto newState = std::make_unique<InitializedFromRealtimePGBAState>(
        logic, transitionModel, std::move(optimizedValues),
        std::move(cachedData));
...
dmvio::InitializedFromPGBAState::InitializedFromPGBAState(
    dmvio::IMUInitializerLogic &imuInitLogic,
    StateTransitionModel &transitionModel,
...
std::pair<std::unique_ptr<IMUInitializerState>, bool>
dmvio::InitializedFromPGBAState::initializeIfReady() {
  auto delayedGraph =
      logic.pgba->prepareGraphForMainOptimization(*optimizedValues);

  logic.callOnInit(delayedGraph->getDelayedCurrValues(), true);
  delayedGraph->setMaxGroupInGraph(
      BAIMULogic::METRIC_GROUP); // From now on all factors should be added to
                                 // this.
...
dmvio::InitializedFromRealtimePGBAState::InitializedFromRealtimePGBAState(
    dmvio::IMUInitializerLogic &imuInitLogic,
    StateTransitionModel &transitionModel,
    std::unique_ptr<gtsam::Values> &&optimizedValuesPassed,
...
std::unique_ptr<IMUInitializerState>
InitializedFromRealtimePGBAState::postBAInit(
    int keyframeId, gtsam::NonlinearFactor::shared_ptr activeHBFactor,
    const gtsam::Values &baValues, double timestamp,
    const gtsam::PreintegratedImuMeasurements &imuMeasurements) {
  ...
  optimizedValues = std::make_unique<gtsam::Values>(logic.pgba->extendGraph(
      activeHBFactor, std::move(*optimizedValues), baValues, cachedData));
  cachedData.clear();
  prepared = true;
  return nullptr;
}
...
std::pair<std::unique_ptr<IMUInitializerState>, bool>
dmvio::InitializedFromRealtimePGBAState::initializeIfReady() {
  if (!prepared)
    return std::make_pair(nullptr,
                          false); // postBAInit was not run yet, which means
                                  // that the new factors have not been added.
  dmvio::TimeMeasurement meas(
      "InitializedFromRealtimePGBAState::initializeIfReady");
  auto delayedGraph =
      logic.pgba->prepareGraphForMainOptimization(*optimizedValues);
...
dmvio::NoMarginalizationReplacementInitializedFromPGBAState::
...
std::pair<std::unique_ptr<IMUInitializerState>, bool> dmvio::
    NoMarginalizationReplacementInitializedFromPGBAState::initializeIfReady() {
  // If this ablation is done in RT mode (which it was not in the paper)
  // building of the graph should be removed as well.
  auto delayedGraph =
      logic.pgba->prepareGraphForMainOptimization(*optimizedValues);
...
void dmvio::NoMarginalizationReplacementInitializedFromPGBAState::print(
    std::ostream &str) const {
  str << "NoMarginalizationReplacementInitializedFromPGBAState";
}

dmvio::InitializedFromCoarseIMUInitState::InitializedFromCoarseIMUInitState(
    dmvio::IMUInitializerLogic &imuInitLogic,
    StateTransitionModel &transitionModel)
    : DefaultActiveIMUInitializerState(imuInitLogic, transitionModel) {}

std::pair<std::unique_ptr<IMUInitializerState>, bool>
dmvio::InitializedFromCoarseIMUInitState::initializeIfReady() {
...
dmvio::PotentialMarginalizationReplacementState::
    PotentialMarginalizationReplacementState(
        dmvio::IMUInitializerLogic &imuInitLogic,
        dmvio::StateTransitionModel &transitionModel, int startIdForSecondTh)
    : DefaultActiveIMUInitializerState(imuInitLogic, transitionModel),
      startIdForSecondTH(startIdForSecondTh) {}

std::unique_ptr<IMUInitializerState>
dmvio::PotentialMarginalizationReplacementState::postBAInit(
    int keyframeId, gtsam::NonlinearFactor::shared_ptr activeHBFactor,
    const gtsam::Values &baValues, double timestamp,
    const gtsam::PreintegratedImuMeasurements &imuMeasurements) {
  DefaultActiveIMUInitializerState::postBAInit(
  ...
  auto *fejVals =
      logic.delayedMarginalizationGraphs->getMainGraph()->fejValues.get();
  if (!fejVals)
    return nullptr;
  bool changed = thresholdVariableChanges(
      useSecondTH ? logic.settings.secondThresholdSettings
                  : logic.settings.thresholdSettings,
      baValues, fejVals->fejValues);
  ...
  logic.pgba->prepareOptimization();
  // We pass noOptimization=true, so that the graph is only built, but not
  // actually optimized.
  ...
  if (!useSecondTH && firstId >= startIdForSecondTH) {
    ...
    bool reallyChanged = thresholdVariableChanges(
        logic.settings.secondThresholdSettings, baValues, fejVals->fejValues);
    if (!reallyChanged) {
      std::cout << "IMUInitialization (switch): Found out that second "
...
void dmvio::PotentialMarginalizationReplacementState::print(
    std::ostream &str) const {
  str << "PotentialMarginalizationReplacementState";
}

dmvio::MarginalizationReplacementReadyState::
...
std::pair<std::unique_ptr<IMUInitializerState>, bool>
dmvio::MarginalizationReplacementReadyState::initializeIfReady() {
...
void dmvio::MarginalizationReplacementReadyState::print(
    std::ostream &str) const {
