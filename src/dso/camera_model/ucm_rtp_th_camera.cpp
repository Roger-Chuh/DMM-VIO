...
bool UCMRTPTHCamera::Project(const Vec3& p_3d, Vec2& p_img, Eigen::Matrix<number_t, 2, 3>* d_img_d_p3d,
  ...
  Distortion(x_undistort, x_distort, (d_img_d_p3d == nullptr && d_img_d_param == nullptr ? nullptr : &d_xd_xu),
  ...
  if (d_img_d_p3d || d_img_d_param) {
    ...
    if (d_img_d_param) {
      ...
      Vec2 d_xu_alpha = Vec2(p_norm[0] * (p_norm[2] - 1), p_norm[1] * (p_norm[2] - 1)) * fi2;
      (*d_img_d_param).block<2, 1>(0, 4) = d_img_xd * d_xd_xu * d_xu_alpha * d_alpha_ralpha;
    }
  }
  return true;
}

bool UCMRTPTHCamera::Project(const Vec3& p_3d, Eigen::Ref<Vec2>& p_img, Eigen::Matrix<number_t, 2, 3>* d_img_d_p3d,
                             Eigen::Matrix<number_t, 2, Eigen::Dynamic>* d_img_d_param) const {
  const number_t& fx = parameters_[0];
  const number_t& fy = parameters_[1];
  const number_t& cx = parameters_[2];
...
bool UCMRTPTHCamera::UnProject(const Vec2& p_img, Vec3& p_3d, Eigen::Matrix<number_t, 3, 2>* d_p3d_d_img,
  ...
  UnDistortion(x_distort, x_undistort, (d_p3d_d_img == nullptr && d_p3d_d_param == nullptr ? nullptr : &d_xu_xd),
  ...
  if (d_p3d_d_img || d_p3d_d_param) {
    ...
    Mat32 d_p3d_xu = d_xu_p3d.transpose() * (d_xu_p3d * d_xu_p3d.transpose()).inverse();
    ...
    if (d_p3d_d_param) {
      ...
      d_xd_fc << -x_distort[0] / fx, 0, -1.0 / fx, 0, 0, -x_distort[1] / fy, 0, -1.0 / fy;

      (*d_p3d_d_param).block<3, 4>(0, 0) = d_p3d_xu * d_xu_xd * d_xd_fc;

      const number_t fi = 1.0 / (real_alpha + (1.0 - real_alpha) * p_3d[2]);
      const number_t fi2 = fi * fi;
      Vec2 d_xu_alpha = Vec2(p_3d[0] * (p_3d[2] - 1), p_3d[1] * (p_3d[2] - 1)) * fi2;

      number_t e_alpha = std::exp(-m_alpha_);
      number_t d_alpha_ralpha = e_alpha / (1 + e_alpha) / (1 + e_alpha);

      (*d_p3d_d_param).block<3, 1>(0, 4) = -d_p3d_xu * d_xu_alpha * d_alpha_ralpha;
...
void UCMRTPTHCamera::Distortion(const Vec2& xy, Vec2& uv, Mat2* d_uv_xy,
  ...
  const number_t th_divr = (r < std::numeric_limits<number_t>::epsilon()) ? 1.0 : th / r;
...
void UCMRTPTHCamera::UnDistortion(const Vec2& uv, Vec2& xy, Mat2* d_xy_uv,
  ...
  if (xr_yrNorm == 0) {
    ...
    for (; j < kMaxIterNum; ++j) {
      ...
      if (std::abs(dthD_dth) > std::numeric_limits<number_t>::epsilon()) {
        ...
        step = (xr_yrNorm - th_radial) * dthD_dth > 0.0 ? 10.0 * std::numeric_limits<number_t>::epsilon()
...
void UCMRTPTHCamera::ComputeDuvDxryr(const Vec2& xr_yr, const number_t& xr_yr_squaredNorm, Mat2& d_uv_xryr) const {
  const Vec2 p_vec = Eigen::Map<const Vec2>(parameters_ + p_start);
  const Vec4 s_vec = Eigen::Map<const Vec4>(parameters_ + s_start);
...
void UCMRTPTHCamera::ComputeDxuDpnorm(const Vec3& p_norm, Mat23& d_xu_pnorm) const {
  const number_t& m_alpha_ = parameters_[4];
  const number_t& real_alpha = 1.0 / (1.0 + exp(-m_alpha_));
  ...
  d_xu_pnorm << xi, 0, -p_norm[0] * (1.0 - real_alpha) * xi2, 0, xi, -p_norm[1] * (1.0 - real_alpha) * xi2;
}