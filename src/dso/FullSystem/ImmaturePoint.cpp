...
//TODO 这里是三角化了，跟LSD-SLAM一样, use the same formular in LSD-SLAM to update new inverse depth search interval
	// ============== set new interval ===================
		...
		float idepth)
{
	...
	for(int idx=0;idx<patternNum;idx++)
	{
		...
		Vec3f KliP;
/// kidding me ? new_idepth was never used, not to mention it's not even the ACTUAL new_idepth in target frame
...
/// dIl传进来只是为了得到数据的地址,用以指向角点4邻域内的数据，它的内容并不参与计算，指针妙用
        ...
        /// pattern of 8, so there should be 8 residuals contributing to 1 photometric factor
		float residual = hitColor[0] - (affLL[0] * color[idx] + affLL[1]);
