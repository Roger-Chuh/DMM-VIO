...
DelayedMarginalizationGraphs::DelayedMarginalizationGraphs(
    int mainGraphDelay, int maxGroupInMainGraph) {
  addMainGraph(
      std::make_shared<DelayedGraph>(mainGraphDelay, maxGroupInMainGraph));
}

std::shared_ptr<DelayedGraph>
dmvio::DelayedMarginalizationGraphs::addDelayedGraph(int delayN,
                                                     int maxGroupInGraph) {
  delayedGraphs.emplace_back(new DelayedGraph(delayN, maxGroupInGraph));
  return delayedGraphs.back();
}

void DelayedMarginalizationGraphs::addDelayedGraph(
    std::shared_ptr<DelayedGraph> graph) {
  delayedGraphs.emplace_back(std::move(graph));
}

void dmvio::DelayedMarginalizationGraphs::addFactor(
    gtsam::NonlinearFactor::shared_ptr factor, int group) {
...
dmvio::DelayedMarginalizationGraphs::getHAndB(
    const gtsam::Values &values, const gtsam::Ordering &ordering,
  ...
  // Compute the Hessian and gradient vector. We use the AugmentedScatter which
  // also works for keys which don't exist in the graph.
...
void dmvio::DelayedMarginalizationGraphs::marginalizeFrame(
    const gtsam::FastVector<gtsam::Key> &keysToMarginalize,
    gtsam::Values::shared_ptr values, std::map<gtsam::Key, size_t> &keyDimMap,
    double currBATimestamp, gtsam::Values::shared_ptr currValues) {
  dmvio::TimeMeasurement meas("DelayedMarginalization");

  // First only marginalize the main graph so that we have separate time
  // measurements.
  dmvio::TimeMeasurement mainMeas(
      "MarginalizeMainGraph"); // This saves only the time necessary for
                               // marginalizing the main graph.
...
double
dmvio::DelayedMarginalizationGraphs::getError(const gtsam::Values &values) {
  return getMainGraph()->getGraph()->error(values);
}

void DelayedMarginalizationGraphs::addMainGraph(
    std::shared_ptr<DelayedGraph> delayedGraph) {
...
void DelayedMarginalizationGraphs::replaceMainGraph(
    std::shared_ptr<DelayedGraph> delayedGraph) {
...
void DelayedMarginalizationGraphs::removeDelayedGraph(
    const DelayedGraph *graph) {
...
void DelayedMarginalizationGraphs::updateEvalValues(
    const gtsam::Values &evalValues) {
  for (auto &&graph : delayedGraphs) {
...
DelayedMarginalizationGraphs::addDisconnectedGraph(int maxGroupInGraph) {
  disconnectedGraphs.emplace_back(
      new DisconnectedDelayedGraph(maxGroupInGraph));
  return disconnectedGraphs.back();
}

void DelayedMarginalizationGraphs::removeDisconnectedGraph(
    const DisconnectedDelayedGraph *graph) {
  auto it = std::find_if(
      disconnectedGraphs.begin(), disconnectedGraphs.end(),
...
dmvio::DelayedGraph::DelayedGraph(int delayN, int maxGroupInGraph)
    : delayN(delayN), maxGroupInGraph(maxGroupInGraph) {
  graph.reset(new gtsam::NonlinearFactorGraph());
  fejValues.reset(new FEJValues);
}

void dmvio::DelayedGraph::addFactor(gtsam::NonlinearFactor::shared_ptr factor,
                                    int group) {
...
void dmvio::DelayedGraph::marginalize(
    const gtsam::FastVector<gtsam::Key> &keysToMarginalize,
...
void dmvio::DelayedGraph::readvanceUntilDelay() {
  ...
  while (marginalizationOrder.size() > delayN) {
    ...
    // Insert values for keysToMarginalize, even though they will be removed
    // below, but they are necessary so that all fejValues contains all values
    // needed for the linearization below.
    fejValues->insertConnectedKeys(keysToMarg, delayedValues);

    // The connected keys have to be inserted into the fejMap before the
    // linearization inside marginalizeOut.
    auto connectedKeyCallback =
        [this](const gtsam::FastSet<gtsam::Key> &connectedKeys) {
          // Get connected keys and insert fejMap.
          // delayedValues should be baEvalValues, i.e. evaluation point values
          // for poses and affine, and the current values for everything else.
          fejValues->insertConnectedKeys(connectedKeys, delayedValues);
        };

    if (!keysToMarg.empty()) {
      // Note that the connectedKeyCallback fills fejValues.fejValues with the
      // needed values before the linearization happens.
...
void DelayedGraph::readvanceGraph(int newDelay) {
  ...
  if (marginalizationPaused) {
    std::cout
        << "WARNING: Trying to readvance graph while marginalization is paused!"
        << std::endl;
  }
  readvanceUntilDelay();
}

int DelayedGraph::getDelayN() const { return delayN; }
...
int DelayedGraph::getMaxGroupInGraph() const { return maxGroupInGraph; }

const std::deque<gtsam::FastVector<gtsam::Key>> &
DelayedGraph::getMarginalizationOrder() const {
  return marginalizationOrder;
}

DelayedGraph::DelayedGraph(
    int delayN, int maxGroupInGraph,
    ...
    : delayN(delayN), maxGroupInGraph(maxGroupInGraph), graph(graph),
      marginalizationOrder(std::move(marginalizationOrder)),
      delayedValues(std::move(delayedValues)),
...
DelayedGraph::DelayedGraph(const DelayedGraph &other)
    : delayN(other.delayN), maxGroupInGraph(other.maxGroupInGraph),
      graph(boost::make_shared<gtsam::NonlinearFactorGraph>(
          other.graph->clone())),
      marginalizationOrder(other.marginalizationOrder),
      delayedValues(other.delayedValues),
      delayedCurrValues(other.delayedCurrValues),
      fejValues(new FEJValues(*(other.fejValues))) {}
...
void DelayedGraph::setDelayN(int delayN) { DelayedGraph::delayN = delayN; }

void DelayedGraph::setFEJValuesForFactors(bool useFEJ) {
  setFEJMapForGraph(*graph, useFEJ ? fejValues : nullptr);
}

DisconnectedDelayedGraph::DisconnectedDelayedGraph(int maxGroupInGraph)
    : maxGroupInGraph(maxGroupInGraph) {}

void DisconnectedDelayedGraph::addFactor(
    gtsam::NonlinearFactor::shared_ptr factor, int group) {
  if (group <= maxGroupInGraph) {
