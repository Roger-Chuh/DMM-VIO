...
class KB20Camera : public CameraBase {
 public:
  ...
  bool opt_eucm = true;
  bool opt_rot = true;
  explicit KB20Camera(CamId camera_id, int width, int height, bool opt_s5s6t1t2_ = true, bool extra_param_ = false)
  ...
  KB20Camera(CamId camera_id, int width, int height, const number_t* parameters, bool opt_s5s6t1t2_ = true,
             bool extra_param_ = false)
  ...
  virtual void SetParamSize() override {
    ...
    printf(
        "param_size: %d, k_start_idx: %d, p0_start_idx: %d, s_start_idx: "
        ...
        kParamLength, k_start_idx, p0_start_idx, s_start_idx, tilt_start_idx, eucm_start_idx, rot_start_idx,
        ofst0_start_idx, ofst1_start_idx, ofst2_start_idx, extra_p_start_idx, p1_start_idx, p2_start_idx);
  }

  virtual bool Project(const Vec3& p_3d22, Vec2& p_img, Eigen::Matrix<number_t, 2, 3>* d_img_d_p3d = nullptr,
                       Eigen::Matrix<number_t, 2, Eigen::Dynamic>* d_img_d_param = nullptr) const override;

  virtual bool Project(const Vec3& p_3d22, Eigen::Ref<Vec2>& p_img,
                       Eigen::Matrix<number_t, 2, 3>* d_img_d_p3d = nullptr,
                       Eigen::Matrix<number_t, 2, Eigen::Dynamic>* d_img_d_param = nullptr) const override;

  virtual bool UnProject(const Vec2& p_img, Vec3& p_3d2, Eigen::Matrix<number_t, 3, 2>* d_p3d2_d_img = nullptr,
                         Eigen::Matrix<number_t, 3, Eigen::Dynamic>* d_p3d2_d_param = nullptr) const override;

 private:
  void compute_duvDistorted_dxryr(const Vec2& xr_yr, Mat2& duvDistorted_dxryr, Mat23& duvDistorted_dq,
                                  const number_t& xr_yr_squaredNorm, const number_t& p1, const number_t& p2,
                                  const number_t& s1, const number_t& s2, const number_t& s3, const number_t& s4,
                                  const number_t& s5, const number_t& s6, const number_t& q1, const number_t& q2,
                                  const number_t& q3) const {
    number_t p_coeff_x = (2 * xr_yr(0) * xr_yr(0) + xr_yr_squaredNorm) * p1 + 2.0 * xr_yr(0) * xr_yr(1) * p2;
    number_t p_coeff_y = (2 * xr_yr(1) * xr_yr(1) + xr_yr_squaredNorm) * p2 + 2.0 * xr_yr(0) * xr_yr(1) * p1;

    number_t q_coeff = 1 + q1 * xr_yr_squaredNorm + q2 * xr_yr_squaredNorm * xr_yr_squaredNorm +
                       q3 * xr_yr_squaredNorm * xr_yr_squaredNorm * xr_yr_squaredNorm;

    number_t d_q_coeff_d_x_r =
        (2.0 * q1 + 4.0 * q2 * xr_yr_squaredNorm + 6.0 * q3 * xr_yr_squaredNorm * xr_yr_squaredNorm) * xr_yr(0);
    number_t d_q_coeff_d_y_r =
        (2.0 * q1 + 4.0 * q2 * xr_yr_squaredNorm + 6.0 * q3 * xr_yr_squaredNorm * xr_yr_squaredNorm) * xr_yr(1);

    duvDistorted_dxryr(0, 0) =
        (1 + (6.0 * xr_yr(0) * p1 + 2.0 * xr_yr(1) * p2) * (q_coeff) + p_coeff_x * (d_q_coeff_d_x_r)) +
        2.0 * (s1 + 2.0 * s2 * xr_yr_squaredNorm + 3.0 * s5 * xr_yr_squaredNorm * xr_yr_squaredNorm) * xr_yr(0);
    duvDistorted_dxryr(0, 1) =
        ((2.0 * xr_yr(1) * p1 + 2.0 * xr_yr(0) * p2) * (q_coeff) + p_coeff_x * (d_q_coeff_d_y_r)) +
        2.0 * (s1 + 2.0 * s2 * xr_yr_squaredNorm + 3.0 * s5 * xr_yr_squaredNorm * xr_yr_squaredNorm) * xr_yr(1);
    duvDistorted_dxryr(1, 0) =
        ((2.0 * xr_yr(1) * p1 + 2.0 * xr_yr(0) * p2) * (q_coeff) + p_coeff_y * (d_q_coeff_d_x_r)) +
        2.0 * (s3 + 2.0 * s4 * xr_yr_squaredNorm + 3.0 * s6 * xr_yr_squaredNorm * xr_yr_squaredNorm) * xr_yr(0);
    duvDistorted_dxryr(1, 1) =
        ((1 + 6.0 * xr_yr(1) * p2 + 2. * xr_yr(0) * p1) * (q_coeff) + p_coeff_y * (d_q_coeff_d_y_r)) +
        2.0 * (s3 + 2.0 * s4 * xr_yr_squaredNorm + 3.0 * s6 * xr_yr_squaredNorm * xr_yr_squaredNorm) * xr_yr(1);

    duvDistorted_dq.setZero();
    duvDistorted_dq.row(0) << xr_yr_squaredNorm, xr_yr_squaredNorm * xr_yr_squaredNorm,
  ...
  void compute_d_uv_w_p_d_xy_wo_p(const Vec3& xy_wo_p, Vec3& uv_w_p, Mat3& d_uv_w_p_d_xy_wo_p, Mat32& d_uv_w_p_d_p,
                                  const number_t& p1, const number_t& p2) const {
    ...
    d_uv_w_p_d_xy_wo_p(0, 0) = 1.0 + 6.0 * xy_wo_p(0) * p1 + 2.0 * xy_wo_p(1) * p2;
    d_uv_w_p_d_xy_wo_p(0, 1) = 2.0 * xy_wo_p(1) * p1 + 2.0 * xy_wo_p(0) * p2;
    d_uv_w_p_d_xy_wo_p(1, 0) = 2.0 * xy_wo_p(1) * p1 + 2.0 * xy_wo_p(0) * p2;
    d_uv_w_p_d_xy_wo_p(1, 1) = 1.0 + 6.0 * xy_wo_p(1) * p2 + 2. * xy_wo_p(0) * p1;
  ...
  bool compute_xy_wo_p_from_uv_w_p(const Vec3& uv_w_p, Vec3& xy_wo_p, Mat3& d_uv_w_p_d_xy_wo_p, Mat32& d_uv_w_p_d_p,
                                   const number_t& p1, const number_t& p2) const {
    xy_wo_p = uv_w_p;
    ...
    for (int i = 0; i < max_iter; i++) {
      // p3d1_est = p3d2;
      compute_d_uv_w_p_d_xy_wo_p(xy_wo_p, uv_w_p_est, d_uv_w_p_d_xy_wo_p, d_uv_w_p_d_p, p1, p2);
      correction.head(2) = d_uv_w_p_d_xy_wo_p.topLeftCorner<2, 2>().inverse() * (uv_w_p.head(2) - uv_w_p_est.head(2));
      xy_wo_p += correction;
  ...
  void computeTiltProjectionMatrix(const number_t& tauX, const number_t& tauY, Mat3& matTilt, Mat3& invMatTilt) const {
    matTilt.setIdentity();
    ...
    matRotX << number_t(1.0), number_t(0.0), number_t(0.0), number_t(0.0), cTauX, sTauX, number_t(0.0), -sTauX, cTauX;
    matRotY << cTauY, number_t(0.0), -sTauY, number_t(0.0), number_t(1.0), number_t(0.0), sTauY, number_t(0.0), cTauY;
    Mat3 matRotXY = matRotY * matRotX;
    matProjZ << matRotXY(2, 2), number_t(0.0), -matRotXY(0, 2), number_t(0.0), matRotXY(2, 2), -matRotXY(1, 2),
        number_t(0.0), number_t(0.0), number_t(1.0);

    matTilt = matProjZ * matRotXY;
    invMatTilt << number_t(1.0) / cTauX, number_t(0.0), number_t(0.0), tTauX * tTauY, number_t(1.0) / cTauY,
        number_t(0.0), -tTauY, tTauX / cTauY, number_t(1.0) / (cTauX * cTauY);
  }

  bool compute_xr_yr_from_uvDistorted(const Vec2& uvDistorted, Vec2& xr_yr, Mat2& duvDistorted_dxryr,
    ...
    for (int i = 0; i < max_iter; i++) {
      ...
      compute_duvDistorted_dxryr(xr_yr, duvDistorted_dxryr, duvDistorted_dq, xr_yr_squaredNorm, p1, p2, s1, s2, s3, s4,
                                 s5, s6, q1, q2, q3);
      correction = duvDistorted_dxryr.inverse() * (uvDistorted - uvDistorted_est);
  ...
  bool getThetaFromNorm_xr_yr(const number_t& th_radialDesired, number_t& th, number_t& dthD_dth, const number_t& k1,
                              const number_t& k2, const number_t& k3, const number_t& k4, const number_t& k5,
    ...
    for (int i = 0; i < max_iter; i++) {
      ...
      for (int j = 0; j < k.size(); j++) {
        ...
        dthD_dth = dthD_dth + (2.0 * number_t(j) + 3.0) * k[startK + j] * theta2is;
  ...
  bool compute_p3d2_from_p3d1(const Vec3& p3d1, Vec3& p3d2, Mat3& d_p3d1_d_p3d2, const number_t& alpha,
                              const number_t& beta, const Vec2& p_img) const {
    p3d2 = p3d1;
      ...
      if (i == max_iter - 1 && extra_param) {
        ...
        if (!bad_param) {
          ...
          if (correction.hasNaN()) {
            ...
            std::cout << "p3d1: " << p3d1.transpose() << ", alpha: " << alpha << ", beta: " << beta << std::endl;
            PrintIntri();
  ...
  Mat3 ComputeBearingJac(const Vec3& transedXYZ_c) const {
    ...
    // 归一化向量对原向量的导数
    Mat3 d_errornorm_d_xyz = Mat3::Identity() / ptNorm - ray_normalize * ray_normalize.transpose() / ptNorm;

    return d_errornorm_d_xyz;
  }
  number_t SolveTheta(const number_t& r_theta, number_t& d_func_d_theta, const number_t& k1, const number_t& k2,
  ...
  void ComputeForwardTiltJac(const number_t& t1, const number_t& t2, const Vec3& uvDistorted,
                             Mat3& d_uvTilted_d_uvDistorted_full, Mat32& d_uvTilted_d_t1t2_full) const {
    /// part1
    number_t stx = sin(t1);
    number_t ctx = cos(t1);
    number_t sty = sin(t2);
    ...
    d_uvTilted_d_uvDistorted_full.topLeftCorner<2, 2>() = d_uvTilted_d_uvDistorted;
  ...
  void ComputeBackwardTiltJac(const number_t& t1, const number_t& t2, const Vec3& uvTilted,
                              Mat3& d_uvDistorted_d_uvTilted_full, Mat32& d_uvDistorted_d_t1t2_full) const {
    /// part1
    number_t stx = sin(t1);
    number_t ctx = cos(t1);
    number_t sty = sin(t2);
    ...
    d_uvDistorted_d_uvTilted_full.topLeftCorner<2, 2>() = d_uvDistorted_d_uvTilted;
