...
EdgeDrawingNFALUT::EdgeDrawingNFALUT(int size, double _prob, int _w, int _h) {
...
EdgeDrawingNFALUT::~EdgeDrawingNFALUT() { delete[] LUT; }

bool EdgeDrawingNFALUT::checkValidationByNFA(int n, int k) {
  if (n >= LUTSize)
    return nfa(n, k) <= 1.0;
  else
    return k >= LUT[n];
...
struct mCircle {
  Point2d center;
  double r;
  mCircle(Point2d _center, double _r) {
    center = _center;
    r = _r;
  }
};
...
struct mEllipse {
  ...
  Size axes;
  double theta;
  mEllipse(Point2d _center, Size _axes, double _theta) {
    center = _center;
    axes = _axes;
    theta = _theta;
  }
...
// A fast implementation of the EdgeDrawingAngleSet class. The slow
// implementation is really bad. About 10 times slower than this!
...
void EdgeDrawingAngleSet::_set(double sTheta, double eTheta) {
  int arc = next++;
...
void EdgeDrawingAngleSet::set(double sTheta, double eTheta) {
  if (eTheta > sTheta) {
    _set(sTheta, eTheta);
  } else {
    _set(sTheta, CV_2PI);
...
double EdgeDrawingAngleSet::_overlap(double sTheta, double eTheta) {
  double o = 0;
...
double EdgeDrawingAngleSet::overlap(double sTheta, double eTheta) {
  double o;
...
double EdgeDrawingAngleSet::coverRatio() {
  int current = head;
