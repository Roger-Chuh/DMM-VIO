...
bool ConfigData::LoadConfig(const std::string& config_path) {
  try {
    ...
    calib_stage = static_cast<CalibStage>(toml::find<int>(main_config, "calib_stage"));
    ...
    if (calib_stage == GRAY_VI) {
      if (!rgb_cid.empty() || gray_cid.empty() || gray_cid.size() != calib_cid.size()) {
    ...
    self_calib_bin_file_points = toml::find<std::string>(dataset_config, "self_calib_bin_file_points");
    self_calib_bin_file_lines = toml::find<std::string>(dataset_config, "self_calib_bin_file_lines");
    skip_detection = toml::find<bool>(dataset_config, "skip_detection");
    use_prior_marker_map = toml::find<bool>(dataset_config, "use_prior_marker_map");
    prior_marker_map_path = toml::find<std::string>(dataset_config, "prior_marker_map_path");
    marker_map_vm_bin_file = toml::find<std::string>(dataset_config, "marker_map_vm_bin_file");
    cam_folder_prefix = toml::find<std::string>(dataset_config, "cam_folder_prefix");
    cam_folder_subfix = toml::find<std::string>(dataset_config, "cam_folder_subfix");
    cam_json = toml::find<std::string>(dataset_config, "cam_json");
    board_type = static_cast<BoardType>(toml::find<int>(dataset_config, "board_type"));
    plate_num = toml::find<int>(dataset_config, "plate_num");
    detect_thread_num = toml::find<int>(dataset_config, "detect_thread_num");

    // imu config
    imu_folder_name = toml::find<std::string>(dataset_config, "imu_folder_name");
    ...
    if (board_type == DOT) {
      ...
      dot_config.dot_bin_file = toml::find<std::string>(dot_plate_config, "dot_bin_file");
      dot_config.dot_bin_file_gray = toml::find<std::string>(dot_plate_config, "dot_bin_file_gray");
      dot_config.dot_bin_file_rgb = toml::find<std::string>(dot_plate_config, "dot_bin_file_rgb");
      dot_config.show_detect_picture = toml::find<bool>(dot_plate_config, "show_detect_picture");
      dot_config.black_dot = toml::find<bool>(dot_plate_config, "black_dot");
      dot_config.adaptive_thresh = toml::find<double>(dot_plate_config, "adaptive_thresh");
      dot_config.window_ratio = toml::find<int>(dot_plate_config, "window_ratio");
      dot_config.grid_spacing = toml::find<double>(dot_plate_config, "grid_spacing");
      ...
      dot_config.grid_size = Eigen::Vector2i(grid_value[0].as_integer(), grid_value[1].as_integer());
      ...
      dot_config.conic_min_area = toml::find<double>(dot_plate_config, "conic_min_area");
      dot_config.conic_symmetry = toml::find<double>(dot_plate_config, "conic_symmetry");
      dot_config.conic_min_aspect = toml::find<double>(dot_plate_config, "conic_min_aspect");
      dot_config.conic_min_aspect = toml::find<double>(dot_plate_config, "conic_min_aspect");
      dot_config.min_area_point_num = toml::find<int>(dot_plate_config, "min_area_point_num");
    } else if (board_type == APRILTAG) {
      std::cout << "use aprilTag plate\n" << std::endl;
      const toml::value& apriltag_plate_config = toml::find(data, "apriltag_plate");
      apriltag_config.show_tag_detect = toml::find<bool>(apriltag_plate_config, "show_tag_detect");
      apriltag_config.apriltag_bin_file = toml::find<std::string>(apriltag_plate_config, "apriltag_bin_file");
      apriltag_config.apriltag_bin_file_gray = toml::find<std::string>(apriltag_plate_config, "apriltag_bin_file_gray");
      apriltag_config.apriltag_bin_file_rgb = toml::find<std::string>(apriltag_plate_config, "apriltag_bin_file_rgb");
      apriltag_config.black_board = toml::find<int>(apriltag_plate_config, "black_board");
      ...
      apriltag_config.apriltag_start_id = toml::find<int>(apriltag_plate_config, "apriltag_start_id");
      apriltag_config.tag_size = toml::find<double>(apriltag_plate_config, "tag_size");
      apriltag_config.tag_gap = toml::find<double>(apriltag_plate_config, "tag_gap");
      apriltag_config.pattern_num = toml::find<int>(apriltag_plate_config, "pattern_num");
      apriltag_config.one_board_tags = apriltag_config.rows * apriltag_config.cols;
    } else {
    ...
    auto calib_check_cid_value_list = toml::find(calib_check_config, "calib_check_cid").as_array();
    for (auto cid_value : calib_check_cid_value_list) {
      calib_check_cid.emplace_back(cid_value.as_integer());
    }
    calib_file_name = toml::find<std::string>(calib_check_config, "calib_file_name");
    calib_check_result_folder = toml::find<std::string>(calib_check_config, "calib_check_result_folder");
    show_calib_check_img = toml::find<bool>(calib_check_config, "show_calib_check_img");
    ...
    vi_config.shift_imu_data = toml::find<bool>(vi_calib_config, "shift_imu_data");
    vi_config.skip_first_frames_num = toml::find<int>(vi_calib_config, "skip_first_frames_num");
    vi_config.high_weight_knot_ratio = toml::find<double>(vi_calib_config, "high_weight_knot_ratio");
    ...
    vi_config.disturb_level = toml::find<double>(vi_calib_config, "disturb_level");
    vi_config.fix_extr = toml::find<bool>(vi_calib_config, "fix_extr");
    vi_config.use_intr_rot_factor = toml::find<bool>(vi_calib_config, "use_intr_rot_factor");
    vi_config.intr_rot_factor_weight = toml::find<double>(vi_calib_config, "intr_rot_factor_weight");
    vi_config.resolution_difference_weight = toml::find<double>(vi_calib_config, "resolution_difference_weight");
    vi_config.res_amplify_ratio = toml::find<double>(vi_calib_config, "res_amplify_ratio");
    vi_config.disable_reproj_factor = toml::find<bool>(vi_calib_config, "disable_reproj_factor");
    vi_config.use_magic_number = toml::find<bool>(vi_calib_config, "use_magic_number");
    ...
    vi_config.gyro_std_high = toml::find<double>(vi_calib_config, "gyro_std_high");
    vi_config.acc_std = toml::find<double>(vi_calib_config, "acc_std");
    vi_config.acc_std2 = toml::find<double>(vi_calib_config, "acc_std2");
    vi_config.acc_std_high = toml::find<double>(vi_calib_config, "acc_std_high");
    ...
    vi_config.opt_rolling_shutter = toml::find<bool>(vi_calib_config, "opt_rolling_shutter");
    vi_config.opt_visual_scale = toml::find<bool>(vi_calib_config, "opt_visual_scale");
    vi_config.fix_imu_intr = toml::find<bool>(vi_calib_config, "fix_imu_intr");
    vi_config.opt_camera_intr = toml::find<bool>(vi_calib_config, "opt_camera_intr");
    vi_config.disable_imu_factor = toml::find<bool>(vi_calib_config, "disable_imu_factor");
    vi_config.is_four_as_one = toml::find<bool>(vi_calib_config, "is_four_as_one");
    ...
    vi_config.four_cam_calib_file_name = toml::find<std::string>(vi_calib_config, "four_cam_calib_file_name");
    vi_config.is_imu_calibrated = toml::find<bool>(vi_calib_config, "is_imu_calibrated");
    vi_config.use_trifocal_tensor_factor = toml::find<bool>(vi_calib_config, "use_trifocal_tensor_factor");
    vi_config.vm_step_reproj = toml::find<int>(vi_calib_config, "vm_step_reproj");
...
void ConfigData::InitConfigs() {
  ...
  for (int camId : calib_cid) {
    camPaths.emplace(camId, dataSet + "/" + cam_folder_prefix + std::to_string(camId) + "/" + cam_folder_subfix);
    camJsonPaths.emplace(camId, dataSet + "/" + cam_folder_prefix + std::to_string(camId) + "/" + cam_json);
  }
  ...
  if ((opendir(extrinsicPath.c_str())) == nullptr) {
    mkdir(extrinsicPath.c_str(), S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH);
  }

  if ((opendir(intrinsicPath.c_str())) == nullptr) {
    mkdir(intrinsicPath.c_str(), S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH);
  }
  ...
  switch (board_type) {
    ...
    case APRILTAG:
      ...
      binFilePath_gray = resultPath + "/" + apriltag_config.apriltag_bin_file_gray;
