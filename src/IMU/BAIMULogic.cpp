...
BAIMULogic::BAIMULogic(PreintegrationProviderBA *preintegrationProvider,
    ...
    : preintegrationProvider(preintegrationProvider),
      baIntegration(baIntegration), imuSettings(imuSettings),
      imuCalibration(imuCalibration),
      scaleQueue(imuSettings.generalScaleIntervalSize),
      optimizeScalePtr(new bool()), optimizeGravityPtr(new bool()),
      optimizedIMUExtrinsicsPtr(new bool()), optimizeScale(*optimizeScalePtr),
  ...
  if (!imuSettings.initSettings.disableVIOUntilFirstInit) {
    ...
    optimizeTransform =
        optimizeScale || optimizeGravity || optimizeIMUExtrinsics;
  } else {
  ...
  transformDSOToIMU.reset(new TransformDSOToIMU(
      gtsam::Pose3(imuCalibration.T_cam_imu.matrix()), optimizeScalePtr,
      optimizeGravityPtr, optimizedIMUExtrinsicsPtr,
      imuSettings.gravityDirectionFixZ));

  baIntegration->setDynamicDSOWeightCallback(
      [this](double lastDSOEnergy, double lastRMSE,
             bool coarseTrackingWasGood) {
        return computeDynamicDSOWeight(lastDSOEnergy, lastRMSE,
                                       coarseTrackingWasGood);
      });

  scaleFile.open(imuSettings.resultsPrefix +
                 "scalesdso.txt"); // contains the current scale for all times.
  baBiasFile.open(imuSettings.resultsPrefix +
                  "babiasdso.txt"); // contains estimated and groundtruth bias.
  baGravDirFile.open(imuSettings.resultsPrefix +
                     "bagravdir.txt"); // contains gravity directions.
  baVelFile.open(imuSettings.resultsPrefix +
                 "bavel.txt"); // contains estimated velocities.
}

bool BAIMULogic::addIMUVarsForKey(int keyframeId) {
  bool addIMUKeys =
      !imuSettings.skipFirstKeyframe || keyframeId != firstFrameId;
...
void dmvio::BAIMULogic::updateBAOrdering(std::vector<dso::EFFrame *> &frames,
...
void dmvio::BAIMULogic::addKeysToMarginalize(
    int fullId, gtsam::FastVector<gtsam::Key> &keysToMarginalize) {
  gtsam::Symbol velKey('v', fullId);
  gtsam::Symbol biasKey('b', fullId);
...
// This method is only relevant if the legacy setting disableVIOUntilFirstInit
// is set to false. Called from addKeyframe, and from initializeFromInitializer!
// It is first called from initializeFromInitializer, but then only the first 5
// lines of the method are executed. Then it will be called from the first
// calling of addKeyframe.
void dmvio::BAIMULogic::addFirstBAFrame(int keyframeId, BAGraphs *baGraphs,
                                        gtsam::Values::shared_ptr baValues) {
  ...
  std::cout << "Add first keyframe: Add priors for keyframe " << keyframeId
            << std::endl;
  ...
  if (imuSettings.setting_prior_bias) {
    gtsam::noiseModel::Diagonal::shared_ptr bias_prior_model =
        gtsam::noiseModel::Diagonal::Variances(
            (gtsam::Vector(6) << 1e-2, 1e-2, 1e-2, 1e-3, 1e-3, 1e-3)
                .finished());
    baGraphs->addFactor(
        boost::make_shared<gtsam::PriorFactor<gtsam::imuBias::ConstantBias>>(
            bias_current_key, initialBias, bias_prior_model),
        BIAS_AND_PRIOR_GROUP);
  }

  if (imuSettings.setting_prior_velocity) {
    gtsam::noiseModel::Diagonal::shared_ptr velocity_prior_model =
        gtsam::noiseModel::Isotropic::Sigma(3, 1e-1);
    baGraphs->addFactor(
        boost::make_shared<gtsam::PriorFactor<gtsam::Vector3>>(
            vel_current_key, initialVelocity, velocity_prior_model),
        BIAS_AND_PRIOR_GROUP);
  }

  // Add priors for gravity direction and extrinsics + insert them and the scale
  // into values.
  auto factors = getPriorsAndAddValuesForTransform(
      *transformDSOToIMU, imuSettings.transformPriors, *baValues);
  for (auto &&factor : factors) {
    baGraphs->addFactor(factor, BIAS_AND_PRIOR_GROUP);
  }

  previousKeyframeId =
      keyframeId; // Set so that the next call of addKeyframeToBA knows about
                  // this first frame.
...
void dmvio::BAIMULogic::setNextBAVel(const gtsam::Vector3 &velocity,
                                     int frameId) {
  nextVelocity = velocity;
  nextVelocityFrameId = frameId;
}

void dmvio::BAIMULogic::addKeyframe(BAGraphs *baGraphs,
                                    gtsam::Values::shared_ptr baValues,
                                    int keyframeId,
                                    const Sophus::SE3 &keyframePose,
  ...
  if (lastKeyframeId == -1 || lastKeyframeId == keyframeId) {
    assert(imuSettings.skipFirstKeyframe); // should not happen unless we want
                                           // to skip the first keyframe!
    ...
    auto &&imuMeasurements =
        preintegrationProvider->getPreintegratedMeasurements(keyframeId);

    // Alternative: Example how the GTSAM IMU Factor can be modified to handle
    // First Estimates Jacobians in the correct way. We disable it because it
    // does not seem to improve the results, as it is sufficient to handle FEJ
    // in the PoseTransformationFactor only). The same technique with our
    // FEJNoiseModelFator can also be used for the bias random walk factors.
    //        gtsam::NonlinearFactor::shared_ptr imuFactorWithFEJ(
    //                new FEJNoiseModelFactor<gtsam::ImuFactor>(pose_prev_key,
    //                vel_prev_key,
    //                                                          pose_current_key,
    //                                                          vel_current_key,
    //                                                          bias_prev_key,
    ...
    gtsam::NonlinearFactor::shared_ptr imuFactor(
        new gtsam::ImuFactor(prevPoseKey, prevVelKey, currPoseKey, currVelKey,
                             prevBiasKey, imuMeasurements));

    // Note: The graph optimizes poses with worldToCam in DSO scale. The IMU
    // factor works on metric poses with imuToWorld. To transform between the
    // two we use our PoseTransformationFactor and the transformDSOToIMU. This
    // will also add the scale and gravity direction as optimizable variables to
    // our factor graph.
    auto transformedFactor = boost::make_shared<PoseTransformationFactor>(
        imuFactor, *transformDSOToIMU,
        PoseTransformationFactor::JACOBIAN_FACTOR);
    baGraphs->addFactor(transformedFactor, METRIC_GROUP);

    gtsam::noiseModel::Diagonal::shared_ptr biasNoiseModel =
        computeBiasNoiseModel(imuCalibration, imuMeasurements);
    ...
    gtsam::imuBias::ConstantBias currentBias =
        baValues->at<gtsam::imuBias::ConstantBias>(prevBiasKey);
    ...
    if (nextVelocityFrameId != -1) {
      if (nextVelocityFrameId == keyframeId) {
        ...
        std::cout << "ERROR: nextVelocity frame not as expected! "
                  << nextVelocityFrameId << " " << nextVelocity << std::endl;
      }
    }
...
void dmvio::BAIMULogic::preSolve(gtsam::Matrix &HFull, gtsam::Vector &bFull,
                                 int dimensionDSOH) {
  if (disableFromKF > 0)
    return;

  if (imuSettings.useScaleDiagonalHack && optimizeScale && !scaleFixed) {
    // Hack which is useful in case we initialize immediately without a scale
    // prior. This stops the scale from converging even if it is far from the
    // optimum. Usually disabled because with the IMU initializer of DM-VIO we
    // don't need it.
...
bool dmvio::BAIMULogic::postSolve(gtsam::Values::shared_ptr values,
                                  gtsam::Values::shared_ptr newValues,
                                  const gtsam::Vector &inc,
                                  const gtsam::Ordering &ordering,
                                  const KeyDimMap &baDimMap) {
  if (disableFromKF > 0)
    return true;
...
void dmvio::BAIMULogic::acceptUpdate(gtsam::Values::shared_ptr values,
                                     gtsam::Values::shared_ptr newValues) {
  ...
  if (optimizeIMUExtrinsics) {
    ...
    std::cout << "Optimized T_cam_imu: "
              << newT_cam_imu.translation().transpose() << end;
  }
}

// Signals that the BA and marginalization operations for this keyframe are
// finished (might be after the KF is already the new tracking ref)
void dmvio::BAIMULogic::finishKeyframeOperations(int keyframeId) {
  ...
  gtsam::imuBias::ConstantBias currentBias =
      baValues->at<gtsam::imuBias::ConstantBias>(
  ...
  if (currGTData) {
    ...
    baBiasFile << std::fixed << std::setprecision(6) << currBATimestamp << ' '
               << std::setprecision(20) << gtTrans.transpose() << ' '
               << gtRot.transpose() << ' '
               << currentBias.accelerometer().transpose() << ' '
  ...
  if (optimizeScale && !scaleFixed) {
    // Compute how much the scale has changed lately to determine if it should
    // be fixed.
    ...
    // Compute and minimum and maximum scale over the last n keyframes
    // optimizations.
    ...
    if (diff < imuSettings.setting_scaleFixTH) {
      // Fix scale!
      scaleFixed = true;
      std::cout << "Fixing scale at time: " << std::fixed
                << std::setprecision(6) << currBATimestamp << std::endl;

      // Useful for testing. Allows to evaluate the visual-only system, for the
      // majority of the time but with the metric scale obtained from IMU.
      if (imuSettings.setting_visualOnlyAfterScaleFixing == 1) {
        std::cout << "DISABLING IMU AND THE GTSAM INTEGRATION COMPLETELY!"
                  << std::endl;
...
BAIMULogic::computeFactorForCoarseGraphAndMarginalCovariances() {
  dmvio::TimeMeasurement meas(
      "computeFactorForCoarseGraphAndMarginalCovariances");

  // The idea behind this method is that we want both, a factor for the coarse
  // tracking, and marginal uncertainty for some more variables. These need to
  // do similar processing, hence we combine them: First we marginalize
  // everything that is neither in factorOrdering or uncertOrdering, and compute
  // marginal covariances. Then we also marginalize everything in uncertOrdering
  // to obtain the factor for the coarse graph.

  gtsam::Ordering
      factorOrdering; // This contains all keys which shall be in the factor.
  ...
  auto graphPair = baIntegration->getBaGraphs().getHAndB(
      *(baIntegration->getBaValues()), ordering, keyDimMap, &additionalKeys);

  gtsam::Ordering completeOrdering;
  completeOrdering.insert(completeOrdering.begin(), ordering.begin(),
                          ordering.end());
  completeOrdering.insert(completeOrdering.end(), additionalKeys.begin(),
                          additionalKeys.end());
  ...
  auto linearizedDSOGraph =
      activeDSOGraph.linearize(*(baIntegration->getBaValues()));

  AugmentedScatter scatter(*linearizedDSOGraph, completeOrdering, keyDimMap);
  std::pair<gtsam::Matrix, gtsam::Vector> pair =
      scatter.computeHessian(*linearizedDSOGraph);

  gtsam::Ordering newAdditionalKeys;
  fillAdditionalKeysFromScatter(completeOrdering, newAdditionalKeys, scatter);
  additionalKeys.insert(additionalKeys.end(), newAdditionalKeys.begin(),
                        newAdditionalKeys.end());

  assert(graphPair.first.size() <= pair.first.size());
  pair.first.block(0, 0, graphPair.first.rows(), graphPair.first.cols()) +=
      graphPair.first;
  ...
  int mSize = std::accumulate(additionalKeys.begin(), additionalKeys.end(), 0,
                              accumFun);

  // Then we first marginalize everything except the necessary variables.

  // Marginalize with preconditioning:
  // First create the proper input (first variables to marginalize, then rest)
  // for computeSchurComplement.
  gtsam::Matrix margInputH(aSize + mSize, aSize + mSize);
  margInputH.block(0, 0, mSize, mSize) =
      pair.first.block(aSize, aSize, mSize, mSize);
  margInputH.block(mSize, mSize, aSize, aSize) =
      pair.first.block(0, 0, aSize, aSize);
  margInputH.block(0, mSize, mSize, aSize) =
      pair.first.block(aSize, 0, mSize, aSize);
  margInputH.block(mSize, 0, aSize, mSize) =
      pair.first.block(0, aSize, aSize, mSize);
  ...
  gtsam::Matrix marginalized = computeSchurComplement(
      augmentedHessianFromPair(std::make_pair(margInputH, margInputB)), mSize,
      aSize);
  ...
  scaleFile << std::fixed << std::setprecision(6)
            << baIntegration->getCurrBaTimestamp() << ' '
            << transformDSOToIMU->getScale() << ' ' << uncertainties[0](0, 0)
            << end;

  Eigen::Vector3d rotLog = transformDSOToIMU->getR_dsoW_metricW().log();
  baGravDirFile << std::fixed << std::setprecision(6)
                << baIntegration->getCurrBaTimestamp() << ' '
                << rotLog.transpose() << ' '
                << uncertainties[1].diagonal().transpose() << end;

  baVelFile
      << std::fixed << std::setprecision(6)
      << baIntegration->getCurrBaTimestamp() << ' '
      << baIntegration->getBaValues()->at<gtsam::Vector3>(velKey).transpose()
      << ' ' << uncertainties[uncertOrdering.size() + 1].diagonal().transpose()
      << end;

  // Now marginalize everything in uncertOrdering, because these variables shall
  // not be in the factor for the coarse graph.
  int secondASize = std::accumulate(factorOrdering.begin(),
                                    factorOrdering.end(), 0, accumFun);
  int secondMSize = std::accumulate(uncertOrdering.begin(),
...
BAIMULogic::finishKeyframeOptimization(int keyframeId) {
  ...
  returning->latestBAVel =
      baValues->at<gtsam::Vector3>(gtsam::Symbol('v', keyframeId));
  returning->latestBABias = baValues->at<gtsam::imuBias::ConstantBias>(
      gtsam::Symbol('b', keyframeId));
  returning->latestBAPose =
      baValues->at<gtsam::Pose3>(gtsam::Symbol('p', keyframeId));
  returning->latestBAPosePrevKeyframe =
      baValues->at<gtsam::Pose3>(gtsam::Symbol('p', previousKeyframeId));

  assert(transformDSOToIMU != nullptr);
  std::shared_ptr<TransformDSOToIMU> copy(
      new TransformDSOToIMU(*transformDSOToIMU));
  returning->transformBAToIMU = copy;
  returning->transformIMUToDSOForCoarse =
      std::make_unique<TransformIMUToDSOForCoarse<TransformDSOToIMU>>(
          copy, keyframeId);
...
// Takeover result from the IMU initializer. Values contains the new values (DSO
// variables will not replaced though). If reinit this is a reinitialization,
// meaning that the IMU is being used already. If willReplaceGraph the IMU
// initializer will replace the marginalization prior (typically obtained by
// readvancing).
void BAIMULogic::initFromIMUInit(const gtsam::Values &values, bool reinit,
  ...
  {
    // If the IMU initializer does not provide a new marginalization prior (e.g.
    // for the ablation studies), we have to manually add the priors.

    // find out the latestKFId
    for (auto &&val : values) {
      gtsam::Symbol sym(val.key);
      if (sym.chr() == 'v' &&
          static_cast<long long>(sym.index()) > latestKFId) {
    ...
    auto factors = getPriorsAndAddValuesForTransform(
        *transformDSOToIMU, imuSettings.transformPriors, nullValues);
    for (auto &&factor : factors) {
    ...
    if (imuSettings.initSettings.scalePriorAfterInit > 0) {
      ...
      gtsam::PriorFactor<ScaleGTSAM>::shared_ptr scalePrior(
          new gtsam::PriorFactor<ScaleGTSAM>(
              S(0), values.at<ScaleGTSAM>(S(0)),
              gtsam::noiseModel::Diagonal::Sigmas(scaleModel)));
      baIntegration->getBaGraphs().addFactor(scalePrior, BIAS_AND_PRIOR_GROUP);
    }

    // don't add keys before this key, because there will not be respective IMU
    // factors in the graph.
    noIMUInOrderingUntilKFId = latestKFId;
  } else if (!reinit) {
    // If we skipped KFs in the PGBA we must skip them now as well.
    // In general it might happen that there is still a very old KF in the
    // active window for which did not get included in the PGBA.
  ...
  for (auto &&val : values) {
    ...
    if (noIMUInOrderingUntilKFId >= 0) {
      if ((sym.chr() == 'v' || sym.chr() == 'b') &&
          sym.index() < noIMUInOrderingUntilKFId) {
        replaceValue = false;
      }
    }
    if (replaceValue) {
  ...
  // When initializing from CoarseIMUInit (ablation study) we might have
  // optimized only a single bias.
  if (!targValues->exists(gtsam::Symbol('b', latestKFId))) {
    targValues->insert(gtsam::Symbol('b', latestKFId),
                       values.at(gtsam::Symbol('b', 0)));
  }

  if (!reinit) {
    std::cout << "INITIALIZED IMU Integration. Using IMU data in main "
...
bool BAIMULogic::isScaleFixed() const { return scaleFixed; }

double BAIMULogic::computeDynamicDSOWeight(double lastDSOEnergy,
                                           double lastRMSE,
                                           bool coarseTrackingWasGood) {
  // Compute dynamic photometric weight. Basically a threshold robust cost
  // function.
