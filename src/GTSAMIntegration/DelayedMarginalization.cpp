...
std::pair<gtsam::Matrix, gtsam::Vector> dmvio::DelayedMarginalizationGraphs::getHAndB(
    const gtsam::Values& values, const gtsam::Ordering& ordering, const std::map<gtsam::Key, size_t>& keyDimMap,
...
void dmvio::DelayedMarginalizationGraphs::marginalizeFrame(const gtsam::FastVector<gtsam::Key>& keysToMarginalize,
  ...
  // measurements.
  dmvio::TimeMeasurement mainMeas("MarginalizeMainGraph");  // This saves only the time necessary for
...
std::shared_ptr<DelayedGraph> DelayedMarginalizationGraphs::getMainGraph() { return delayedGraphs[mainGraphInd]; }
...
void DelayedMarginalizationGraphs::removeDelayedGraph(const DelayedGraph* graph) {
  auto it = std::find_if(delayedGraphs.begin(), delayedGraphs.end(),
                         [graph](const std::shared_ptr<DelayedGraph>& comp) { return comp.get() == graph; });
...
std::shared_ptr<DisconnectedDelayedGraph> DelayedMarginalizationGraphs::addDisconnectedGraph(int maxGroupInGraph) {
...
void DelayedMarginalizationGraphs::removeDisconnectedGraph(const DisconnectedDelayedGraph* graph) {
  auto it =
      std::find_if(disconnectedGraphs.begin(), disconnectedGraphs.end(),
                   [graph](const std::shared_ptr<DisconnectedDelayedGraph>& comp) { return comp.get() == graph; });
...
void dmvio::DelayedGraph::marginalize(const gtsam::FastVector<gtsam::Key>& keysToMarginalize,
...
gtsam::NonlinearFactorGraph::shared_ptr DelayedGraph::getGraph() const { return graph; }

const gtsam::Values& DelayedGraph::getDelayedValues() const { return delayedValues; }

const gtsam::Values& DelayedGraph::getDelayedCurrValues() const { return delayedCurrValues; }
...
DelayedGraph::DelayedGraph(int delayN, int maxGroupInGraph, const gtsam::NonlinearFactorGraph::shared_ptr& graph,
                           ...
                           gtsam::Values delayedCurrValues, std::shared_ptr<FEJValues> fejValuesPassed)
    : delayN(delayN),
...
DelayedGraph::DelayedGraph(const DelayedGraph& other)
    : delayN(other.delayN),
      maxGroupInGraph(other.maxGroupInGraph),
...
void DelayedGraph::setMaxGroupInGraph(int maxGroupInGraph) { DelayedGraph::maxGroupInGraph = maxGroupInGraph; }

void DelayedGraph::setDelayN(int delayN) { DelayedGraph::delayN = delayN; }

void DelayedGraph::setFEJValuesForFactors(bool useFEJ) { setFEJMapForGraph(*graph, useFEJ ? fejValues : nullptr); }
