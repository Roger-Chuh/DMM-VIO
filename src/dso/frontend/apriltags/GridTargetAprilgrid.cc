...
#include "GridTargetAprilgrid.hpp"
#include <Eigen/Core>
#include <algorithm>
  ...
  if (detections.size() > 1) {
    for (unsigned i = 0; i < detections.size() - 1; i++)
      if (detections[i].id == detections[i + 1].id) {
        if (!_options.showExtractionVideo) {
          printf("Duplicate Apriltags detected. Hide them\n");
          return false;
        }
        ...
        std::cout << "\n[ERROR]: Found apriltag not belonging to calibration "
                     "board. Check the image for the tag and "
                     "hide it.\n";
        cv::waitKey();
        exit(0);
      }
  ...
  if (_options.showExtractionVideo) {
    ...
    for (unsigned i = 0; i < detections.size(); i++)
      for (unsigned j = 0; j < 4; j++) {
        ...
        // cv::circle(imageCopy1, cv::Point2f(detections[i].p[j].first,
