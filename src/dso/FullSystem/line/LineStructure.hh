...
struct OctaveSingleLine {
  ...
  void PrintInfo() const {
    printf(
        "level: %d, start_end: [(%f %f) (%f %f)], start_end_scaled: [(%f %f) (%f %f)], dir: %f, salience: %f, length: "
        "%f, pix_num: %d, descrip_size: %d\n",
        octaveCount, startPointX, startPointY, endPointX, endPointY, sPointInOctaveX, sPointInOctaveY, ePointInOctaveX,
        ePointInOctaveY, direction, salience, lineLength, numOfPixels, descriptor.size());
  }
};
...
typedef std::vector<LinesVec> ScaleLines;  // each element in ScaleLines is a vector of lines which corresponds the same
                                           // line detected in different octave images.
}  // namespace LBD
}  // namespace yvr_vio
