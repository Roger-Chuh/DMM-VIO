...
struct LineGroup {
  explicit LineGroup(const Triple& o) : ops({o.vs[0]->id, o.vs[1]->id, o.vs[2]->id}) {}
...
class KDTree {
 public:
  ...
  void insert(Vertex* point) { root = insertRecursive(std::move(root), point, 0); }
 ...
 private:
  ...
  std::unique_ptr<KDNode> insertRecursive(std::unique_ptr<KDNode> node, Vertex* point, size_t depth) {
    if (!node) {
  ...
  void searchRecursive(const KDNode* node, const Vertex& query, size_t k, std::vector<Vertex*>& nearestNeighbors,
                       size_t depth) const {
    if (!node) {
      return;
    }
    ...
    // 检查当前节点是否更近
    if (nearestNeighbors.size() < k || query.distance(*node->point) < query.distance(*nearestNeighbors.back())) {
      nearestNeighbors.push_back(node->point);
      std::sort(nearestNeighbors.begin(), nearestNeighbors.end(),
                [&query](const Vertex* a, const Vertex* b) { return query.distance(*a) < query.distance(*b); });
    ...
    if (nearestNeighbors.size() < k ||
        std::abs(query.pc(axis) - node->point->pc(axis)) < query.distance(*nearestNeighbors.back())) {
