...
ImageProcessing::ImageProcessing(double grid_spacing, const Eigen::Vector2i& grid_size, uint32_t seed, int plate_num,
                                 int unique_size)
    : boardInfo_(grid_spacing, grid_size, seed, plate_num, unique_size) {}

void ImageProcessing::ProcessPic(const cv::Mat& grayscale_image, const ParamsImageProcessing& params,
                                 CalibIO::CurFrameRes& res, cv::Mat* p_binary_mat) const {
  // 将仅有两个邻居的点删除
  cv::Mat grayImg;
  ...
  // Process image
  cv::Mat threshPic;
  cv::adaptiveThreshold(grayImg, threshPic, 255, cv::AdaptiveThresholdTypes::ADAPTIVE_THRESH_MEAN_C, cv::THRESH_BINARY,
                        params.window_size, params.at_threshold * 25);
#ifndef __ANDROID__
  if (verbose) {
  ...
  LabelTreshPic(threshPic, labels, params.conic_min_area, params.conic_max_area, params.conic_symmetry,
                params.conic_min_aspect, 0, 2, p_binary_mat);
  //  std::vector<Conic> conics;
  ...
  FindConics(threshPic.cols, threshPic.rows, labels, &imgD[0], conics, params.black_on_white);
  ...
  if (verbose) {
    ...
    for (const auto& label : labels) {
      auto point_color = cv::Scalar(int(label.cluster_center.x() * 10) % 255, int(label.cluster_center.y() * 10) % 255,
    ...
    for (const auto& curconic : conics) {
      cv::rectangle(boxMat, cv::Point2i(curconic.bbox.x1, curconic.bbox.y1),
                    cv::Point2i(curconic.bbox.x2, curconic.bbox.y2), cv::Scalar(0, 0, 255));
      cv::rectangle(fit_pixel_mat, cv::Point2i(curconic.bbox.x1, curconic.bbox.y1),
                    cv::Point2i(curconic.bbox.x2, curconic.bbox.y2), cv::Scalar(0, 0, 255));
      ...
      ellipse_center.x() = (m_B * m_E - 2 * m_C * m_D) / (4 * m_A * m_C - m_B * m_B);
      ellipse_center.y() = (m_B * m_D - 2 * m_A * m_E) / (4 * m_A * m_C - m_B * m_B);
      double a_a = 2 *
                   (m_A * ellipse_center.x() * ellipse_center.x() + m_C * ellipse_center.y() * ellipse_center.y() +
      ...
      if (a_a > 0 && b_b > 0) {
        ...
        cv::ellipse(boxMat, cv::Point2d(ellipse_center.x(), ellipse_center.y()), cv::Size(ellipse_a, ellipse_b),
                    ellipse_theta * 180.0 / M_PI, 0, 360, cv::Scalar(255, 255, 0));
        cv::circle(boxMat, cv::Point2d(ellipse_center.x(), ellipse_center.y()), 0, cv::Scalar(255, 255, 0));
      }
    }
  ...
  //    return;
  //  }
  TargetGridDot::FindTarget(conics, plateConics, boardInfo_, params, (verbose ? &grayscale_image : nullptr));
  ...
  for (const auto& plateData : plateConics) {
    ...
    res.mImagePointSets.emplace(plateData.first, std::vector<Eigen::Vector2d>{});
    res.mObjectPointSets.emplace(plateData.first, std::vector<Eigen::Vector3d>{});
