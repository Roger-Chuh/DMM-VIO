...
ImmaturePoint::ImmaturePoint(int u_, int v_, FrameHessian* host_, float type, CalibHessian* HCalib,
                             const int& host_cid_, const int& host_level_)
    : u(u_),
      v(v_),
      host(host_),
      my_type(type),
      host_cid(host_cid_),
      host_level(host_level_),
      idepth_min(0),
      idepth_max(NAN) {
  ...
  for (int idx = 0; idx < patternNumSeed; idx++) {
    ...
    // Vec3f ptc = getInterpolatedElement33BiLin(host->dI, u + dx, v + dy,
    // wG[0]);
    Vec3f ptc = getInterpolatedElement33BiLin(host->dIp[host_level_] + wG[host_level] * hG[host_level] * host_cid,
                                              u + dx, v + dy, wG[host_level]);

    color[idx] = ptc[0];
#ifdef USE_EDGE_ALIGN
    Vec3f dt_dx_dy = getInterpolatedElement33BiLin(
        host->dt_dx_dy[host_level_] + wG[host_level] * hG[host_level] * host_cid, u + dx, v + dy, wG[host_level]);
    ...
    //! 点的权重 c^2 / ( c^2 + ||grad||^2 )
    weights_gray[idx] =
        sqrtf(setting_outlierTHSumComponent / (setting_outlierTHSumComponent + ptc.tail<2>().squaredNorm()));
#if 1  // ndef USE_EDGE_ALIGN
    weights[idx] = weights_gray[idx];
#else
    weights[idx] =
        sqrtf(setting_outlierTHSumComponent / (setting_outlierTHSumComponent + dt_dx_dy.tail<2>().squaredNorm()));
#endif
  }
  for (int idx = 0; idx < patternNum; idx++) {
  ...
  energyTH_converged =
      patternNum * setting_outlierTH_init * setting_outlierTH_init;  // 只被用来判断是不是finite，没用具体数值
  energyTH *= setting_overallEnergyTHWeight * setting_overallEnergyTHWeight;
  energyTH_converged *= setting_overallEnergyTHWeight * setting_overallEnergyTHWeight;
 ...
 */
float ImmaturePoint::CalcZncc(const Eigen::MatrixXf& host_, const Eigen::MatrixXf& target_) {
...
ImmaturePointStatus ImmaturePoint::traceOn(const int& target_cid, FrameHessian* frame, const Mat33f& hostToFrame_KRKi,
  ...
  if (lastTraceStatus[target_cid] == ImmaturePointStatus::IPS_OOB) return lastTraceStatus[target_cid];
  // float setting_huberTH_use = setting_huberTH_loose;
  debugPrint = false;  // rand()%100==0;
#ifdef USE_MULTI_CAM
  float maxPixSearch =
      (wG[lvl] + hG[lvl]) * setting_maxPixSearch * (is_first_frame ? 2.0f : 1.0f);  // 极限搜索的最大长度
#else
  float maxPixSearch = (wG[lvl] + hG[lvl]) * setting_maxPixSearch;  // 极限搜索的最大长度
#endif

  if (debugPrint) {
    printf(
        "is_first_frame: %d, cid: [%d %d], level: [%d], trace pt (%.1f "
        "%.1f) from frame %d to %d. Range %f -> %f. t %f %f "
        "%f!\n",
        is_first_frame, host_cid, target_cid, lvl, u, v, host->shell->id, frame->shell->id, idepth_min, idepth_max,
  ...
  for (int idx = 0; idx < patternNumSeed; idx++) {
    rotatetPattern[idx] = Rplane * Vec2f(patternPSeed[idx][0] * pattern_scale_seed_point_opt,
  ...
  // 如果超出图像范围则设为 OOB
  if (!(uMin > boundU && vMin > boundV && uMin < wG[lvl] - boundU - 1 && vMin < hG[lvl] - boundV - 1)) {
    if (debugPrint)
  ...
  if (std::isfinite(idepth_max)) {
    ...
    if (!(uMax > boundU && vMax > boundV && uMax < wG[lvl] - boundU - 1 && vMax < hG[lvl] - boundV - 1)) {
    ...
    if (dist < setting_trace_slackInterval) {
      ...
      lastTraceUV[target_cid] = Vec2f(uMax + uMin, vMax + vMin) * 0.5;  // 直接设为中值
...
#endif
      return lastTraceStatus[target_cid] = ImmaturePointStatus::IPS_SKIPPED;  //跳过
    }
    assert(dist > 0);
  } else {  //* 上限无穷大, 则设为最大值
    dist = maxPixSearch;
  ...
  if (!(idepth_min < 0 || (ptpMin[2] > 0.1 /*0.5 0.75*/ && ptpMin[2] < 10.0 /*2 .01.5*/))) {
  ...
  int numSteps = 1.9999f + dist / (setting_trace_stepsize * std::pow(2.0, 0.0 /*-lvl*/));  // 步数
...
#endif
  if (numSteps >= step_num /*150*/) numSteps = step_num - 1;  //[149]
  ...
  for (int i = 0; i < numSteps; i++) {
    ...
    for (int idx = 0; idx < patternNumSeed; idx++) {
      ...
      if (!is_first_frame) {
        hitColor =
            getInterpolatedElement31(frame->dI + wG[lvl] * hG[lvl] * target_cid, (float)(ptx + rotatetPattern[idx][0]),
                                     (float)(pty + rotatetPattern[idx][1]), wG[lvl]);
        hitColor_edge = getInterpolatedElement31(frame->dt_dx_dy_0 + wG[lvl] * hG[lvl] * target_cid,
                                                 (float)(ptx + rotatetPattern[idx][0]),
                                                 (float)(pty + rotatetPattern[idx][1]), wG[lvl]);
      } else {
        hitColor = getInterpolatedElement31(frame->dIp[lvl] + wG[lvl] * hG[lvl] * target_cid,
                                            (float)(ptx + rotatetPattern[idx][0]),
                                            (float)(pty + rotatetPattern[idx][1]), wG[lvl]);
        hitColor_edge = getInterpolatedElement31(frame->dt_dx_dy[lvl] + wG[lvl] * hG[lvl] * target_cid,
                                                 (float)(ptx + rotatetPattern[idx][0]),
                                                 (float)(pty + rotatetPattern[idx][1]), wG[lvl]);
      }
#ifdef SHOW_TRACEON
      if (show_image) {
        img_target->setPixel9(ptx + rotatetPattern[idx][0], pty + rotatetPattern[idx][1], makeRainbow3B(1), target_cid);
        if (i == 0) {
          img_target->setPixelCirc(ptx + rotatetPattern[idx][0], pty + rotatetPattern[idx][1], Vec3b(255, 0, 0),
                                   target_cid);
        }
        if (i == numSteps - 1) {
          img_target->setPixelCirc(ptx + rotatetPattern[idx][0], pty + rotatetPattern[idx][1], Vec3b(0, 255, 255),
                                   target_cid);
        }
      }
#endif
      ...
      float residual = hitColor - (float)(hostToFrame_affine[0] * (color[idx /* + wG[0] * hG[0] *  host_cid*/]) +
                                          hostToFrame_affine[1]);
#else
      float residual = hitColor_edge;
#endif
      float hw = fabs(residual) < setting_huberTH_trace_on ? 1 : setting_huberTH_trace_on / fabs(residual);
      if (debugPrint) {
        printf(
            "step: %d, idx: %d, residual: %f, setting_huberTH_trace_on: %f, "
            "hw: %f\n",
      ...
      host_val_each(valid_count_each, 0) =
          (float)(hostToFrame_affine[0] * (color[idx /* + wG[0] * hG[0] *  host_cid*/]) + hostToFrame_affine[1]);
  ...
  for (int it = 0; it < setting_trace_GNIterations; it++) {
    ...
    for (int idx = 0; idx < patternNumSeed; idx++) {
      ...
      Vec3f hitColor_edge;
      if (!is_first_frame) {
        hitColor = getInterpolatedElement33(frame->dI + wG[lvl] * hG[lvl] * target_cid, posU, posV, wG[lvl]);
        hitColor_edge =
            getInterpolatedElement33(frame->dt_dx_dy_0 + wG[lvl] * hG[lvl] * target_cid, posU, posV, wG[lvl]);
      } else {
        hitColor = getInterpolatedElement33(frame->dIp[lvl] + wG[lvl] * hG[lvl] * target_cid, posU, posV, wG[lvl]);
        hitColor_edge =
            getInterpolatedElement33(frame->dt_dx_dy[lvl] + wG[lvl] * hG[lvl] * target_cid, posU, posV, wG[lvl]);
      }
      ...
      float dResdDist = dx * hitColor[1] + dy * hitColor[2];  /// 极线方向梯度, jacobian
#else
      float residual = hitColor_edge[0];
      float dResdDist = dx * hitColor_edge[1] + dy * hitColor_edge[2];  /// 极线方向梯度, jacobian
#endif

      float hw = fabs(residual) < setting_huberTH_trace_on ? 1 : setting_huberTH_trace_on / fabs(residual);
      if (debugPrint) {
        printf(
            "iter: %d, idx: %d, res: %f, hw: %f, setting_huberTH_trace_on: "
            "%f\n",
      ...
      hw_count += 1.0;
#ifdef USE_ZNCC_SEARCH
      energy += weights_gray[idx] * weights_gray[idx] * hw * residual * residual * (2 - hw);
#else
      ...
      host_val(valid_count, 0) = (hostToFrame_affine[0] * color[idx] + hostToFrame_affine[1]);
  ...
  if (debugPrint) {
    printf(
        ...
        bestEnergy, energyTH, setting_trace_extraSlackOnTH, bestEnergy_zncc, setting_outlierTH_zncc_tracker,
  ...
  // host->frameGradTH*0.50f))
  {
    ...
    if (lastTraceStatus[target_cid] == ImmaturePointStatus::IPS_OUTLIER) {
      ...
      if (show_image) {
        printf(
            "return 10, is_first_frame: %d,  host_cid: %d, target_cid: %d, "
  ...
  if (show_image) {
    printf(
        "GOOD !!! is_first_frame: %d,  host_cid: %d, target_cid: %d, "
        "idepth_min: %f, idepth_max: %f\n",
        is_first_frame, host_cid, target_cid, idepth_min, idepth_max);
    IOWrap::displayImage("host", img_host);
    IOWrap::displayImage("target", img_target);
  ...
  return lastTraceStatus[target_cid] = ImmaturePointStatus::IPS_GOOD;  //上一次的位置
}
...
float ImmaturePoint::getdPixdd(CalibHessian* HCalib, ImmaturePointTemporaryResidual* tmpRes, float idepth) {
  ...
  projectPoint(this->u, this->v, idepth, 0, 0, HCalib, precalc->PRE_RTll, PRE_tTll, drescale, u, v, Ku, Kv, KliP,
               new_idepth);
...
float ImmaturePoint::calcResidual(CalibHessian* HCalib, const float outlierTHSlack,
  ...
  for (int idx = 0; idx < patternNumSeed; idx++) {
    ...
    if (!projectPoint(this->u + patternPSeed[idx][0] * pattern_scale_seed_point_opt,
                      this->v + patternPSeed[idx][1] * pattern_scale_seed_point_opt, idepth, PRE_KRKiTll, PRE_KtTll, Ku,
                      Kv)) {
      return 1e10;
    }
    ...
    float residual = hitColor_edge[0];
#endif
    float hw = fabsf(residual) < setting_huberTH_trace_on ? 1 : setting_huberTH_trace_on / fabsf(residual);
#ifdef USE_ZNCC_SEARCH
    energyLeft += weights_gray[idx] * weights_gray[idx] * hw * residual * residual * (2 - hw);
#else
...
double ImmaturePoint::linearizeResidual(const int& target_cid, CalibHessian* HCalib, const float outlierTHSlack,
  ...
  const Eigen::Vector3f* dIl = tmpRes->target->dIp[lvl_target] + wG[lvl_target] * hG[lvl_target] * target_cid;
  const Eigen::Vector3f* dIl_edge = tmpRes->target->dt_dx_dy[lvl_target] + wG[lvl_target] * hG[lvl_target] * target_cid;
  const Mat33f& PRE_RTll = precalc->a_PRE_RTll[host_cid * kCameraNumUsed + target_cid];
  const Vec3f& PRE_tTll = precalc->a_PRE_tTll[host_cid * kCameraNumUsed + target_cid];
  ...
  if (show_image) {
    ...
    img_host->setPixel9(this->u + 0.5, this->u + 0.5, makeRainbow3B(1), host_cid);
  }
  ...
  for (int idx = 0; idx < patternNumSeed; idx++) {
    ...
    bool projectedd = projectPoint(this->u, this->v, idepth, dx, dy, HCalib, PRE_RTll, PRE_tTll, drescale, u, v, Ku, Kv,
                                   KliP, new_idepth, lvl_target);
#ifdef SHOW_POINT_OPT
    if (show_image) {
      if (idx == 0 && (Ku > 15 && Kv > 15 && Ku < wG[0] - 15 && Kv < hG[0] - 15 && new_idepth > 0)) {
    ...
    float hw = fabsf(residual) < setting_huberTH_linearize ? 1 : setting_huberTH_linearize / fabsf(residual);
#ifdef USE_ZNCC_SEARCH
    energyLeft += weights_gray[idx] * weights_gray[idx] * hw * residual * residual * (2 - hw);
#else
    energyLeft += weights[idx] * weights[idx] * hw * residual * residual * (2 - hw);
    ...
    if (print_info) {
      printf(
          "idx: %d, weights[idx]: %f,weights_gray[idx]: %f, residual: %f, "
  ...
  zncc = CalcZncc(host_val, target_val);
  if (energyLeft > energyTH * outlierTHSlack ||
      zncc < setting_outlierTH_zncc_angle_epi_linearize /*setting_outlierTH_zncc_tracker*/) {
    energyLeft = energyTH * outlierTHSlack;
  ...
  if (print_info) {
    printf(
        "hw_mean: %f, energyLeft: %f, zncc: %f, energyTH: %f, "
        "outlierTHSlack: %f, setting_outlierTH_zncc_tracker: %f, "
        "setting_outlierTH_zncc_angle_epi_linearize: %f\n",
        hw_sum / hw_count, energyLeft, zncc, energyTH, outlierTHSlack, setting_outlierTH_zncc_tracker,
