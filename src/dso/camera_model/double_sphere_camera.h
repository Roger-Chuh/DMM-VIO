...
class DoubleSphereCamera : public CameraBase {
 public:
  ...
  explicit DoubleSphereCamera(CamId camera_id, int width, int height) : CameraBase(camera_id, width, height) {
    camera_model_ = CameraModel::kDoubleSphere;
    kParamLength = 6;
  }

  // parameters: fx, fy, cx, cy, xi, alpha
  DoubleSphereCamera(uint8_t camera_id, int width, int height, const number_t* parameters)
