...
namespace dso {
class UCMRTPTHCamera : public CameraBase {
 public:
  using Ptr = std::shared_ptr<UCMRTPTHCamera>;

  explicit UCMRTPTHCamera(CamId camera_id, int width, int height) : CameraBase(camera_id, width, height) {
    camera_model_ = CameraModel::kUcmRTPth;
    kParamLength = 17;
  }

  // parameters: fx, fy, cx, cy, alpha, k1, k2, k3, k4, k5, k6, p1, p2, s1, s2,
  // s3, s4
  UCMRTPTHCamera(CamId camera_id, int width, int height, const number_t* parameters, const int& level)
      : CameraBase(camera_id, width, height, parameters, 17) {
    camera_model_ = CameraModel::kUcmRTPth;

    //    level_ = level;
    //    level_scale_ = std::pow(2.0, -level_);
  }
 ...
 private:
  ...
  void ComputeDxuDpnorm(const Vec3& p_norm, Mat23& d_xu_pnorm) const;

  static constexpr int k_start = 5;
  static constexpr int p_start = 11;
  static constexpr int s_start = 13;

  //  number_t level_scale_;
};
}  // namespace dso