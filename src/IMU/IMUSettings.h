...
// There are the following ways to set them (where the latter ones overwrite the
// former ones):
...
public:
  ...
  // could be usefull for some applications. 0.005 or 0.007 are good values for
  // it.
  double setting_scaleFixTH = 0;
  int generalScaleIntervalSize = 60;

  // Maximum number of measurements to include for the simple gravity
  // initializer.
  ...
  // Use the visual only system after scale has been fixed, which can be useful
  // for debugging (only makes sense together with setting_scaleFixTH). 1 means
  // that also the gtsamIntegration is not used anymore, while 2 means that the
  // gtsamIntegration is still used with IMUExtension removed.
