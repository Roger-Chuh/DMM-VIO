...
#ifndef DMVIO_EDGE_DRAWING_HPP__
#define DMVIO_EDGE_DRAWING_HPP__
...
/** @brief Class implementing the ED (EdgeDrawing) @cite topal2012edge, EDLines
 * @cite akinlar2011edlines, EDPF @cite akinlar2012edpf and EDCircles @cite
...
public:
  ...
  struct CV_EXPORTS_W_SIMPLE Params {
    CV_WRAP Params();
    //! Parameter Free mode will be activated when this value is set as true.
    //! Default value is false.
    CV_PROP_RW bool PFmode;
    /** @brief indicates the operator used for gradient calculation.

    one of the flags cv::ximgproc::EdgeDrawing::GradientOperator. Default value
    is PREWITT
    */
    CV_PROP_RW int EdgeDetectionOperator;
    //! threshold value of gradiential difference between pixels. Used to create
    //! gradient image. Default value is 20
    ...
    /** @brief minimun connected pixels length processed to create an edge
    segment.

    in gradient image, minimum connected pixels length processed to create an
    edge segment. pixels having upper value than GradientThresholdValue will be
    processed. Default value is 10
    */
    ...
    //! Default value is true. indicates if NFA (Number of False Alarms)
    //! algorithm will be used for line and ellipse validation.
  ...
  */
  ...
  /** @brief Returns std::vector<std::vector<Point>> of detected edge segments,
   * see detectEdges()
   */
  CV_WRAP virtual std::vector<std::vector<Point>> getSegments() = 0;

  /** @brief Returns for each line found in detectLines() its edge segment index
   * in getSegments()
   */
  ...
  @param lines  output Vec<4f> contains the start point and the end point of
  detected lines.
  ...
  */
  ...
  @param ellipses  output Vec<6d> contains center point and perimeter for
  circles, center point, axes and angle for ellipses.
  @note you should call detectEdges() before calling this function.
  */
  ...
  this function is meant to be used for parameter setting in other languages
  than c++ like python.
  @param parameters
  */
