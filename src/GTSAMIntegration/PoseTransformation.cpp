...
// Default implementation for child classes uses numeric Jacobians and should
// work out of the box.
gtsam::Matrix66
PoseTransformation::getPoseDerivative(const PoseType &posePassed,
...
PoseTransformation::getAllDerivatives(const PoseType &pose,
                                      DerivativeDirection direction) {
...
gtsam::Matrix66
TransformIdentity::getPoseDerivative(const PoseType &pose,
                                     DerivativeDirection direction) {
  ...
  if (direction == DerivativeDirection::RIGHT_TO_LEFT) {
    ...
    gtsam::Matrix numJac =
        computeNumericJacobian(*this, poseForNum, &poseForNum, direction);
    assertNumericJac(numJac, returning);
#endif
    return returning;
  }
  return PoseTransformation::getPoseDerivative(
      pose, direction); // Use numeric derivative if not implemented.
}

// Exchanges rotation and translation (the first 6 rows/columns) in a Jacobian
// matrix.
...
void dmvio::convertAllPosesWithTransform(
    const gtsam::Values &values, const PoseTransformation &transformation,
  ...
  for (int i = 0; i < keysToInclude.size(); ++i) {
    ...
    if (gtsam::Symbol(key).chr() == 'p') {
      ...
      insertInto.insert(
          key, gtsam::Pose3(transformation.transformPose(pose.matrix())));
    } else {
...
std::pair<gtsam::Matrix, gtsam::Vector> dmvio::convertHAndBFromDSO(
    const dso::MatXX &HInput, const dso::VecX &bInput,
    PoseTransformation &poseTransformation, double weightDSOToGTSAM,
    const gtsam::Ordering &ordering, const gtsam::Values &values,
  ...
  // Here we use the fact that the Hessian from DSO will always contain first
  // calibration parameters, and then poses (each followed by a and b).
  int numCPARS = 0;
  ...
  // For converting the Hessian the derivative is always the other way round, so
  // here we have to pass RIGHT_TO_LEFT.
  auto pair = convertHAndBWithPoseTransformation(
      std::make_pair(H, b), ordering, keyDimMap, values, poseTransformation,
...
dmvio::convertHAndBWithPoseTransformation(
  ...
  gtsam::Matrix bigJ =
      buildRelativeJacobian(n, ordering, sizes, values, poseTransformation,
                            derivativeDirection, numOpt, optPositions);
...
gtsam::Matrix dmvio::buildRelativeJacobian(
    int n, const gtsam::FastVector<gtsam::Key> &ordering,
  ...
  for (int i = 0; i < ordering.size(); ++i) {
    ...
    if (chr == 'p') // Only convert poses.
    {
      ...
      std::vector<Matrix> derivatives = poseTransformation.getAllDerivatives(
          pose.matrix(), derivativeDirection);
...
std::pair<gtsam::Matrix, gtsam::Vector>
dmvio::convertCoarseHToGTSAM(PoseTransformation &transform,
                             const dso::Mat88 &HInput, const dso::Vec8 &bInput,
                             const gtsam::Pose3 &currentPose) {
  // Correct order (switch translation and rotation and put the affine
  // parameters to the front!) The Sophus twist constains first translation and
  // then rotation. The GTSAM Pose3 contains first rotation and then
  // translation!
  gtsam::Matrix8 H;
  gtsam::Vector8 b;
