...
GetValidateError_En(ValidateErrorCode validateErrorCode) {
  ...
  case kValidateErrorMultipleOf:
    return RAPIDJSON_ERROR_STRING("Number '%actual' is not a multiple of the "
                                  "'multipleOf' value '%expected'.");
  case kValidateErrorMaximum:
    return RAPIDJSON_ERROR_STRING(
        "Number '%actual' is greater than the 'maximum' value '%expected'.");
  case kValidateErrorExclusiveMaximum:
    return RAPIDJSON_ERROR_STRING(
        "Number '%actual' is greater than or equal to the 'exclusiveMaximum' "
        "value '%expected'.");
  case kValidateErrorMinimum:
    return RAPIDJSON_ERROR_STRING(
        "Number '%actual' is less than the 'minimum' value '%expected'.");
  case kValidateErrorExclusiveMinimum:
    return RAPIDJSON_ERROR_STRING("Number '%actual' is less than or equal to "
                                  "the 'exclusiveMinimum' value '%expected'.");
  ...
  case kValidateErrorMaxItems:
    return RAPIDJSON_ERROR_STRING("Array of length '%actual' is longer than "
                                  "the 'maxItems' value '%expected'.");
  case kValidateErrorMinItems:
    return RAPIDJSON_ERROR_STRING("Array of length '%actual' is shorter than "
                                  "the 'minItems' value '%expected'.");
  case kValidateErrorUniqueItems:
    return RAPIDJSON_ERROR_STRING("Array has duplicate items at indices "
                                  "'%duplicates' but 'uniqueItems' is true.");
  ...
  case kValidateErrorMaxProperties:
    return RAPIDJSON_ERROR_STRING("Object has '%actual' members which is more "
                                  "than 'maxProperties' value '%expected'.");
  case kValidateErrorMinProperties:
    return RAPIDJSON_ERROR_STRING("Object has '%actual' members which is less "
                                  "than 'minProperties' value '%expected'.");
  ...
  case kValidateErrorAdditionalProperties:
    return RAPIDJSON_ERROR_STRING(
        "Object has an additional member '%disallowed' that is not allowed by "
        "the schema.");
  ...
  case kValidateErrorDependencies:
    return RAPIDJSON_ERROR_STRING("Object has missing property or schema "
                                  "dependencies, refer to following errors.");

  case kValidateErrorEnum:
    return RAPIDJSON_ERROR_STRING("Property has a value that is not one of its "
                                  "allowed enumerated values.");
  case kValidateErrorType:
    return RAPIDJSON_ERROR_STRING("Property has a type '%actual' that is not "
                                  "in the following list: '%expected'.");

  case kValidateErrorOneOf:
    return RAPIDJSON_ERROR_STRING(
        "Property did not match any of the sub-schemas specified by 'oneOf', "
        "refer to following errors.");
  case kValidateErrorOneOfMatch:
    return RAPIDJSON_ERROR_STRING(
        "Property matched more than one of the sub-schemas specified by "
        "'oneOf', indices '%matches'.");
  ...
  case kValidateErrorReadOnly:
    return RAPIDJSON_ERROR_STRING("Property is read-only but has been provided "
                                  "when validation is for writing.");
  case kValidateErrorWriteOnly:
    return RAPIDJSON_ERROR_STRING("Property is write-only but has been "
                                  "provided when validation is for reading.");
...
/*!
    \ingroup RAPIDJSON_ERRORS
    \param schemaErrorCode Error code obtained from compiling the schema
   document. \return the error message. \note User can make a copy of this
   function for localization. Using switch-case is safer for future modification
   of error codes.
*/
inline const RAPIDJSON_ERROR_CHARTYPE *
GetSchemaError_En(SchemaErrorCode schemaErrorCode) {
  switch (schemaErrorCode) {
  case kSchemaErrorNone:
    return RAPIDJSON_ERROR_STRING("No error.");

  case kSchemaErrorStartUnknown:
    return RAPIDJSON_ERROR_STRING("Pointer '%value' to start of schema does "
                                  "not resolve to a location in the document.");
  case kSchemaErrorRefPlainName:
    return RAPIDJSON_ERROR_STRING(
        "$ref fragment '%value' must be a JSON pointer.");
  ...
  case kSchemaErrorRefPointerInvalid:
    return RAPIDJSON_ERROR_STRING("$ref fragment '%value' is not a valid JSON "
                                  "pointer at offset '%offset'.");
  ...
  case kSchemaErrorRefNoRemoteSchema:
    return RAPIDJSON_ERROR_STRING("$ref '%value' is remote but the remote "
                                  "provider did not return a schema.");
  case kSchemaErrorRegexInvalid:
    return RAPIDJSON_ERROR_STRING("Invalid regular expression '%value' in "
...
GetPointerParseError_En(PointerParseErrorCode pointerParseErrorCode) {
  ...
  case kPointerParseErrorCharacterMustPercentEncode:
    return RAPIDJSON_ERROR_STRING(
        "A character must be percent encoded in a URI fragment.");
