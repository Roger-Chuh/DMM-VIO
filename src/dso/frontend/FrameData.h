...
inline bool camJsonLoad(const std::string& jsonPath, int camId, CameraJsonData& camJson) {
...
inline bool imuJsonLoad(const std::string& jsonPath, ImuJsonData* imuJson) {
  ...
  for (std::size_t i = 0; i < IMUData.Size() - 0; i++) {
    ...
    imuJson->acc_data.back().timestamp = static_cast<number_t>(timestamp) * 1e-9;
    ...
    imuJson->gyro_data.back().timestamp = static_cast<number_t>(timestamp) * 1e-9;
...
inline bool camJsonAlign(std::map<int, CalibIO::CameraJsonData>& allCamJson, int64_t minGap = 1000000,
                         int64_t frameGap = 30000000) {  // unit ns
  if (allCamJson.size() < 2) return true;

  int idx = 0;
  for (auto it = allCamJson.begin()->second.frames.begin(); it != allCamJson.begin()->second.frames.end();) {
    ...
    for (auto map_it = std::next(allCamJson.begin()); map_it != allCamJson.end(); ++map_it) {
      int camId = map_it->first;
      if (idx < map_it->second.frames.size()) {
        timeDiff.emplace_back(camId, map_it->second.frames[idx].timestamp - cam0Time);
      } else {
        dataOver = true;
        break;
      }
    ...
    if (dataOver) {
      for (auto& [camId, oneCamJson] : allCamJson) {
        if (oneCamJson.frames.size() > idx) {
          ...
          oneCamJson.frames.erase(oneCamJson.frames.begin() + idx, oneCamJson.frames.end());
        }
      }
      return true;
    }
    ...
    for (auto& i : timeDiff) {
      ...
      if (std::abs(data) <= minGap) inCount.emplace_back(temp);
      if (std::abs(data) > minGap && std::abs(data) <= frameGap / 3) outCount.emplace_back(temp);
    ...
    if (wrongCount.empty()) {
      if (!outCount.empty()) {
        ...
        for (auto map_it = std::next(allCamJson.begin()); map_it != allCamJson.end(); ++map_it) {
          LOG_FRONT_WARN("Drop other can't align camera frame: %s, cam %d\n",
                         map_it->second.frames[idx].filename.c_str(), map_it->second.camId);
      ...
      for (const std::pair<int, int64_t>& oneCam : wrongCount) {
        ...
        allCamJson[oneCam.first].frames.erase(allCamJson[oneCam.first].frames.begin() + idx);
      }
    }
  }
...
struct CurFrameRes {
  ...
  CurFrameRes() : mObjectPointSets(), mImagePointSets(), mGridId(), enough_points(false), frameName() {}
  ~CurFrameRes() = default;
};
...
inline void saveCamFrames(const std::unordered_map<int, CamFrames>& multiCamFrames, const std::string& filename) {
  ...
  for (const auto& oneCam : multiCamFrames) {
    ...
    for (const auto& frame : camFrames.eachFrameInfo) {
      // Save frameInfo
      file.write(reinterpret_cast<const char*>(&frame.timestamp), sizeof(double));
      file.write(reinterpret_cast<const char*>(&frame.exposure), sizeof(double));
      file.write(reinterpret_cast<const char*>(&frame.gain), sizeof(int));

      // Save frameName
      size_t frameNameSize = frame.frameName.size();
      file.write(reinterpret_cast<const char*>(&frameNameSize), sizeof(size_t));
      file.write(frame.frameName.c_str(), frameNameSize);

      // Save enough_points
      file.write(reinterpret_cast<const char*>(&frame.enough_points), sizeof(bool));

      // Save mObjectPointSets
      size_t mObjectPointSetsSize = frame.mObjectPointSets.size();
      file.write(reinterpret_cast<const char*>(&mObjectPointSetsSize), sizeof(size_t));
      for (const auto& pair : frame.mObjectPointSets) {
        ...
        for (const auto& vec : pair.second) {
          file.write(reinterpret_cast<const char*>(vec.data()), sizeof(double) * vec.size());
        }
      }

      // Save mImagePointSets
      size_t mImagePointSetsSize = frame.mImagePointSets.size();
      file.write(reinterpret_cast<const char*>(&mImagePointSetsSize), sizeof(size_t));
      ...
      for (const auto& onePair : frame.mGridId) {
        ...
        file.write(reinterpret_cast<const char*>(&boardid), sizeof(int));
        file.write(reinterpret_cast<const char*>(&vecSize), sizeof(size_t));
        file.write(reinterpret_cast<const char*>(onePair.second.data()), sizeof(int) * vecSize);
      }
    }
  }
...
inline bool loadCamFrames(const std::string& filename, std::unordered_map<int, CamFrames>& multiCamFrames) {
  ...
  for (size_t idx = 0; idx < mapSize; ++idx) {
    ...
    for (size_t i = 0; i < numFrames; ++i) {
      ...
      file.read(reinterpret_cast<char*>(&mObjectPointSetsSize), sizeof(size_t));
      for (size_t j = 0; j < mObjectPointSetsSize; ++j) {
        ...
        for (size_t k = 0; k < vecSize; ++k) {
          Eigen::Vector3d v;
          file.read(reinterpret_cast<char*>(v.data()), sizeof(double) * v.size());
          vec[k] = v;
        }
