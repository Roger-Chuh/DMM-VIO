...
// Information the Coarse tracking needs from the previous BA optimization when
// the KF changes.
struct InformationBAToCoarse {
  ...
  // Transform poses from IMU to coarse DSO poses. Typically of type
...
// Responsible for making sure that it always contains the preintegrated data
...
// This class adds velocity, bias, scale and gravity direction as additional
...
class BAIMULogic : public BAExtension {
 public:
  ...
  // factors (group 0) to the delayed graph for IMU reinitialization.
  enum FactorGroups { NO_IMU_GROUP = 0, BIAS_AND_PRIOR_GROUP, METRIC_GROUP };
  ...
  // Takeover result from the IMU initializer. Values contains the new values
  // (DSO variables will not replaced though). If reinit this is a
  ...
  // tracking reference changed ->
  // -> in other thread marginalization and post BA stuff ->
  // finishKeyframeOperations())
  void finishKeyframeOperations(int keyframeId);

  // Called before finishKeyframeOptimization (and before the IMU init might be
  // called).
 ...
 private:
  ...
  // Prior factor to be used by the CoarseIMULogic. It is obtained by
  // marginalizing everything in the BA graph, except the newest bias and
  // velocity. This is a useful prior for the coarse tracking.
