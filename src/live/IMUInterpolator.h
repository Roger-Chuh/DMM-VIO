...
// designed for cases where accelerometer and gyroscope data are not
// synchronized (meaning they arrive with different timestamps). This means that
// all accelerometer data is interpolated to fit the timestamps of the gyroscope
...
class IMUInterpolator {
 public:
  ...
  // Called when a new image arrives. Will be forwarded to the frameContainer,
 ...
 private:
  ...
  // gyrData. Then for all gyr data and all images, an entry in output is
  // created. The methods insertAccDataIfNecessary and addGyrDataIfNecessary
  // fill in missing IMU data in output by interpolating the nearest measurement
  // inside accData/gyrData.
  ...
  // For all entries in output with accDataSet==false, this method adds an
  // (interpolated) accelerometer measurement if possible.
  void insertAccDataIfNecessary();

  // You should get a lock on mutex before calling this method.
  // For all entries in output with gyrDataSet==false, this method adds an
  // (interpolated) gyroscope measurement if possible.
  ...
  // These images have arrived but could not yet be send to FrameContainer as
  // they are missing IMU data.
  std::deque<Frame> imagesInProcess;

  // Maximum number of IMU measurements stored in accData/gyrData before IMU
  // interpolation.
