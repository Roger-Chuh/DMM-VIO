 ...
 * @brief if |Gx|>|Gy|, it is a proposal of vertical edge(EDGE_VER), or it is
 * horizontal edge(EDGE_HOR)
 */
enum EDGE_DIR { EDGE_HOR, EDGE_VER };

/**
 * @brief: a sign for recording status of each pixel in tracing
 */
enum STATUS { STATUS_UNKNOWN = 0, STATUS_BACKGROUND = 1, STATUS_EDGE = 255 };

/**
 * @brief: Trace direction
 */
enum TRACE_DIR { TRACE_LEFT, TRACE_RIGHT, TRACE_UP, TRACE_DOWN };
 ...
 * @param status [in|out] status record of each pixel, see the definition of
 * STATUS
...
void traceFromAnchor(const cv::Mat& M, const cv::Mat& O, const int proposal_thresh, const cv::Point& anchor,
  ...
  if (O.at<uchar>(anchor.y, anchor.x) == EDGE_HOR) {
    // go left first
    // sssume the last visited point is the right hand side point and TRACE_LEFT
    ...
    // the initial while condition, the same below
