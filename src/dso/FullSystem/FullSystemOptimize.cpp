...
void FullSystem::linearizeAll_Reductor(bool fixLinearization, std::vector<PointFrameResidual*>* toRemove, int min, int max, Vec10* stats, int tid)
{
	...
	{ /// 对每一个host点（landmark）遍历，算出他们的光度误差
...
{
	for(int k=min;k<max;k++) {
        //todo 因为是刚把activeResidual线性化的，所以这里还是为activeResidual拷贝雅可比和residual
...
/// this includes newest keyframe, and newly triangulated PointHesiian in older keyframes
/// 所谓的activeResiduals是指允许他自由relinearize，而不是fix linearization point
	...
	for(FrameHessian* fh : frameHessians)
		for(PointHessian* ph : fh->pointHessians)
		{
			for(PointFrameResidual* r : ph->residuals)
			{
				...
				{//TODO 这个会一直进入这个判断，只有要marg的点才会线性化残差，它的残差使用fej状态求的，不需要用最新状态
					activeResiduals.push_back(r);// 新加入的残差 //TODO r中包含host target帧id，uv，idepth，Jac这些信息
...
//[ ***step 2*** ] 线性化activeResiduals的残差, 计算边缘化的能量值 (然而这里都设成0了)
	...
	Vec3 lastEnergy = linearizeAll(false);//TODO 这里是第一次线性化，后面还没有优化，所以还没有剔除点的过程，对应pdf里“先第一次统一构建，再第二次里提出误差大的点”的说辞
    //? 和linearizeAll计算的有啥区别
	double lastEnergyL = calcLEnergy();// islinearized的量的能量 //TODO 还能通过显式的残差构建来算energy，部分状态用的是Fej（idp，pose，camera），部分状态用的是最新估计（gradient，ab），但host帧的b0用的是fej，算是一个比较强的prior吧
	double lastEnergyM = calcMEnergy(false);// HM部分的能量 //TODO 指被marg掉的那些帧的残差，已经没有显式的残差构建了，只有H和m，

    printf("********* energies: [%f %f %f]\n", lastEnergy(0), lastEnergyL, lastEnergyM);


// 把线性化的结果给efresidual //TODO 刚刚首次计算的那些Jac，因为刚刚fixLinearization = false，都是船新的状态
	if(multiThreading)
...
//	double lambda = 1e-1;
	...
	for(int iteration=0;iteration<mnumOptIts;iteration++)
	{
        ...
        // 因为优化迭代还未完成，要继续更新线性化点（对梯度，ab是这样，pose idp camera的雅可比不会变）【NEED TO PRINT SOM LOG】
		...
		else
		{
		    //TODO 不接受, roll back
			loadSateBackup();
			lastEnergy = linearizeAll(false);//TODO 理论上为了少算一次，应该把上次迭代的这部分变量也buffer住的，这里因为已经覆盖，只能重新计算一次
    ...
    //TODO 因为时relative pose，所以只有01，12，23，34，45，以及56，56的connection是新加的，所以不用fix linearization point，理解的对吗？ (scratch that)
    //TODO 代码和我的注释好像南辕北辙啊
	...
	frameHessians.back()->setEvalPT(frameHessians.back()->PRE_worldToCam,
			newStateZero); //TODO 最新帧设置为线性化点, 待估计量, and calculate nullspace
	EFDeltaValid=false;
	EFAdjointsValid=false;
	ef->setAdjointsF(&Hcalib);//TODO 重新计算adj 因为新加入帧了，重新计算一下伴随的东西
	setPrecalcValues();//TODO 更新增量 after optimization 只要位姿更新了，就一定要重新计算以下这些相对位姿的值
    ...
    //* 把优化结果给shell
	{
		...
		for(FrameHessian* fh : frameHessians)
		{
            ...
            //TODO  存的不是线性化点的位姿，而是最新估计的状态，frameShell可理解成frameHessian的简化版
		}
	}
