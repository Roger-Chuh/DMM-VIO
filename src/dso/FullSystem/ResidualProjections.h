...
EIGEN_STRONG_INLINE bool projectPoint(const float& u_pt, const float& v_pt, const float& idepth, const int& dx,
  ...
  /// depth in new frame, or you can say, "predicted depth", its actually =
  /// 1/t[2]*id = d/t[2]
  drescale = 1.0f / ptp[2];        // target帧逆深度 比 host帧逆深度 (scratch that)
  new_idepth = idepth * drescale;  // 新的帧上逆深度 (scratch that)
  ...
  return Ku > 1.1f && Kv > 1.1f && Ku < wG[lvl_target] - 3 && Kv < hG[lvl_target] - 3;
}
