...
#define RAPIDJSON_DEFAULT_ALLOCATOR                                            \
  ::RAPIDJSON_NAMESPACE::MemoryPoolAllocator<                                  \
  ...
  */
  ...
  operator[](const GenericValue<Encoding, SourceAllocator> &name) {
    ...
    if (member != MemberEnd())
      ...
      alignas(
          GenericValue) thread_local static char buffer[sizeof(GenericValue)];
      ...
      return *new (buffer) GenericValue();
                                   ...
                                   // This will generate -Wexit-time-destructors
                                   // in clang, but that's
          // better than having under-alignment.
          __thread static GenericValue buffer;
          return buffer;
#else
                                   // Don't know what compiler this is, so don't
                                   // know how to ensure
          // thread-locality.
...
private:
  ...
  struct MapTraits {
    ...
    typedef std::multimap<Data, SizeType, Less, StdAllocator<Pair, Allocator>>
        Map;
  ...
  Map *&DoReallocMap(Map **oldMap, SizeType newCapacity, Allocator &allocator) {
    Map **newMap =
        static_cast<Map **>(allocator.Malloc(GetMapLayoutSize(newCapacity)));
...
public:
  ...
  typedef StackAllocator
      StackAllocatorType; //!< StackAllocator type from template parameter.
