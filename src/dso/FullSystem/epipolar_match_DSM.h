...
class EpipolarMatchDSM {
 public:
  enum MatchResult { kMatchGood = 0, kMatchDuplicate, kMatchFail, kMatchReject };
  ...
  EpipolarMatchDSM(MultiCamera* p_level_to_multi_camera, const number_t& fov_threshold,
 ...
 private:
  TriResult TriangulateWithCheckTheta(number_t& idp, const Mat4& T01, const Vec3& v0, const Vec3& v1);

  std::vector<size_t> FindLocalMaxima(const std::vector<SearchResult>& search_result_vec);

  bool InFrame(const Vec2& uv, const std::shared_ptr<AlgsImage>& p_img, int border);

  int maxSteps_ = 25;
  number_t rad_step_ = 0.7 * 0.2438 / 180 * M_PI;
  number_t OOB_check_cos_theta_threshold_ = -1;          // FOV Threshold cos(80)
  number_t cos_grad_epipolar_dir_theta_threshold_ = -1;  // duplicate checking threshold cos(80)
