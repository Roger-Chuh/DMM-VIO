...
// weight and height
	...
	for(int y=0;y<h32;y++)
		for(int x=0;x<w32;x++)
		{ /// 一个格子的左上顶点的地址(top right corner of the 32x32 grid)
			...
			{ /// it and jt are coords w.r.t. the whole image
...
/// gradHistFrame的初值是赋成0的，只有调过一次makeHists(fh)，gradHistFrame才会被赋成fh, 所以要加上这个if判断
...
//? 在哪改变的状态 PixelSelectorStatus ?
    //TODO update: here there's 4 channels of status, which make the map_out or SelectionMap
    //             a w*h*4 matrix.
