...
int ComputeMinLineLength(int width, int height) {
  ...
  const number_t logNT = 2.0 * (std::log10((number_t)width) + std::log10((number_t)height));
...
//
void LineFit(number_t* x, number_t* y, int count, number_t& a, number_t& b, number_t& e, int& invert) {
  if (count < 2) return;
...
//
void LineFit(number_t* x, number_t* y, int count, number_t& a, number_t& b, int invert) {
  if (count < 2) return;
...
number_t ComputeMinDistance(number_t x1, number_t y1, number_t a, number_t b, int invert) {
  number_t x2, y2;
...
//
void ComputeClosestPoint(number_t x1, number_t y1, number_t a, number_t b, int invert, number_t& xOut, number_t& yOut) {
  number_t x2, y2;
...
//
void SplitSegment2Lines(number_t* x, number_t* y, int noPixels, int segmentNo, std::vector<LineSegment>& lines,
                        int min_line_len = MIN_LINE_LENGTH, number_t line_error = MAX_LINE_ERROR) {
  lines.clear();
...
// TODO description
void GroupLines2PolygonalChains(const number_t* x, const number_t* y, int noPixels, int segmentNo,
                                const std::vector<LineSegment>& lines, std::vector<PolygonalChain>& chains) {
  chains.clear();
...
void ComputeClosestPoint(number_t x, number_t y, MultiCamera* p_multi_camera, const int& cid, const Vec3& axis,
                         number_t& xOut, number_t& yOut) {
  Vec3 p;
  ...
  Vec3 v = p - p.dot(axis) * axis;  // direction vector projected onto fitted plane
...
// TODO Description
void ValidatePolygonalChain(const number_t* x, const number_t* y, int segmentNo, MultiCamera* p_multi_camera,
                            const int& cid, const std::vector<PolygonalChain>& chains,
  ...
  for (const PolygonalChain& chain : chains) {
    ...
    for (int index = 0; index < noLines; index++) {
      ...
      if (noLines > 1) {
        for (int k = 0; k < line.len; k++) {
          ...
          p_multi_camera->cid_to_cam.at(cid)->UnProject(Vec2(x[line.firstPixelIndex + k], y[line.firstPixelIndex + k]),
                                                        v);
          // v /= v(2);
    ...
    if (noLines > 1) {
      for (int index = 0; index < noLines; index++) {
        ...
        Vec3 s =
            endpoints[2 * index] - endpoints[2 * index].dot(axis) * axis;  // start point projected onto fitted plane
        ...
        Vec3 e = endpoints[2 * index + 1] -
                 endpoints[2 * index + 1].dot(axis) * axis;  // end point projected onto fitted plane
    ...
    for (int line_index = 0; line_index <= noLines; line_index++) {
      ...
      for (int k = 0; k < gap.len; k++) {
        ...
        number_t _x, _y;
        ComputeClosestPoint(x[index], y[index], p_multi_camera, cid, axis, _x, _y);
      ...
      if (line_index < noLines) {
        ...
        for (int k = 0; k < line.len; k++)
          distortedLineSegment.segment.emplace_back(
              cv::Point(x[line.firstPixelIndex + k], y[line.firstPixelIndex + k]));
      }
    }
...
void dlsd(const cv::Mat& image, MultiCamera* p_multi_camera, const int& cid,
  ...
  ED ed = ED(image, SOBEL_OPERATOR, 5, 8, 1, 10, 1.0, true);  // apply ED algorithm
  ...
  for (const std::vector<cv::Point>& segment : ed.getSortedSegments()) {
    ...
    for (const PolygonalChain& chain : chains) {
      ...
      for (int index : chain.lines) {
        ...
        for (const GapSegment& gap : chain.gaps) {
          for (int k = 0; k < gap.len; k++)
            cv::circle(imageChains, cv::Point2d(x[gap.firstPixelIndex + k], y[gap.firstPixelIndex + k]), 2,
                       cv::Scalar(0, 200, 0), -1);
        }
      }
    }
    ...
    ValidatePolygonalChain(x, y, segmentNo, p_multi_camera, cid, chains, lines, distortedLineSegments);
