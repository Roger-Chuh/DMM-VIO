...
class GLine2D {
 public:
  ...
  //! the point on the that is perpendicular toa line passing through the origin
  //! and the line.
   ...
   * a<=b<=c. This is implemented by computing the dot product of the vector 'p'
   * with the line's direct unit vector.
   */
 ...
 private:
  float dx, dy;
  std::pair<float, float> p;  //!< A point the line passes through; when normalized, it is the point
                              //!< closest to the origin (hence perpendicular to the line)
