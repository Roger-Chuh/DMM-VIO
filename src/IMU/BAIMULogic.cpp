...
BAIMULogic::BAIMULogic(PreintegrationProviderBA* preintegrationProvider, BAGTSAMIntegration* baIntegration,
  ...
  baIntegration->setDynamicDSOWeightCallback([this](double lastDSOEnergy, double lastRMSE, bool coarseTrackingWasGood) {
    return computeDynamicDSOWeight(lastDSOEnergy, lastRMSE, coarseTrackingWasGood);
  });
...
void dmvio::BAIMULogic::updateBAOrdering(std::vector<dso::EFFrame*>& frames, gtsam::Ordering* ordering,
                                         KeyDimMap& baDimMap) {
  ...
  if (imuSettings.setting_optIMUExtrinsics && optimizeIMUExtrinsics /*|| true*/) {
...
void dmvio::BAIMULogic::addFirstBAFrame(int keyframeId, BAGraphs* baGraphs, gtsam::Values::shared_ptr baValues) {
  ...
  if (imuSettings.setting_prior_bias) {
    gtsam::noiseModel::Diagonal::shared_ptr bias_prior_model =
        gtsam::noiseModel::Diagonal::Variances((gtsam::Vector(6) << 1e-2, 1e-2, 1e-2, 1e-3, 1e-3, 1e-3).finished());
    baGraphs->addFactor(boost::make_shared<gtsam::PriorFactor<gtsam::imuBias::ConstantBias>>(
  ...
  if (imuSettings.setting_prior_velocity) {
    ...
    baGraphs->addFactor(
        boost::make_shared<gtsam::PriorFactor<gtsam::Vector3>>(vel_current_key, initialVelocity, velocity_prior_model),
        BIAS_AND_PRIOR_GROUP);
  }
  ...
  previousKeyframeId = keyframeId;  // Set so that the next call of addKeyframeToBA knows about
                                    // this first frame.
...
void dmvio::BAIMULogic::addKeyframe(BAGraphs* baGraphs, gtsam::Values::shared_ptr baValues, int keyframeId,
  ...
  if (lastKeyframeId == -1 || lastKeyframeId == keyframeId) {
    ...
    gtsam::NonlinearFactor::shared_ptr imuFactor(
        new gtsam::ImuFactor(prevPoseKey, prevVelKey, currPoseKey, currVelKey, prevBiasKey, imuMeasurements));
    ...
    // our factor graph.
    auto transformedFactor = boost::make_shared<PoseTransformationFactor>(imuFactor, *transformDSOToIMU,
    ...
    gtsam::NonlinearFactor::shared_ptr biasFactor(new gtsam::BetweenFactor<gtsam::imuBias::ConstantBias>(
        prevBiasKey, currBiasKey, gtsam::imuBias::ConstantBias(gtsam::Vector3::Zero(), gtsam::Vector3::Zero()),
        biasNoiseModel));
...
bool dmvio::BAIMULogic::postSolve(gtsam::Values::shared_ptr values, gtsam::Values::shared_ptr newValues,
                                  const gtsam::Vector& inc, const gtsam::Ordering& ordering,
  ...
  std::map<char, double> thresholds = {{'v', 0.0001}, {'b', 0.0005}, {'s', 0.0005}, {'g', 0.0005}, {'i', 0.0005}};
  int pos = 0;
...
void dmvio::BAIMULogic::finishKeyframeOperations(int keyframeId) {
  ...
  gtsam::imuBias::ConstantBias currentBias = baValues->at<gtsam::imuBias::ConstantBias>(gtsam::Symbol('b', keyframeId));
...
gtsam::LinearContainerFactor::shared_ptr BAIMULogic::computeFactorForCoarseGraphAndMarginalCovariances() {
  ...
  gtsam::Ordering uncertOrdering;  // This contains all keys for which we need uncertainty
  ...
  auto graphPair =
      baIntegration->getBaGraphs().getHAndB(*(baIntegration->getBaValues()), ordering, keyDimMap, &additionalKeys);
  ...
  auto accumFun = [&keyDimMap](int num, const gtsam::Key& key) { return num + keyDimMap.at(key); };
  ...
  gtsam::Matrix marginalized =
      computeSchurComplement(augmentedHessianFromPair(std::make_pair(margInputH, margInputB)), mSize, aSize);
  ...
  gtsam::LinearContainerFactor::shared_ptr lcf(
      new gtsam::LinearContainerFactor(gtsam::HessianFactor(factorOrdering, sm), *(baIntegration->getBaValues())));
  return lcf;
}
...
std::unique_ptr<InformationBAToCoarse> BAIMULogic::finishKeyframeOptimization(int keyframeId) {
  ...
  returning->transformIMUToDSOForCoarse =
      std::make_unique<TransformIMUToDSOForCoarse<TransformDSOToIMU>>(copy, keyframeId);
...
std::shared_ptr<TransformDSOToIMU> BAIMULogic::getTransformDSOToIMU() const { return transformDSOToIMU; }
...
void BAIMULogic::initFromIMUInit(const gtsam::Values& values, bool reinit, bool willReplaceGraph) {
  ...
  if (!willReplaceGraph && !reinit)  // For reinit we don't need to do this.
  {
    ...
    if (imuSettings.initSettings.scalePriorAfterInit > 0) {
      ...
      gtsam::PriorFactor<ScaleGTSAM>::shared_ptr scalePrior(new gtsam::PriorFactor<ScaleGTSAM>(
          S(0), values.at<ScaleGTSAM>(S(0)), gtsam::noiseModel::Diagonal::Sigmas(scaleModel)));
