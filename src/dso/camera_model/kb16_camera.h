...
public:
  ...
  explicit KB16Camera(CamId camera_id, int width, int height)
  ...
  virtual bool Project(const Vec3 &p_3d, Vec2 &p_img,
                       Eigen::Matrix<number_t, 2, 3> *d_img_d_p3d = nullptr,
                       Eigen::Matrix<number_t, 2, Eigen::Dynamic>
                           *d_img_d_param = nullptr) const override;

  virtual bool Project(const Vec3 &p_3d, Eigen::Ref<Vec2> &p_img,
                       Eigen::Matrix<number_t, 2, 3> *d_img_d_p3d = nullptr,
                       Eigen::Matrix<number_t, 2, Eigen::Dynamic>
                           *d_img_d_param = nullptr) const override;

  virtual bool UnProject(const Vec2 &p_img, Vec3 &p_3d,
                         Eigen::Matrix<number_t, 3, 2> *d_p3d_d_img = nullptr,
...
private:
  ...
  void DistortionLength(
      const Vec2 &xy, Vec2 &uv, Mat2 *d_uv_xy = nullptr,
      Eigen::Matrix<number_t, 2, Eigen::Dynamic> *d_uv_params = nullptr) const;

  void UnDistortionLength(
      const Vec2 &uv, Vec2 &xy, Mat2 *d_xy_uv = nullptr,
      Eigen::Matrix<number_t, 2, Eigen::Dynamic> *d_xy_params = nullptr) const;
