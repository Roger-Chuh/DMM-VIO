...
class KBL16Camera : public CameraBase {
 public:
  ...
  explicit KBL16Camera(CamId camera_id, int width, int height) : CameraBase(camera_id, width, height) {
    camera_model_ = CameraModel::kKBL16;
    kParamLength = 16;
  }

  // parameters: fx, fy, cx, cy, k1, k2, k3, k4, k5, k6, p1, p2, s1, s2, s3, s4
  KBL16Camera(CamId camera_id, int width, int height, const number_t* parameters)
