...
inline void PrintValidatorPointersData(const char *s, const char *d,
                                       unsigned depth) {
  printf("    Sch: %*s'%s'\n    Doc: %*s'%s'\n", depth * 4, " ", s, depth * 4,
         " ", d);
}

inline void PrintValidatorPointersData(const wchar_t *s, const wchar_t *d,
                                       unsigned depth) {
  wprintf(L"    Sch: %*ls'%ls'\n    Doc: %*ls'%ls'\n", depth * 4, L" ", s,
          depth * 4, L" ", d);
}

inline void PrintSchemaIdsData(const char *base, const char *local,
                               const char *resolved) {
  printf("    Resolving id: Base: '%s', Local: '%s', Resolved: '%s'\n", base,
         local, resolved);
}

inline void PrintSchemaIdsData(const wchar_t *base, const wchar_t *local,
...
inline void PrintMethodData(const char *method, const char *s1,
                            const char *s2) {
  printf("%s, Data: '%s', '%s'\n", method, s1, s2);
}

inline void PrintMethodData(const char *method, const wchar_t *s1,
                            const wchar_t *s2) {
...
#define RAPIDJSON_SCHEMA_PRINT(name, ...)                                      \
  internal::Print##name##Data(__VA_ARGS__)
#else
#define RAPIDJSON_SCHEMA_PRINT(name, ...)
#endif
#endif
...
struct Specification {
  ...
  bool IsSupported() const {
    return ((draft >= kDraftMin && draft <= kDraftMax) &&
            ((oapi == kVersionNone) ||
             (oapi >= kVersionMin && oapi <= kVersionMax)));
  }
  SchemaDraft draft;
  OpenApiVersion oapi;
};
...
template <typename SchemaDocumentType> struct SchemaValidationContext {
  ...
  SchemaValidationContext(SchemaValidatorFactoryType &f, ErrorHandlerType &eh,
                          const SchemaType *s, unsigned fl = 0)
      : factory(f), error_handler(eh), schema(s), flags(fl), valueSchema(),
...
public:
  ...
  Schema(SchemaDocumentType *schemaDocument, const PointerType &p,
      ...
      : allocator_(allocator), uri_(schemaDocument->GetURI(), *allocator),
        id_(id, allocator), spec_(schemaDocument->GetSpecification()),
        ...
        exclusiveMaximum_(false), defaultValueLength_(0), readOnly_(false),
        writeOnly_(false), nullable_(false) {
    GenericStringBuffer<EncodingType> sb;
    p.StringifyUriFragment(sb);
    RAPIDJSON_SCHEMA_PRINT(Method, "Schema::Schema", sb.GetString(),
                           id.GetString());
    ...
    // PR #1393
    // Early add this Schema and its $ref(s) in schemaDocument's map to avoid
    // infinite recursion (with recursive schemas), since
    // schemaDocument->getSchema() is always checked before creating a new one.
    // Don't cache typeless_, though.
    if (this != typeless_) {
    ...
    if (spec_.oapi != kVersion20 && spec_.oapi != kVersion30)
      if (const ValueType *v = GetMember(value, GetIdString())) {
        if (v->IsString()) {
          ...
          RAPIDJSON_SCHEMA_PRINT(SchemaIds, id.GetString(), v->GetString(),
                                 id_.GetString());
        }
      }
    ...
    if (const ValueType *v = GetMember(value, GetEnumString())) {
      if (v->IsArray() && v->Size() > 0) {
        ...
        for (ConstValueIterator itr = v->Begin(); itr != v->End(); ++itr) {
          typedef Hasher<EncodingType, MemoryPoolAllocator<AllocatorType>>
              EnumHasherType;
          char buffer[256u + 24];
          MemoryPoolAllocator<AllocatorType> hasherAllocator(buffer,
                                                             sizeof(buffer));
    ...
    if (spec_.oapi != kVersion20 && spec_.oapi != kVersion30)
      if (const ValueType *v = GetMember(value, GetPatternPropertiesString())) {
        ...
        for (ConstMemberIterator itr = v->MemberBegin(); itr != v->MemberEnd();
          ...
          schemaDocument->CreateSchema(
              &patternProperties_[patternPropertyCount_].schema, r, itr->value,
              document, id_);
    ...
    if (const ValueType *v = GetMember(value, GetPatternString()))
      pattern_ = CreatePattern(*v, schemaDocument,
                               p.Append(GetPatternString(), allocator_));
    ...
    if (context.inArray) {
      ...
      if (itemsList_)
        ...
        if (context.arrayElementIndex < itemsTupleCount_)
          ...
          // Must set valueSchema for when kValidateContinueOnErrorFlag is set,
          // else reports spurious type error
          context.valueSchema = typeless_;
          // Must bump arrayElementIndex for when kValidateContinueOnErrorFlag
          // is set
...
private:
  ...
  RegexType *CreatePattern(const ValueType &value, SchemaDocumentType *sd,
                           const PointerType &p) {
    if (value.IsString()) {
      RegexType *r = new (allocator_->Malloc(sizeof(RegexType)))
          RegexType(value.GetString(), allocator_);
      if (!r->IsValid()) {
        sd->SchemaErrorValue(kSchemaErrorRegexInvalid, p, value.GetString(),
  ...
  // Creates parallel validators for allOf, anyOf, oneOf, not and schema
  // dependencies, if required. Also creates a hasher for enums and array
  // uniqueness, if required. Also a useful place to add type-independent error
  // checks.
  bool CreateParallelValidator(Context &context) const {
    ...
    if (validatorCount_) {
      ...
      std::memset(context.validators, 0,
                  sizeof(ISchemaValidator *) * validatorCount_);
    ...
    if (!minimum_.IsNull()) {
      if (minimum_.IsInt64()) {
        if (exclusiveMinimum_ ? i <= minimum_.GetInt64()
          ...
          RAPIDJSON_INVALID_KEYWORD_RETURN(exclusiveMinimum_
  ...
  bool CheckDoubleMultipleOf(Context &context, double d) const {
    ...
    double scaledEpsilon =
        (q + qRounded) * std::numeric_limits<double>::epsilon();
...
public:
  ...
  GetRemoteDocument(const GenericUri<ValueType, AllocatorType> uri,
                    Specification &spec) {
...
public:
  ...
  /*!
     ...
     instance for allocating memory. Can be null. \param pointer An optional
     JSON pointer to the start of the schema document \param spec Optional
     schema draft or OpenAPI version. Used if no specification in document.
  ...
  */
  explicit GenericSchemaDocument(
      ...
      : remoteProvider_(remoteProvider), allocator_(allocator), ownAllocator_(),
        ...
        schemaRef_(allocator, kInitialSchemaRefSize), spec_(spec),
        error_(kObjectType), currentError_() {
    ...
    // We only ever look for '$schema' or 'swagger' or 'openapi' at the root of
    // the document.
  ...
  ~GenericSchemaDocument() {
    ...
    // these may contain some allocator data so clear before deleting
    // ownAllocator_
    uri_.SetNull();
    error_.SetNull();
  ...
  void SchemaErrorValue(const SchemaErrorCode code, const PointerType &location,
                        const Ch *value, SizeType length) {
    currentError_ = GValue(kObjectType);
    currentError_.AddMember(GetValueString(),
                            GValue(value, length, *allocator_).Move(),
                            *allocator_);
    AddCurrentError(code, location);
  }
  ...
  void SchemaErrorPointer(const SchemaErrorCode code,
                          const PointerType &location, const Ch *value,
                          SizeType length, const PointerType &pointer) {
    currentError_ = GValue(kObjectType);
    currentError_.AddMember(GetValueString(),
                            GValue(value, length, *allocator_).Move(),
                            *allocator_);
...
private:
  ...
  void AddErrorInstanceLocation(GValue &result, const PointerType &location) {
    ...
    GValue instanceRef(sb.GetString(),
                       static_cast<SizeType>(sb.GetSize() / sizeof(Ch)),
                       *allocator_);
  ...
  void AddCurrentError(const SchemaErrorCode code,
...
#define RAPIDJSON_STRING_(name, ...)                                           \
  ...
  RAPIDJSON_STRING_(Null, 'n', 'u', 'l', 'l')
  RAPIDJSON_STRING_(SpecUnknown, 'S', 'p', 'e', 'c', 'U', 'n', 'k', 'n', 'o',
                    'w', 'n')
  RAPIDJSON_STRING_(SpecUnsupported, 'S', 'p', 'e', 'c', 'U', 'n', 's', 'u',
                    'p', 'p', 'o', 'r', 't', 'e', 'd')
  RAPIDJSON_STRING_(SpecIllegal, 'S', 'p', 'e', 'c', 'I', 'l', 'l', 'e', 'g',
                    'a', 'l')
  RAPIDJSON_STRING_(StartUnknown, 'S', 't', 'a', 'r', 't', 'U', 'n', 'k', 'n',
                    'o', 'w', 'n')
  RAPIDJSON_STRING_(RefPlainName, 'R', 'e', 'f', 'P', 'l', 'a', 'i', 'n', 'N',
                    'a', 'm', 'e')
  RAPIDJSON_STRING_(RefInvalid, 'R', 'e', 'f', 'I', 'n', 'v', 'a', 'l', 'i',
                    'd')
  RAPIDJSON_STRING_(RefPointerInvalid, 'R', 'e', 'f', 'P', 'o', 'i', 'n', 't',
                    'e', 'r', 'I', 'n', 'v', 'a', 'l', 'i', 'd')
  RAPIDJSON_STRING_(RefUnknown, 'R', 'e', 'f', 'U', 'n', 'k', 'n', 'o', 'w',
                    'n')
  RAPIDJSON_STRING_(RefCyclical, 'R', 'e', 'f', 'C', 'y', 'c', 'l', 'i', 'c',
                    'a', 'l')
  RAPIDJSON_STRING_(RefNoRemoteProvider, 'R', 'e', 'f', 'N', 'o', 'R', 'e', 'm',
                    'o', 't', 'e', 'P', 'r', 'o', 'v', 'i', 'd', 'e', 'r')
  RAPIDJSON_STRING_(RefNoRemoteSchema, 'R', 'e', 'f', 'N', 'o', 'R', 'e', 'm',
                    'o', 't', 'e', 'S', 'c', 'h', 'e', 'm', 'a')
  RAPIDJSON_STRING_(ReadOnlyAndWriteOnly, 'R', 'e', 'a', 'd', 'O', 'n', 'l',
                    'y', 'A', 'n', 'd', 'W', 'r', 'i', 't', 'e', 'O', 'n', 'l',
                    'y')
  RAPIDJSON_STRING_(RegexInvalid, 'R', 'e', 'g', 'e', 'x', 'I', 'n', 'v', 'a',
                    'l', 'i', 'd')
...
#undef RAPIDJSON_STRING_
  ...
  static SchemaDraft GetSchemaDraft(const ValueType &document) {
    static const Ch kDraft03String[] = {
        'h', 't', 't', 'p', ':', '/', '/', 'j', 's', 'o', 'n', '-', 's', 'c',
        'h', 'e', 'm', 'a', '.', 'o', 'r', 'g', '/', 'd', 'r', 'a', 'f', 't',
        '-', '0', '3', '/', 's', 'c', 'h', 'e', 'm', 'a', '#', '\0'};
    static const Ch kDraft04String[] = {
        'h', 't', 't', 'p', ':', '/', '/', 'j', 's', 'o', 'n', '-', 's', 'c',
        'h', 'e', 'm', 'a', '.', 'o', 'r', 'g', '/', 'd', 'r', 'a', 'f', 't',
        '-', '0', '4', '/', 's', 'c', 'h', 'e', 'm', 'a', '#', '\0'};
    static const Ch kDraft05String[] = {
        'h', 't', 't', 'p', ':', '/', '/', 'j', 's', 'o', 'n', '-', 's', 'c',
        'h', 'e', 'm', 'a', '.', 'o', 'r', 'g', '/', 'd', 'r', 'a', 'f', 't',
        '-', '0', '5', '/', 's', 'c', 'h', 'e', 'm', 'a', '#', '\0'};
    static const Ch kDraft06String[] = {
        'h', 't', 't', 'p', ':', '/', '/', 'j', 's', 'o', 'n', '-', 's', 'c',
        'h', 'e', 'm', 'a', '.', 'o', 'r', 'g', '/', 'd', 'r', 'a', 'f', 't',
        '-', '0', '6', '/', 's', 'c', 'h', 'e', 'm', 'a', '#', '\0'};
    static const Ch kDraft07String[] = {
        'h', 't', 't', 'p', ':', '/', '/', 'j', 's', 'o', 'n', '-', 's', 'c',
        'h', 'e', 'm', 'a', '.', 'o', 'r', 'g', '/', 'd', 'r', 'a', 'f', 't',
        '-', '0', '7', '/', 's', 'c', 'h', 'e', 'm', 'a', '#', '\0'};
  ...
  static OpenApiVersion GetOpenApiVersion(const ValueType &document) {
    ...
    static const Ch kVersion30String[] = {'3', '.', '0', '.',
                                          '\0'}; // ignore patch level
    static const Ch kVersion31String[] = {'3', '.', '1', '.',
                                          '\0'}; // ignore patch level
    ...
    // Get the open api version from the swagger / openapi keyword at the
    // supplied location
    typename ValueType::ConstMemberIterator itr =
  ...
  // Get the draft of the schema or the open api version (which implies the
  // draft). Report an error if schema draft or open api version not supported
  // or not recognized, or both in document, and carry on.
  void SetSchemaSpecification(const ValueType &document) {
    ...
    // Use document draft or open api version if present or use spec from
    // constructor
  ...
  void CreateSchemaRecursive(const SchemaType **schema,
    ...
    if (v.GetType() == kObjectType) {
      UriType newid =
          UriType(CreateSchema(schema, pointer, v, document, id), allocator_);
  ...
  const UriType &CreateSchema(const SchemaType **schema,
    ...
    GenericStringBuffer<EncodingType> sb;
    pointer.StringifyUriFragment(sb);
    RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaDocument::CreateSchema",
                           sb.GetString(), id.GetString());
    if (v.IsObject()) {
  ...
  bool HandleRefSchema(const PointerType &source, const SchemaType **schema,
    ...
    RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaDocument::HandleRefSchema",
                           sb.GetString(), id.GetString());
    ...
    if (itr->value.IsString()) {
      ...
      if (len == 0)
        ...
        RAPIDJSON_SCHEMA_PRINT(SchemaIds, id.GetString(),
                               itr->value.GetString(), ref.GetString());
        // See if the resolved $ref minus the fragment matches a resolved id in
        // this document Search from the root. Returns the subschema in the
        ...
        if (!base) {
          ...
          if (!remoteProvider_)
            ...
            if (const GenericSchemaDocument *remoteDocument =
              ...
              if (len <= 1 || s[1] == '/') {
                ...
                if (!pointer.IsValid())
                  SchemaErrorPointer(kSchemaErrorRefPointerInvalid, source, s,
                                     len, pointer);
                else {
                  // Get the subschema
                  if (const SchemaType *sc =
                    ...
                    SchemaErrorValue(kSchemaErrorRefUnknown, source,
                                     ref.GetString(), ref.GetStringLength());
                }
              } else
                // Plain name fragment, not allowed in remote schema
                SchemaErrorValue(kSchemaErrorRefPlainName, source, s, len);
            } else
              SchemaErrorValue(kSchemaErrorRefNoRemoteSchema, source,
                               ref.GetString(), ref.GetStringLength());
          }
          ...
          if (len <= 1 || s[1] == '/') {
            ...
            if (!relPointer.IsValid())
              SchemaErrorPointer(kSchemaErrorRefPointerInvalid, source, s, len,
                                 relPointer);
            else {
              // Get the subschema
              if (const ValueType *pv = relPointer.Get(*base)) {
                ...
                for (SizeType i = 0; i < relPointer.GetTokenCount(); i++)
                  pointer =
                      pointer.Append(relPointer.GetTokens()[i], allocator_);
                if (IsCyclicRef(pointer))
                  SchemaErrorValue(kSchemaErrorRefCyclical, source,
                                   ref.GetString(), ref.GetStringLength());
                else {
                  // Call CreateSchema recursively, but first compute the
                  // in-scope id for the $ref target as we have jumped there
            ...
            // Search from the base we just established. Returns the subschema
            // in the document and its absolute JSON pointer.
    ...
    if (doc.GetType() == kObjectType) {
      ...
      if (localuri.Match(finduri, full)) {
        RAPIDJSON_SCHEMA_PRINT(Method, "GenericSchemaDocument::FindId (match)",
                               full ? localuri.GetString()
                                    : localuri.GetBaseString());
        ...
        if (m->value.GetType() == kObjectType ||
          ...
          resval = FindId(m->value, finduri, resptr, localuri, full,
                          here.Append(m->name.GetString(),
                                      m->name.GetStringLength(), allocator_));
        }
        if (resval)
          break;
      }
  ...
  */
  GenericSchemaValidator(
      ...
      : schemaDocument_(&schemaDocument), root_(schemaDocument.GetRoot()),
        ...
        valid_(true), flags_(kValidateDefaultFlags), depth_(0) {
    RAPIDJSON_SCHEMA_PRINT(Method,
...
#define RAPIDJSON_SCHEMA_HANDLE_BEGIN_(method, arg1)                           \
  ...
  if ((!BeginValue() && !GetContinueOnErrors()) ||                             \
    ...
    *documentStack_.template Push<Ch>() = '\0';                                \
    ...
    RAPIDJSON_SCHEMA_PRINT(InvalidDocument,                                    \
...
private:
  ...
  GenericSchemaValidator(
      ...
      const char *basePath, size_t basePathSize, unsigned depth,
      StateAllocator *allocator = 0,
      size_t schemaStackCapacity = kDefaultSchemaStackCapacity,
      size_t documentStackCapacity = kDefaultDocumentStackCapacity)
      : schemaDocument_(&schemaDocument), root_(root),
        ...
        documentStack_(allocator, documentStackCapacity), outputHandler_(0),
        error_(kObjectType), currentError_(), missingDependents_(),
        valid_(true), flags_(kValidateDefaultFlags), depth_(depth) {
    RAPIDJSON_SCHEMA_PRINT(
        Method, "GenericSchemaValidator::GenericSchemaValidator (internal)",
  ...
  bool EndValue() {
    ...
    *documentStack_.template Push<Ch>() = '\0';
    documentStack_.template Pop<Ch>(1);
    RAPIDJSON_SCHEMA_PRINT(ValidatorPointers, sb.GetString(),
  ...
  RAPIDJSON_FORCEINLINE void PushSchema(const SchemaType &schema) {
    new (schemaStack_.template Push<Context>())
        Context(*this, *this, &schema, flags_);
  }
