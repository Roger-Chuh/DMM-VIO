...
public:
  ImageProcessing(double grid_spacing, const Eigen::Vector2i &grid_size,
                  uint32_t seed, int plate_num = 3, int unique_size = 6);

  void ProcessPic(const cv::Mat &grayscale_image,
  ...
  static void gradientMat(const cv::Mat &I, std::vector<Vec2> &grad) {
    for (int curRow = 1; curRow < I.rows - 1; ++curRow) {
      for (int curCol = 1; curCol < I.cols - 1; ++curCol) {
        ...
        pOut[0] = number_t(I.at<unsigned char>(curRow, curCol + 1)) -
                  number_t(I.at<unsigned char>(curRow, curCol - 1));
        pOut[1] = number_t(I.at<unsigned char>(curRow + 1, curCol)) -
                  number_t(I.at<unsigned char>(curRow - 1, curCol));
      }
    }
  }

  template <typename TI, typename TintI, typename Tout>
  void AdaptiveThreshold(int w, int h, const TI *image, const TintI *intImage,
                         Tout *out, float threshold, int rad, int min_diff,
                         Tout pass, Tout fail) {
    ...
    for (int j = 0; j < h; ++j) {
      ...
      for (int i = 0; i < w; ++i) {
        ...
        const TintI sum =
            intIy2[x2] - intIy1m1[x2] - intIy2[x1 - 1] + intIy1m1[x1 - 1];
        const float avg = sum / count;
        unsigned id = j * w + i;
        out[id] =
            (image[id] < threshold * (avg - (float)min_diff)) ? pass : fail;
      }
    }
  }
