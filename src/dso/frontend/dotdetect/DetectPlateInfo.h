...
class DetectPlateInfo {
 ...
 public:
  DetectPlateInfo(std::vector<Vertex*>& vs, const ParamsImageProcessing& params, const cv::Mat* img)
      : vs_(vs), params_(params), img_(img) {}
  DetectPlateInfo(std::set<Vertex*>& vs, const ParamsImageProcessing& params, const cv::Mat* img)
  ...
  bool GetVerNeighborsAndCenter() {
    ...
    std::vector<Vertex*> nearestNeighbors = tree.searchNearestNeighbors(center, std::max(100, (int)vs_.size()));
...
#ifdef DEBUGDOT
    std::cerr << "No central point found. nearestNeighbors: " << nearestNeighbors.size() << std::endl;
    cv::Mat tripleMat;
    cv::cvtColor(*img_, tripleMat, cv::COLOR_GRAY2BGR);
    cv::rectangle(tripleMat, cv::Point(central_->conic->bbox.x1, central_->conic->bbox.y1),
                  cv::Point(central_->conic->bbox.x2, central_->conic->bbox.y2), cv::Scalar(255, 0, 0));
  ...
  bool GetMapGridEllipse() {
    if (central_ == nullptr || principle_.empty()) return false;
    // Search structures
    ...
    for (const auto& label : map_grid_ellipse_) {
      cv::rectangle(boxMat, cv::Point2i(label.second->conic->bbox.x1, label.second->conic->bbox.y1),
                    cv::Point2i(label.second->conic->bbox.x2, label.second->conic->bbox.y2), cv::Scalar(0, 0, 255));
    }
  ...
  bool FindBoards(std::vector<std::set<Vertex*>>& multiPlate) {
    ...
    size_t k = 9;  // include self
    for (Vertex* ver : vs_) {
      std::vector<Vertex*> nearestNeighbors = tree.searchNearestNeighbors(*ver, k);
      GetNeighborsTriples(nearestNeighbors, params_.max_line_dist_ratio, params_.max_norm_triple_area, img_);
    ...
    for (Vertex* oneVer : vs_) {
      for (Triple& oneTri : oneVer->triples)
        cv::line(lineImg, cv::Point((int)oneTri.Neighbour(0).pc.x(), (int)oneTri.Neighbour(0).pc.y()),
                 cv::Point((int)oneTri.Neighbour(1).pc.x(), (int)oneTri.Neighbour(1).pc.y()), cv::Scalar(0, 0, 255), 1);
      if (vertConnectCount.count(oneVer) > 0 && vertConnectCount.at(oneVer) > 1) {
        cv::circle(lineImg, cv::Point((int)oneVer->pc.x(), (int)oneVer->pc.y()), 2, cv::Scalar(0, 255, 0), 2);
      }
    }
    ...
    for (std::pair<Vertex* const, int> oneVertCount : vertConnectCount) {
      if (oneVertCount.second <= 1 || oneVertCount.first->used || oneVertCount.first->triples.size() < 2) continue;
      ...
      while (!queueDfs.empty()) {
        ...
        for (Triple& triple : curVer->triples) {
          for (int i = 0; i < 2; ++i) {
#ifdef DEBUGDOT
            cv::line(lineImg, cv::Point((int)triple.Neighbour(0).pc.x(), (int)triple.Neighbour(0).pc.y()),
                     cv::Point((int)triple.Neighbour(1).pc.x(), (int)triple.Neighbour(1).pc.y()),
                     cv::Scalar(0, 155, 0));
#endif
            if (vertConnectCount.count(&triple.Neighbour(i)) > 0 && vertConnectCount.at(&triple.Neighbour(i)) > 1 &&
                !triple.Neighbour(i).used) {
              triple.Neighbour(i).used = true;
              queueDfs.push(&triple.Neighbour(i));
              onePlate.insert(&triple.Neighbour(i));
#ifdef DEBUGDOT
              cv::circle(lineImg, cv::Point((int)curVer->pc.x(), (int)curVer->pc.y()), 2, cv::Scalar(0, 0, 255));
...
//              cv::waitKey(0);
      ...
      if (onePlate.size() < 15) continue;
      multiPlate.emplace_back(onePlate);

#ifdef DEBUGDOT
      for (Vertex* oneVer : onePlate) {
        cv::circle(lineImg, cv::Point((int)oneVer->pc.x(), (int)oneVer->pc.y()), 3, cv::Scalar(0, 0, 255), 2);
      }
      cv::imshow("plate img", lineImg);
      cv::waitKey(0);
#endif
 ...
 private:
  ...
  static bool GetNeighborsTriples(std::vector<Vertex*>& nearestNeighbors, double thresh_dist, double thresh_area,
                                  const cv::Mat* img) {
    if (nearestNeighbors.size() < 3) return false;
    // check neighbor exactly
    std::vector<bool> used;
    used.resize(nearestNeighbors.size(), false);  // pretend overload
    double minDist = (nearestNeighbors[0]->pc - nearestNeighbors[1]->pc).norm() * 2;

#ifdef DEBUGDOTLINE
    cv::Mat lineMat;
    cv::cvtColor(*img, lineMat, cv::COLOR_GRAY2BGR);
    cv::rectangle(lineMat, cv::Point2i(nearestNeighbors[0]->conic->bbox.x1, nearestNeighbors[0]->conic->bbox.y1),
                  cv::Point2i(nearestNeighbors[0]->conic->bbox.x2, nearestNeighbors[0]->conic->bbox.y2),
                  cv::Scalar(0, 255, 0));
    for (int i = 1; i < nearestNeighbors.size(); ++i) {
      cv::rectangle(lineMat, cv::Point2i(nearestNeighbors[i]->conic->bbox.x1, nearestNeighbors[i]->conic->bbox.y1),
                    cv::Point2i(nearestNeighbors[i]->conic->bbox.x2, nearestNeighbors[i]->conic->bbox.y2),
                    cv::Scalar(255, 0, 0));
    }
#endif

    for (int n1 = 1; n1 < nearestNeighbors.size(); ++n1) {
      const double& d1 = (nearestNeighbors[n1]->pc - nearestNeighbors[0]->pc).norm();
      for (int n2 = n1 + 1; n2 < nearestNeighbors.size(); ++n2) {
        if (used[n1] || used[n2]) continue;
        const double& d2 = (nearestNeighbors[n2]->pc - nearestNeighbors[0]->pc).norm();
        if (d2 > minDist) {
          used[n2] = true;
          continue;
        }
        if (!used[n1] && !used[n2] &&
            NormArea(nearestNeighbors[n1]->pc, nearestNeighbors[0]->pc,
                     nearestNeighbors[n2]->pc) < thresh_area) {             // Check points are colinear with center
          if (2.0 * fabs(d2 - d1) / (fabs(d1) + fabs(d2)) < thresh_dist) {  // Check distances are similar
            used[n1] = true;
            used[n2] = true;
            nearestNeighbors[0]->neighbours.insert(nearestNeighbors[n1]);
            nearestNeighbors[0]->neighbours.insert(nearestNeighbors[n2]);
            nearestNeighbors[0]->triples.emplace_back(*nearestNeighbors[n1], *nearestNeighbors[0],
                                                      *nearestNeighbors[n2]);
#ifdef DEBUGDOTLINE
            cv::line(lineMat, cv::Point2i(nearestNeighbors[0]->pc.x(), nearestNeighbors[0]->pc.y()),
                     cv::Point2i(nearestNeighbors[n1]->pc.x(), nearestNeighbors[n1]->pc.y()), cv::Scalar(0, 0, 255));
            cv::line(lineMat, cv::Point2i(nearestNeighbors[0]->pc.x(), nearestNeighbors[0]->pc.y()),
                     cv::Point2i(nearestNeighbors[n2]->pc.x(), nearestNeighbors[n2]->pc.y()), cv::Scalar(0, 0, 255));
#endif
          }
        }
      }
  ...
  static double NormArea(const Eigen::Vector2d& p1, const Eigen::Vector2d& p2, const Eigen::Vector2d& p3) {
    // Compute signed area
  ...
  static double SignedArea(const Eigen::Vector2d& p1, const Eigen::Vector2d& p2, const Eigen::Vector2d& p3) {
    return p1(0) * (p2(1) - p3(1)) + p2(0) * (p3(1) - p1(1)) + p3(0) * (p1(1) - p2(1));
  }

  static std::set<DotDetect::Vertex*> Neighbours(
      std::map<Eigen::Vector2i const, DotDetect::Vertex*, lessCompare,
               Eigen::aligned_allocator<std::pair<Eigen::Vector2i const, DotDetect::Vertex*>>>& map,
      const Vertex& v) {
