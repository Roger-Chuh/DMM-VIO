...
std::unique_ptr<dmvio::StateTransitionModel> dmvio::createTransitionModel(InitTransitionMode mode,
...
dmvio::CombinedTransitionModel::CombinedTransitionModel(dmvio::IMUInitializerLogic& logic) : logic(logic) {}
...
dmvio::IMUInitializerState::unique_ptr dmvio::CombinedTransitionModel::coarseIMUInitOptimized(
    const gtsam::Values& optimizedValues, dmvio::IMUInitVariances initVariances) {
...
std::pair<bool, IMUInitializerState::unique_ptr> dmvio::CombinedTransitionModel::pgbaOptimized(
    std::unique_ptr<gtsam::Values>& optimizedValues, dmvio::IMUInitVariances initVariances) {
...
std::pair<bool, IMUInitializerState::unique_ptr> dmvio::CombinedTransitionModel::takeOverLargeBAOptim(
...
dmvio::CombinedTransitionModelNoInitialMarginalizationReplacement::
    CombinedTransitionModelNoInitialMarginalizationReplacement(dmvio::IMUInitializerLogic& logic)
    : CombinedTransitionModel(logic) {}
...
dmvio::CombinedTransitionModelNoInitialMarginalizationReplacement::takeOverLargeBAOptim(
    std::unique_ptr<gtsam::Values>& optimizedValues, IMUInitVariances&& initVariances) {
  lastInitUncert = std::move(initVariances);
  return std::make_pair(true, std::make_unique<NoMarginalizationReplacementInitializedFromPGBAState>(
...
dmvio::IMUInitializerState::unique_ptr dmvio::OnlyCoarseIMUInitTransitionModel::coarseIMUInitOptimized(
    const gtsam::Values& optimizedValues, dmvio::IMUInitVariances initVariances) {
  if (!initVariances.indetermined && initVariances.scaleVariance < logic.settings.coarseScaleUncertaintyThresh) {
...
std::pair<bool, IMUInitializerState::unique_ptr> dmvio::OnlyCoarseIMUInitTransitionModel::pgbaOptimized(
    std::unique_ptr<gtsam::Values>& optimizedValues, dmvio::IMUInitVariances initVariances) {
  assert(false);
  return std::make_pair(false, nullptr);
}
