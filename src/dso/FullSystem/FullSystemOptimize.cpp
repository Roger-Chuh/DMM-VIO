...
void FullSystem::setNewFrameEnergyTH() {
  ...
  int nthIdx = setting_frameEnergyTHN *
               allResVec.size(); // 以 setting_frameEnergyTHN 的能量为阈值
...
float FullSystem::optimize(int mnumOptIts) {
  ...
  /// linearization point
  activeResiduals
      .clear(); // TODO 使用前先清零，所以在applyRes时swap也没毛病，不影响啥
  int numPoints = 0;
  int numLRes = 0;
  for (FrameHessian *fh : frameHessians)
    for (PointHessian *ph : fh->pointHessians) {
  ...
  for (int iteration = 0; iteration < mnumOptIts; iteration++) {
    ...
    if (imuIntegration.getImuSettings().updateDynamicWeightDuringOptimization ||
      ...
      dynamicGTSAMWeight = baIntegration->updateDynamicWeight(
          lastEnergy[0],
    ...
    //[ ***step 4*** ] 判断是否接受这次计算
    if (setting_forceAceptStep || (newEnergy[0] + newEnergy[1] + newEnergyL +
                                       newEnergyM / dynamicGTSAMWeight <
                                   lastEnergy[0] + lastEnergy[1] + lastEnergyL +
                                       lastEnergyM / dynamicGTSAMWeight)) {
  ...
  // Update again!
  baIntegration->updateDynamicWeight(
      lastEnergy[0], sqrtf((float)(lastEnergy[0] / (patternNum * ef->resInA))),
      frameHessians.back()->shell->trackingWasGood);
  ...
  frameHessians.back()->setEvalPT(
      frameHessians.back()->PRE_worldToCam,
      newStateZero); // TODO 最新帧设置为线性化点, 待估计量, and calculate
                     // nullspace
  EFDeltaValid = false;
  EFAdjointsValid = false;
  ef->setAdjointsF(
      &Hcalib); // TODO 重新计算adj 因为新加入帧了，重新计算一下伴随的东西
  ...
  //* 把优化结果给shell
  {
    ...
    for (FrameHessian *fh : frameHessians) {
      ...
      fh->shell->aff_g2l =
          fh->aff_g2l(); // TODO 存的不是线性化点的位姿，而是最新估计的状态
      // TODO
