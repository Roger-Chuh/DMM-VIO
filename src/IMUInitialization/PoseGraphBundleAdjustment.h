...
class PoseGraphBundleAdjustment {
 public:
  ...
  // Should be called for every keyframe added, providing the corresponding IMU
  // measurements.
  void addKeyframe(int id, gtsam::PreintegratedImuMeasurements imuMeasurements);

  // This must be called before calling optimize!
  // This method must be called from the BA thread (between optimization and
  // marginalization), wheres optimize can be called later in a different
  // thread.
  void prepareOptimization();

  // Adds IMU factors to the delayed factor graph and then optimizes it (unless
  // noOptimization==true). After calling optimize either
  // optimizationResultNotUsed must be called or
  ...
  // which was added during optimization to the graph, and potentially performs
