...
gtsam::Values dmvio::PoseGraphBundleAdjustment::optimize(gtsam::NonlinearFactor::shared_ptr activeDSOFactor,
  ...
  if (!imuInitValues.empty()) {
    ...
    std::cout << "Updating transform with index " << transformDSOToIMU->getSymbolInd()
...
gtsam::Values dmvio::PoseGraphBundleAdjustment::extendGraph(gtsam::NonlinearFactor::shared_ptr activeDSOFactor,
  ...
  // baValues.
  numIMUFactors =
      insertIMUFactorsAndValues(graph, cachedData, imuInputValues, values, minConnectedPoseInd, imuBias, velocity);
...
gtsam::Values dmvio::PoseGraphBundleAdjustment::buildGraph(gtsam::NonlinearFactorGraph& graph,
                                                           const gtsam::Values& poseInputValues,
                                                           const gtsam::Values& poseInputValues2,
                                                           const gtsam::Values& imuInputValues, bool noOptimization) {
  gtsam::Values values;
  ...
  long long minConnectedPoseInd =
      insertValuesAndGetMinConnectedPoseInd(poseInputValues, poseInputValues2, values, keyVector);
...
int dmvio::PoseGraphBundleAdjustment::insertIMUFactorsAndValues(
    ...
    const Values& imuInputValues, Values& values, long long int minConnectedPoseInd, imuBias::ConstantBias& imuBias,
    Vector3& velocity) {
  int numFactors = 0;
  for (auto&& pair : preintegratedMeasurements) {
    ...
    NonlinearFactor::shared_ptr imuFactor(new ImuFactor(P(prevId), V(prevId), P(id), V(id), prevBiasKey, imuData));

    // The IMUFactor needs to be transformed (from IMU frame to DSO frame).
    auto transformedFactor =
        boost::make_shared<PoseTransformationFactor>(imuFactor, *transformDSOToIMU, settings.conversionType);
    graph.add(transformedFactor);

    auto biasNoiseModel = computeBiasNoiseModel(imuCalibration, imuData);
    NonlinearFactor::shared_ptr bias_factor(new BetweenFactor<imuBias::ConstantBias>(
        prevBiasKey, B(id), imuBias::ConstantBias(gtsam::Vector3::Zero(), gtsam::Vector3::Zero()), biasNoiseModel));
...
void PoseGraphBundleAdjustment::updateGraphMarginalizationOrder() {
  ...
  for (auto& keyVector : marginalizationOrder) {
    ...
    keyVector.erase(std::remove_if(keyVector.begin(), keyVector.end(),
                                   [&dontMargKeys](const gtsam::Key& key) { return dontMargKeys.exists(key); }),
                    keyVector.end());
...
void PoseGraphBundleAdjustment::mainPreparation(const gtsam::Values& optimizedValues) {
  ...
  for (auto&& pair : optimizedValues) {
    ...
    if (chr != 'a' && chr != 'p')  // We don't overwrite DSO variables, as the evaluation point
...
std::unique_ptr<DelayedGraph> PoseGraphBundleAdjustment::prepareGraphForMainOptimization(
...
int PoseGraphBundleAdjustment::getNumOptimizedPoses() const { return numOptimizedPoses; }

int PoseGraphBundleAdjustment::getNumImuFactors() const { return numIMUFactors; }

int PoseGraphBundleAdjustment::getLatestInd() const { return latestInd; }

std::shared_ptr<DelayedGraph> PoseGraphBundleAdjustment::getInputDelayedGraph() const { return inputDelayedGraph; }
