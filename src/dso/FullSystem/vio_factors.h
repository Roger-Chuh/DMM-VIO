...
class DirectVisualFactor {
 public:
  ...
  DirectFactorRes Evaluate(const Mat4& T10, const number_t& idp, const std::shared_ptr<AlgsImage>& target_image,
                           const Patch& patch, CameraBase* camera_ptr, const int& border, Patch::ArrayV& r_vec,
                           number_t& ws2, number_t& r2, Vec2* p_uv = nullptr, Vec3* p_target_dir = nullptr,
                           number_t* p_cos_theta = nullptr, Vec3* p_dp_didp = nullptr, number_t* p_zncc = nullptr,
                           const number_t& pass_threshold = 0.8);

  DirectFactorRes Evaluate_UV(const Patch::Matrix2P& target_uv, const int& border,
  ...
  number_t cos_max_dist_angle_ = 0.34202;                 // cos(70)
