...
std::pair<float, float> TagDetection::interpolate(float x, float y) const {
  ...
  if (z == 0)
    return std::pair<float, float>(0, 0);  // prevents returning a pair with a
                                           // -NaN, for which gcc 4.4 flubs isnan
...
bool TagDetection::overlapsTooMuch(const TagDetection& other) const {
  ...
  float radius = (MathUtil::distance2D(p[0], p[1]) + MathUtil::distance2D(p[1], p[2]) +
                  ...
                  MathUtil::distance2D(other.p[0], other.p[1]) + MathUtil::distance2D(other.p[1], other.p[2]) +
                  MathUtil::distance2D(other.p[2], other.p[3]) + MathUtil::distance2D(other.p[3], other.p[0])) /
                 16.0f;
  // 16.0f;
...
void TagDetection::draw(cv::Mat& image) const {
  ...
  strSt << "#" << id;
  cv::putText(image, strSt.str(), cv::Point2f(cxy.first + 10, cxy.second + 10), cv::FONT_HERSHEY_PLAIN, 1,
              cv::Scalar(0, 0, 255));
}
