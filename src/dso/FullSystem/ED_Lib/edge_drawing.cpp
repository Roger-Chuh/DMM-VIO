...
EdgeDrawingImpl::EdgeDrawingImpl() {
  ...
  nfa = new EdgeDrawingNFALUT(1, 1 / 2, 1, 1);
...
void EdgeDrawingImpl::detectEdges(InputArray src) {
  Mat _src = src.getMat();
  // std::cerr << "[EdgeDrawing::detectEdges] empty=" << _src.empty()
  //           << " type=" << _src.type()
  //           << " (CV_8UC1=" << CV_8UC1 << ", CV_8UC3=" << CV_8UC3 << ")"
  //           << " channels=" << _src.channels()
  //           << " size=" << _src.cols << "x" << _src.rows << std::endl;
...
#define CIRCLE_MIN_LINE_LEN 6
  ...
  for (int i = 0; i < noCircles3; i++) {
    if (circles3[i].isEllipse) {
      ...
      _ellipses.push_back(cv::Vec6d(xc, yc, 0, a, b, theta * 180 / CV_PI));
    } else {
