...
class CameraDetection {
 private:
  ...
  static int computeAprilTagPatternID(int tagId, int pattern_num,
                                      const std::vector<std::pair<int, int>>& startID_endIDs);

  static void dotSingleThreadDetect(const int& camId, const CalibIO::ConfigData& config,
  ...
  static void GetBmpNames(const std::string& path, std::vector<std::string>& filenames);

  void drawPointsRangePic() const;

 public:
  explicit CameraDetection(const CalibIO::ConfigData& config) : m_config(config) {
    int tag_num_per_pattern = m_config.apriltag_config.rows * m_config.apriltag_config.cols;
    m_startID_endIDs.resize(m_config.apriltag_config.pattern_num);
    for (int i = 0; i < m_config.apriltag_config.pattern_num; i++) {
      m_startID_endIDs[i] = (std::make_pair(tag_num_per_pattern * i + m_config.apriltag_config.board_offset,
                                            tag_num_per_pattern * (i + 1) - 1 + m_config.apriltag_config.board_offset));
    }
    ...
    for (unsigned r = 0; r < m_config.apriltag_config.rows * 2; r++) {    // 12
      for (unsigned c = 0; c < m_config.apriltag_config.cols * 2; c++) {  // 12
        ...
        point(0) = (int)(c / 2) * (1 + m_config.apriltag_config.tag_gap) * m_config.apriltag_config.tag_size +
                   (c % 2) * m_config.apriltag_config.tag_size;
        point(1) = (int)(r / 2) * (1 + m_config.apriltag_config.tag_gap) * m_config.apriltag_config.tag_size +
  ...
  bool pipeline(dso::aligned_vector<dso::CalibFrame>& res, CalibIO::ImuJsonData* p_imuData = nullptr,
                bool skip_half_data = false, dso::aligned_vector<dso::CalibFrame>* p_rgb_data = nullptr) {
 ...
 private:
  ...
  Eigen::Matrix<number_t, Eigen::Dynamic, Eigen::Dynamic, Eigen::RowMajor> m_grid_points;
};
