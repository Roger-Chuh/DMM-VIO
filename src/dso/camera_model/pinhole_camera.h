...
class PinholeCamera : public CameraBase {
 public:
  ...
  explicit PinholeCamera(CamId camera_id, int width, int height) : CameraBase(camera_id, width, height) {
    camera_model_ = CameraModel::kPinhole;
    kParamLength = 4;
  }

  // parameters: fx, fy, cx, cy
  PinholeCamera(CamId camera_id, int width, int height, const number_t* parameters)
