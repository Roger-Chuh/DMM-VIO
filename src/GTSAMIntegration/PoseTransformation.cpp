...
std::vector<gtsam::Matrix> PoseTransformation::getAllDerivatives(const PoseType& pose, DerivativeDirection direction) {
  std::vector<gtsam::Matrix> returning;
  returning.push_back(getPoseDerivative(pose, direction));
  return returning;
}

std::vector<gtsam::Key> PoseTransformation::getAllOptimizedSymbols() const { return std::vector<gtsam::Key>(); }
...
std::pair<gtsam::Matrix, gtsam::Vector> dmvio::convertHAndBFromDSO(
    const dso::MatXX& HInput, const dso::VecX& bInput, PoseTransformation& poseTransformation, double weightDSOToGTSAM,
    const gtsam::Ordering& ordering, const gtsam::Values& values, const std::map<gtsam::Key, size_t>& keyDimMap) {
  ...
  auto pair = convertHAndBWithPoseTransformation(std::make_pair(H, b), ordering, keyDimMap, values, poseTransformation,
...
std::pair<gtsam::Matrix, gtsam::Vector> dmvio::convertHAndBWithPoseTransformation(
  ...
  gtsam::Matrix bigJ =
      buildRelativeJacobian(n, ordering, sizes, values, poseTransformation, derivativeDirection, numOpt, optPositions);
...
gtsam::Matrix dmvio::buildRelativeJacobian(int n, const gtsam::FastVector<gtsam::Key>& ordering,
...
std::pair<gtsam::Matrix, gtsam::Vector> dmvio::convertCoarseHToGTSAM(PoseTransformation& transform,
  ...
  std::vector<gtsam::Matrix> derivatives =
      transform.getAllDerivatives(currentPose.matrix(), DerivativeDirection::RIGHT_TO_LEFT);
